...
| Stopped         | Radius stopped at a safe boundary.                                                               | Choose **Continue setup**, **Roll back environment setup**, or **Exit setup** when offered. |
...
The export contains the installed plugin version, generated operation ID, schema version, lifecycle and stage states, timestamps and duration, attempt and command counts, failure classification, cleanup outcome counts, provider-recovery status counts, and whether verification was dispatched.
