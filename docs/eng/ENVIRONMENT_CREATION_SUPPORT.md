...
Expand **Show details**, then activate **Download diagnostic snapshot**. The link appears only after Stop is requested, while Radius is waiting for input, or after a non-success terminal outcome such as failure, cancellation, or external action required. It stays hidden after a successful setup, including success with warnings, and during normal forward progress, retry, reconciliation, and cleanup. For support during a long-running operation, request Stop first; Radius records the request and exposes the snapshot without interrupting an in-flight provider mutation. Radius creates a local JSON file named `radius-environment-operation-diagnostics.json`. The browser does not upload it, and Radius does not create a second retained diagnostic record.

The review dialog lists what is always included. The default `support_safe` export contains the installed plugin version, generated operation ID, schema version, lifecycle and stage states, timestamps and duration, attempt and command counts, failure classification, cleanup outcome counts, provider-recovery status counts, whether verification was dispatched, and the allowlisted state of its saved verification workflow.

The default export excludes repository and environment names, Azure tenant, subscription, application, principal, role-assignment, and resource identifiers, GitHub account, branch, workflow, run, package, and resource identifiers, persisted request and resume inputs, resource labels and targets, URLs, command lines, environment variables, idempotency keys, stdout, stderr, logs, evidence, free-form error messages, and step text. Unknown future states appear only as `unknown`, with `unrecognizedValueCount` showing how many values were omitted.

Select **Include contextual identifiers** only when support needs the exact repository, branch, environment, or GitHub login to diagnose an account or branch mismatch. Radius loads a server-built preview and requires **I reviewed these identifiers** before download. The preview is fingerprinted; if any value changes before download, Radius refuses the file and asks for another review. The contextual profile contains only those four bounded values. It still excludes credential-profile names, worktree paths, cloud resource identities, workflow and run identities, and every secret or raw evidence field. Treat names in the file as untrusted customer data, inspect the file before sharing it, and use the organization's approved support channel.
