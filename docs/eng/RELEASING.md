...
A plugin whose `keywords` include `canvas` follows the [Awesome Copilot canvas-extension guide](https://github.com/github/awesome-copilot/blob/main/CONTRIBUTING.md#adding-canvas-extensions) for its manifest and preview, and carries its runtime entry point at the published plugin root:

| Requirement                                                             | Where it is produced                          |
|-------------------------------------------------------------------------|-----------------------------------------------|
| `plugin.json#extensions.com.github.copilot.logo` = `assets/preview.png` | tracked in `plugins/<plugin>/plugin.json`     |
| `assets/preview.png`                                                    | tracked in `extensions/<plugin>/assets/`      |
| `package.json#main` = `extension.mjs`                                   | tracked in `extensions/<plugin>/package.json` |
| `extension.mjs`                                                         | generated by `build.mjs` at the artifact root |

The namespaced `extensions` object is valid Agent Plugins 1.0.0 metadata. `validate-plugin-dist.mjs` checks the whole manifest against the live schema, requires the exact namespaced preview path and file, and validates the root entry point independently and again through `package.json#main`. A path-valued `plugin.json#extensions` is rejected; the artifact has no nested `extensions/extension.mjs` compatibility wrapper. Do not add `com.github.awesome-copilot`: that namespace is source-composition metadata consumed by Awesome Copilot's own materializer, while Radius publishes an already-assembled external plugin whose skills use the standard `skills/` location.

The source tree differs from Awesome Copilot's hand-authored example only at the build boundary: `extensions/<plugin>/extension.mjs` is generated because Radius bundles TypeScript workspace packages, so it exists at edge and stable release refs rather than on `main`. Current [external-plugin intake](https://github.com/github/awesome-copilot/blob/main/CONTRIBUTING.md#adding-external-plugins) also expects a legacy top-level `logo` and an additional `extensions/extension.mjs` or `extensions/<extension>/extension.mjs` beneath `source.path`. The schema-compliant, root-only Radius artifact deliberately carries neither legacy shape, so it cannot pass that external canvas gate until upstream reconciles it with the canvas-extension guide. Once that blocker is removed, submit `source.path: extensions/<plugin>` with the immutable stable `<plugin>@<version>` tag and its full artifact commit SHA, never the mutable `<plugin>@edge` ref used by this repository's default marketplace.
