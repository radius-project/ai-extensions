...
- **force-recreates `releases/<plugin>/edge` as an orphan branch** whose single root commit contains that otherwise git-ignored tree published as `extensions/<plugin>/`, the complete `.github/extension/` workflow/action tree, and the marketplace catalog, and
...
`.artifacts/<plugin>/` is a complete, self-contained plugin: `plugin.json`, `package.json`, `README.md`, `CHANGELOG.md`, `assets/`, `extensions/`, all of `skills/`, the compiled `extension.mjs` (plus its source map), and `workflows/`, a complete copy of `.github/extension/` including reusable workflows, composite actions, scripts, and their checked-in bundles. The plugin's own `build` script assembles it from the two tracked source directories, wiping and rebuilding the tree each time - and refusing to wipe it at all if `git ls-files` reports anything tracked there. `package.json#radiusSourceRef` and the compiled generator both carry the full source commit, so an installed extension never follows a mutable ref when fetching a template or generating a first-party action reference.

`assets/` is tracked extension source copied verbatim; `extensions/` is generated, because its only content points at the build output. See [the canvas contract](#the-canvas-contract) for why both exist.
...
Because the branch is an orphan, it carries **no monorepo source and no history** - a clone of `releases/<plugin>/edge` contains only the artifact, its workflow/action assets, and the catalog. The complete workflow tree appears twice: `extensions/<plugin>/workflows/` ships inside the installable plugin and stable tarball, while root `.github/extension/` preserves the same assets at their canonical repository paths on the self-contained branch. Generated `radius-project/ai-extensions/.github/extension/actions/...@<source-sha>` references resolve from the immutable source commit, not the orphan branch commit. Verification requires the two artifact copies to have identical paths and blob SHAs. The commit message and built package independently record the full source SHA.

The plugin source in [`.github/plugin/marketplace.json`](../../.github/plugin/marketplace.json) pins `path: extensions/<plugin>` - the path the assembled tree is published at, not the canvas source that sits there on `main`. Each entry follows its `<plugin>@edge` tag while that plugin is in preview; changing its ref to a released `<plugin>@<version>` makes stable the default later, for that plugin alone. Generated edge and stable catalogs point their own copies at the corresponding artifact ref. Each install therefore resolves matching skills and canvas files from one generated artifact commit. This is why the build output can stay git-ignored on `main`.
...
A plugin whose `keywords` include `canvas` owes [`github/awesome-copilot`](https://github.com/github/awesome-copilot) three things at the submitted ref, resolved against the entry's `source.path`:

| Requirement                               | Where it is produced                             |
|-------------------------------------------|--------------------------------------------------|
| `plugin.json#logo` = `assets/preview.png` | tracked in `plugins/<plugin>/plugin.json`        |
| `plugin.json#extensions` = `extensions`   | tracked in `plugins/<plugin>/plugin.json`        |
| `assets/preview.png`                      | tracked in `extensions/<plugin>/assets/`         |
| `extensions/extension.mjs`                | generated by `build.mjs` into the assembled tree |

The keyword is the trigger: drop it and none of these are checked, but the plugin also stops being listed as a canvas.

Both manifest fields are **outside Agent Plugins 1.0.0**, whose schema is closed and types `extensions` as a client-namespace object. Their intake requires them anyway, and their own spec-compliance gate reports the deviation as a non-blocking warning. `validate-plugin-dist.mjs` therefore withholds exactly these two fields from the schema check for a canvas plugin and enforces them directly instead - each must hold the literal the intake expects **and** resolve to a real file or directory inside the dist, with `extensions/` containing `extension.mjs` or `<extension>/extension.mjs`. For any other plugin both fields are still rejected by the closed schema.

The canvas entry point re-exports the bundle at the plugin root rather than duplicating it, so `package.json#main`, the module-relative skill lookup, and the credentials path are all unaffected by its presence.

This repository splits its source the same way they do - the manifest under `plugins/<name>/`, the hand-authored extension under root `extensions/<name>/` - so a future plugin can compose an existing extension without moving files. Where the two diverge is the materialization: they run `eng/materialize-plugins.mjs` into `plugins/<name>/com.github.copilot/extensions/<name>/` on their `marketplace` branch and serve those plugins directly, so they never meet the intake rules above. This repository instead assembles into `.artifacts/<name>/`, publishes that tree at `extensions/<name>/` on an orphan release branch, and shapes it to the intake contract.

### Submitting a release to awesome-copilot

Their [external plugin issue form](https://github.com/github/awesome-copilot/blob/main/CONTRIBUTING.md#adding-external-plugins) takes the locators below. The release model satisfies their ref/sha and version gates by construction, because the release tag is placed **on** the artifact commit:

| Field         | Value                                  |
|---------------|----------------------------------------|
| `source.repo` | `radius-project/ai-extensions`         |
| `source.path` | `extensions/<plugin>`                  |
| `source.ref`  | `<plugin>@<version>`                   |
| `source.sha`  | the artifact commit that tag points at |
| `version`     | the released version                   |

Their gates re-check every canvas path at both `ref` and `sha`, and require the manifest version at each to equal the submitted version. Because the orphan branch contains only the plugin, their recursive tree fetches cannot truncate the way they would against a monorepo root.
