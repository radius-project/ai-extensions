...
`.artifacts/<plugin>/` is a complete, self-contained plugin: `plugin.json`, `package.json`, `README.md`, `CHANGELOG.md`, `assets/`, all of `skills/`, the compiled root `extension.mjs` (plus its source map), a nested `extensions/<plugin>/` package with its re-export and matching `package.json`, and `workflows/`, a complete copy of `.github/extension/` including reusable workflows, composite actions, scripts, and their checked-in bundles. The plugin's own `build` script assembles it from the two tracked source directories, wiping and rebuilding the tree each time - and refusing to wipe it at all if `git ls-files` reports anything tracked there. `package.json#radiusSourceRef` and the compiled generator both carry the full source commit, so an installed extension never follows a mutable ref when fetching a template or generating a first-party action reference.
