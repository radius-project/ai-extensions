...
**As of this writing, no run has completed the cloud lifecycle.** Before the fixture was published, scheduled runs completed successfully by stopping after fixture resolution and skipping every cloud step. The private [`radius-project/ai-extensions-fixture`](https://github.com/radius-project/ai-extensions-fixture) repository is now fully provisioned through [`radius-project/ai-extensions-fixture#4`](https://github.com/radius-project/ai-extensions-fixture/pull/4), and this repository pins its current `main` baseline at commit `1b9cb3a56ebc52792c0665c2d8497469f65da759`. The fixture repository is no longer a blocker. Five external prerequisites remain:
...
`AIEXT_CLOUD_E2E_FIXTURE_REPOSITORY` is published as `radius-project/ai-extensions-fixture`. The workflow checks that value against the source pin before authenticating to the fixture; any disagreement fails before `pnpm test:cloud` runs. What is verified today is the workflows' safety and wiring contracts, asserted in [`packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts`](../../packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts), and the journey's own decision logic, asserted in the `test/e2e-cloud/support/*.test.ts` unit suites. Nothing verifies that a real run succeeds. **No stage - create, deploy, deployment deletion, or environment deletion - has completed against real cloud.**
