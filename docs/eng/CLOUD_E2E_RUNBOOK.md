...
| The job is cancelled at 90 minutes               | The run genuinely hung                                                        | This should not happen: Playwright caps each test at 45 minutes and the whole suite at 75, writing the trace first. If GitHub cancelled first, the ordering is broken - see the timeout comment in `cloud-e2e.yml` |
