...
| Deploy reaches `complete` but no ready labelled workload exists on AKS         | The workflow went green without landing a runnable Radius application        | Use the test's explicit kubeconfig and query `radapp.io/application=<app>` in namespace `default-<app>`            |
