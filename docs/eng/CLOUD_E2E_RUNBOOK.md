...
The short version, for whoever picks up a red **Cloud E2E** run. The [Cloud E2E design](../design/2026-08-cloud-e2e-environment-lifecycle.md) explains why the tier exists and how it is built; this page is only what you do.
...
- **A complete run deliberately retains shared Azure identity.** Stage five proves the product deletes the GitHub Environment and every federated credential observed in stage one. The exact repository-scoped Entra application, service principal, and complete role-assignment inventory survive by design because other environments can share them, so `reclaimLeakedProductArtifacts()` removes that test-only residue after the assertions. The fixture retains unit-tested app-registration and role-assignment absence mirrors for any future contract that can prove exclusive ownership, but the current journey does not call them.
...
**As of this writing, neither workflow has ever executed, and neither can.** The private [`radius-project/ai-extensions-fixture`](https://github.com/radius-project/ai-extensions-fixture) repository and its baseline are prepared in [`radius-project/ai-extensions-fixture#1`](https://github.com/radius-project/ai-extensions-fixture/pull/1) and [`radius-project/ai-extensions-fixture#2`](https://github.com/radius-project/ai-extensions-fixture/pull/2), and this repository pins commit `07deb510c0a663047eca085f429e51c8bea384f1`. Five external prerequisites remain:
...
4. The dedicated Cloud E2E GitHub App must be created and installed on the fixture repository alone with `actions: write`, `administration: read`, `contents: write`, `deployments: read`, `environments: write`, `pull requests: write`, `variables: write`, and `workflows: write`. Store its client ID and private key on `radius-project/ai-extensions` as `CLOUD_E2E_BOT_CLIENT_ID` and `CLOUD_E2E_BOT_PRIVATE_KEY`; the encrypted private key is used only to mint a short-lived installation token for each job. Neither this repository nor `wellknown#134` provisions those two secrets.
5. A dedicated machine or bot account with permission to create and delete packages in the fixture owner's namespace must provide a classic personal access token with `read:packages`, `write:packages`, and `delete:packages`. Store that account's login and token on `radius-project/ai-extensions` as `CLOUD_E2E_PACKAGES_USER` and `CLOUD_E2E_PACKAGES_TOKEN`. The journey exposes them only through the isolated `GH_PACKAGES_USER` and `GH_PACKAGES_TOKEN` package path; repository and workflow APIs continue to use the short-lived fixture-scoped GitHub App token. A GitHub App installation token cannot manage GitHub Packages, and the `ai-extensions` workflow token cannot bootstrap access to a new package linked to another repository.
The `aiext_fixture_repository` tfvar remains unset, so `AIEXT_CLOUD_E2E_FIXTURE_REPOSITORY` is not published. While that variable is unpublished, a dispatched run stops after checkout with a specific notice and the cleanup workflow does not reach destructive steps. If a published value disagrees with the source pin, the `Verify the published fixture repository matches the pin` step fails before `pnpm test:cloud` runs. Neither case passes hollowly. What is verified today is the workflows' safety and wiring contracts, asserted in [`packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts`](../../packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts), and the journey's own decision logic, asserted in the `test/e2e-cloud/support/*.test.ts` unit suites. Nothing verifies that a real run succeeds. **No stage - create or delete - has ever executed against real cloud.**
