...
**As of this writing, neither workflow has ever executed, and neither can.** Five prerequisites are outstanding, none of them in this repository:
...
5. The fixture repository's private GHCR state packages must grant the `radius-project/ai-extensions` Actions repository admin access. The journey uses its short-lived GitHub App token for repository and workflow APIs, but uses the workflow's native `GITHUB_TOKEN` through the isolated `GH_PACKAGES_TOKEN` and `GH_PACKAGES_USER` path for package preflight, deletion, leak detection, and cleanup; GitHub App installation tokens cannot manage GitHub Packages.
