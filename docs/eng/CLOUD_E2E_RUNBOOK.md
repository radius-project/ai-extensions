...
The cleanup workflow runs automatically twice daily. For each expired test environment, it first dispatches the fixture repository's `delete-application.yml` workflow and waits for Radius to delete the Kubernetes and Azure resources in the application, including the Azure-backed PostgreSQL resource. Only after that workflow succeeds does cleanup remove the run's Entra identity, Azure role assignments, GitHub deployment records, GitHub Environment, GHCR state package, generated workflow branches, and repository changes. It also retains the legacy sweep for tagged per-run resource groups created by older workflow revisions. The sweep explicitly excludes `AIEXT_CLOUD_E2E_RESOURCE_GROUP`, even if that group later matches the legacy prefix and tags. It never deletes the configured shared resource group or AKS cluster. **Run it by hand rather than deleting things yourself**:
...
- **The shared AKS cluster is fixture scaffolding, not disposable test output.** CI requires the cluster to report provisioning state `Succeeded` and power state `Running`, then uses it for discovery and deployment. Neither fixture teardown nor scheduled cleanup deletes it. Radius application deletion removes the application's Azure and Kubernetes resources first. A label-scoped Kubernetes deletion then removes any residual application workloads without deleting the namespace or unrelated workloads.
...
- **Product deletion and fixture reclamation own different artifacts.** Deployment deletion preserves the GitHub Environment, its variables, the repository-scoped Entra application, its federated credentials, and its role assignments so the environment can deploy again. Final environment deletion removes the GitHub Environment and its per-environment federated credentials, as implemented by `radius-project/ai-extensions#398`, while deliberately retaining the shared Entra application and role assignments. After assertions, `reclaimLeakedProductArtifacts()` runs Radius application deletion before removing identity or recovery state, then removes only the three expected assignments for the product-created service principal at the configured resource-group and AKS scopes.
- **A deployment record outlives the environment that names it.** GitHub links a deployment to its environment by name rather than by id, so deleting the environment strands its records instead of removing them, and the product never deletes them — to it, a deployment record is history, not an owned resource. Only the suite that created them can, so both fixture teardown and scheduled cleanup delete them explicitly, before the environment. Each deletion is two calls: mark the record `inactive`, then `DELETE` it. GitHub rejects deleting an active record, and a record whose job never reported a status is active by default. If any record cannot be reclaimed, both paths hold the environment back rather than deleting it, so the next sweep can still find the records by name and retry.
- **Cleanup fails closed.** If Radius application deletion fails, cleanup preserves the Entra identity, role assignments, GitHub Environment, GHCR state package, generated delete workflow, and repository state so a developer can retry or investigate. If a role assignment is unexpected or cannot be removed, cleanup preserves both the service principal and its parent application registration; deleting the application would cascade-delete the principal and leave an orphaned role assignment. A `gh` command that exits zero is treated as a request rather than proof: teardown polls Entra until a deleted application registration is actually absent, because a run once reported the deletion succeeded while the application was still live, wedging the next run's clean-slate check.
