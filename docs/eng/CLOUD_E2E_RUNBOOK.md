...
**As of this writing, neither workflow has executed successfully.** The fixture repository is provisioned and pinned, but these external prerequisites still block a real run:
...
3. The `RADIUS_GHOPERATIONS_BOT` GitHub App must be installed on `radius-project/ai-extensions` with `metadata: read`, `secrets: write`, and `variables: write`. Terraform cannot install an App, so that half is manual; repository administration access is not required because the Cloud E2E identity keeps GitHub's default OIDC subject.
4. The dedicated Cloud E2E GitHub App must be created and installed on the fixture repository alone with `actions: write`, `administration: read`, `contents: write`, `deployments: read`, `environments: write`, `pull requests: write`, `secrets: write`, `variables: write`, and `workflows: write`. `actions: write` dispatches the deploy and delete workflows and lets the job cancel a run it started; `deployments: read` lets the journey list and inspect GitHub Deployments to prove one exists or is gone; `secrets: write` lets a secure Bicep parameter be stored as the Environment's `RADIUS_DEPLOY_PARAMS` secret. Store its client ID and private key on `radius-project/ai-extensions` as `CLOUD_E2E_BOT_CLIENT_ID` and `CLOUD_E2E_BOT_PRIVATE_KEY`; the encrypted private key is used only to mint a short-lived installation token for each job. Neither this repository nor `wellknown#134` provisions those two secrets.
5. The organization-level `GH_RAD_CI_BOT_PAT` Actions secret must include `radius-project/ai-extensions` in its selected-repository access. The classic PAT belongs to `rad-ci-bot` and requires `read:packages`, `write:packages`, and `delete:packages`; its value remains stored once at organization scope. Store only the account login on `radius-project/ai-extensions` as `CLOUD_E2E_PACKAGES_USER`. The journey exposes the organization secret through the isolated `GH_PACKAGES_TOKEN` package path, while repository and workflow APIs continue to use the short-lived fixture-scoped GitHub App token. A GitHub App installation token cannot manage GitHub Packages, and the `ai-extensions` workflow token cannot bootstrap access to a new package linked to another repository.
The private fixture repository now exists and [`FIXTURE_BASELINE_SHA`](../../packages/adapter-canvas/test/e2e-cloud/support/fixture-repository.ts) pins its immutable `main` commit. While the fixture variable is unpublished, a dispatched run stops after checkout with a specific notice and the cleanup workflow no-ops before authenticating to Azure or GitHub. If the published value disagrees with the checked-in repository pin, both workflows fail before touching the fixture. Neither case passes hollowly. What is verified today is the workflows' safety and wiring contracts, asserted in [`packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts`](../../packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts), and the journey's own decision logic, asserted in the `test/e2e-cloud/support/*.test.ts` unit suites. Nothing verifies that a real run succeeds. **No stage - create, deploy, deployment deletion, or environment deletion - has ever executed against real cloud.**
