...
While the fixture variable is unpublished, a dispatched run stops after checkout with a specific notice and the cleanup workflow no-ops on its provisioned check. If the variable is published before the source pin is updated, the `Verify the published fixture repository matches the pin` step fails before `pnpm test:cloud` runs. Neither case passes hollowly. What is verified today is the workflows' safety and wiring contracts, asserted in [`packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts`](../../packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts). Nothing verifies that a real run succeeds.
