...
**As of this writing, neither workflow has executed successfully.** The fixture repository, Azure identity, and repository configuration are provisioned. One external prerequisite remains:

1. Create a dedicated GitHub machine user used only by Cloud E2E, grant it administrator access to the private `radius-project/ai-extensions-fixture` repository, and allow it to create and delete packages in the `radius-project` namespace. Create a classic personal access token with `repo`, `workflow`, `read:packages`, `write:packages`, and `delete:packages`. Store the login and token on `radius-project/ai-extensions` as `CLOUD_E2E_GITHUB_USER` and `CLOUD_E2E_GITHUB_TOKEN`.

The same token is passed as `GH_TOKEN` for repository, environment, workflow, deployment, and cleanup operations and as `GH_PACKAGES_TOKEN` for GHCR bootstrap and cleanup. This deliberately mirrors a developer using one GitHub CLI identity for the extension. `delete:packages` is specific to the test harness: normal extension setup needs package read and write access, while Cloud E2E must also remove its disposable state package.

The private fixture repository exists and [`FIXTURE_BASELINE_SHA`](../../packages/adapter-canvas/test/e2e-cloud/support/fixture-repository.ts) pins its immutable `main` commit. While the fixture variable is unpublished, a dispatched run stops after checkout with a specific notice and the cleanup workflow no-ops before authenticating to Azure or GitHub. If the published value disagrees with the checked-in repository pin, both workflows fail before touching the fixture. Neither case passes hollowly. What is verified today is the workflows' safety and wiring contracts, asserted in [`packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts`](../../packages/adapter-canvas/test/ci/cloud-e2e-workflows.test.ts), and the journey's own decision logic, asserted in the `test/e2e-cloud/support/*.test.ts` unit suites. Nothing verifies that a real run succeeds. **No stage - create, deploy, deployment deletion, or environment deletion - has ever executed against real cloud.**
