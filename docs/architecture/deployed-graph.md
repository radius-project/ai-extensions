...
Nested `outputResources` do not become graph nodes. The route merges them from `deploy-graph.json` onto the exact modeled parent id and the deploy-mode renderer uses the same representative-output selection as Planned graph. When final deployment metadata omits `displayType`, the merge carries the matching planned output's friendly name forward by exact output id or normalized concrete type. A failed deployment takes this presentation metadata from the exact deployment attempt snapshot and copies only `type` and `displayType`; it never carries planned resource IDs or portal URLs into the deployed view. The selected output's non-empty `displayType` becomes the node's primary label, such as **Azure Database for MySQL**; when no friendly name is available, the renderer falls back to the normalized concrete `type`. The exact concrete type remains available in the node tooltip and details panel without adding lock, database, or firewall children to the topology. When final metadata is missing or does not match an exact parent id, the modeled resource and its existing metadata remain unchanged; no provider type is inferred.
