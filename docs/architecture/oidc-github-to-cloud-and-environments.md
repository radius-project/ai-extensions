...
### C.3 `adapters/canvas/src/azure-oidc.ts` — Azure decision logic
...
### C.4 `adapters/canvas/src/gh.ts` — GitHub identity & multiple accounts
...
2. `adapters/canvas/src/azure-oidc.ts` + `azure-oidc.test.ts` — the pure Azure
   decisions (app selection, SMR, FIC dedup, validation).
3. `adapters/canvas/src/gh.ts` + `gh.test.ts` — multi-account identity and GHCR
