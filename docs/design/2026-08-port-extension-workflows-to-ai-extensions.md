...
- **Retired mechanical parity check** — `extension-parity.live.test.ts` guarded the migration by proving this repo's `.github/extension/` tree matched the pinned Radius source after the documented transforms. The guard was removed after [radius#12719](https://github.com/radius-project/radius/pull/12719) merged and deleted the duplicate Radius tree; `ai-extensions` is now the sole source of truth.
...
- **Manual / functional (required at cutover)** — the live tests proved template validity and the retired migration guard proved tree parity, but they ran inside `ai-extensions` and did **not** prove that a target user repo could resolve the remote composite actions or that catalog loading and deployment worked end to end. Recording a successful Azure and AWS deploy from a target repo using the generated workflows — including `load-contrib-catalog` — was therefore a gate before the Radius copy was deleted.
...
1. **Only then remove from radius.** Done: [radius#12719](https://github.com/radius-project/radius/pull/12719) merged and deleted the duplicated `.github/extension/` and its consumer-side wiring while keeping `deploy/manifest/defaults.yaml` in Radius for immutable runtime fetches.
