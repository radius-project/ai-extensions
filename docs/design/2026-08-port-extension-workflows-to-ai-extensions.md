...
- **Mechanical parity check** — `extension-parity.live.test.ts` proves this repo's entire `.github/extension/` tree equals the Radius tree it was ported from (pinned commit `RADIUS_PARITY_REF`) after the documented repoint transform. It fails on any missing asset, unexpected extra file, or content drift, so a stale or partial port cannot ship. Two files are intentionally exempt from byte comparison (existence only): the self-contained `load-contrib-catalog/action.yml` and `README.md`; `install-yq.sh` is the one ai-extensions-only addition.
- **Action shell tests** — the ported `*_test.sh` files (e.g. `compute-build-platforms_test.sh`, `deploy-parameters_test.sh`, `publish-deploy-status_test.sh`, `apply-custom-recipe-packs_test.sh`, `progress_test.sh`, `refresh-azure-oidc-token_test.sh`, `action-shell-syntax_test.sh`, `verify-azure_test.sh`) exercise the action scripts and are validated by `shellcheck` and YAML parse.
...
1. **Prove it works.** CI on #424 is green (build, Vitest, lint, live YAML validity, parity check) **and** a real Azure and AWS deploy from a target user repo succeeds end to end against the ai-extensions copy — exercising the remote composite-action resolution and `load-contrib-catalog` that the in-repo live tests cannot cover. This runtime evidence is the final gate.
...
1. **Radius duplicate removal timing.** The removal PR ([radius#12719](https://github.com/radius-project/radius/pull/12719)) is ready and parity is now complete and mechanically enforced (`extension-parity.live.test.ts`, pinned to `radius/main@921e55c6`). The one remaining gate is recording a successful end-to-end Azure and AWS deploy from a target repo against the ai-extensions copy; the removal stays in draft until that runtime evidence exists.
