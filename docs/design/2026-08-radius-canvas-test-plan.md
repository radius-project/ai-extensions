...
- Keep tests local and repeatable. Do not use personal credentials, live cloud resources, mutable repositories, or public network assets. This applies to the eleven hermetic layers, which is every layer that gates a pull request. The [Cloud E2E layer](./2026-08-cloud-e2e-environment-lifecycle.md) is the single, explicit exception: it exists to prove the facts a fake cloud cannot, runs only on a schedule or on demand, never gates a pull request, and uses dedicated test credentials and a dedicated fixture repository rather than personal ones.
