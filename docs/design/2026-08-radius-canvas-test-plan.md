...
- Keep pull request checks local and repeatable. Do not use personal credentials, live cloud resources, mutable repositories, or public network assets. The [Cloud E2E layer](./2026-08-cloud-e2e-environment-lifecycle.md) is the single exception: it uses dedicated test credentials and a dedicated fixture repository, runs only on a schedule or by manual dispatch, and never gates a pull request.
...
| Cloud E2E                   | Never required for merge; scheduled and on demand                                | Acceptance of requests and resulting state in real external services    |
