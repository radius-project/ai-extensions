...
> **Citation style.** This document cites **symbol names**, never file line numbers. An
> earlier draft cited lines, and by first review every one of them had drifted onto an
> unrelated symbol — `writeFakeCli` had moved from 222 to 445, and line 222 had become
> `planHarnessProcess()`. A stale line number is worse than no citation, because it reads
> as precise. Symbol names survive edits above them and are greppable when they do change.
...
but it needs a *second* fixture repository to hold a durable application, because it
cannot share one with a suite whose precondition is that no application exists. Recording
the trade-off matters more than the choice: a future change that "fixes" clean-slate by
leaving the application behind would silently convert stage one into a tautology.

The fixture must not dodge this by making the name unique per run. The product owns that
derivation, and overriding it in test would exercise something no user ever runs.
...
Discovery being subscription-wide is correct product behaviour — a user picks from everything they can reach — so this is a constraint on the test, not a defect to file.
...
**What we reuse.** Nearly everything. `wellknown` already has an `entra` submodule that creates a federated identity and assigns it Azure roles, so the new identity is a second use of that module rather than new Terraform. It already fans repository secrets out to more than one repository through an additional-secrets map. And granting Microsoft Graph roles is an established pattern there: `00-tfbackend` grants five Graph roles to the bootstrap service principal, `Application.ReadWrite.OwnedBy` among them. The identity work below therefore mirrors an existing file.

**What we add.**

1. **A dedicated bootstrap identity**, declared as a *second instance* of the `entra` submodule rather than a new key in the shared `github_repositories` map. It federates `ai-extensions` on `main` only and holds subscription `Contributor`, `Role Based Access Control Administrator`, and Graph `Application.ReadWrite.OwnedBy`. A separate instance is what keeps the Graph grant off the shared Radius identity, which is the entire point of having a dedicated one. Pull-request and merge-queue federation stay off, because this identity holds subscription `Contributor`, RBAC administration, and a Graph grant, and pull-request federation would let a workflow triggered from any pull request mint a token for it.
2. **`ai-extensions` added to the existing additional-secrets map**, which another repository already uses.
3. **An additional-variables map** — the only genuinely new capability in this list. Secrets already fan out to extra repositories, but variables are hard-bound to a single repository, so there is currently no way to publish `AIEXT_CLOUD_E2E_AZURE_LOCATION` and `AIEXT_CLOUD_E2E_FIXTURE_REPOSITORY` to `ai-extensions` at all.
...
- **Federating `ai-extensions` changes its repository-wide OIDC subject format.** The `entra` submodule hardcodes subjects of the form `repository_owner_id:<id>:repository_id:<id>:<context>`, and its credentials only match when the customization template is managed for that repository. This one is a consequence of reusing the submodule rather than a defect we can fix or a setting we can decline, so it is accepted rather than fixed, and its downstream effect on release provenance is tracked in the [Security](#security) table.
