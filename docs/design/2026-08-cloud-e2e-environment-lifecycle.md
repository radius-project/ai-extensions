...
| Test crashes mid-run                               | `dispose()` runs from an `always()` teardown. The application cleanup target is registered before deployment dispatch, so normal failure teardown deletes only its labelled deployments and pods. If the runner dies outright, scheduled cleanup reclaims product-created Entra and GitHub state but deliberately preserves the shared resource group and AKS cluster.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Run succeeds without reaching the delete stage     | `dispose()` owns this, not the product. A stage-one-only run leaves an application registration and a GitHub Environment that no product code path will ever remove, because nothing asked the product to delete them. `dispose()` therefore deletes both unconditionally at the end of every run, successful or not, and its deletions are idempotent so they are safe after a journey that already deleted them itself. This is also why cleanup deletes the application rather than leaving it for reuse.                                                                                                                                                                                                                                                                                              |
