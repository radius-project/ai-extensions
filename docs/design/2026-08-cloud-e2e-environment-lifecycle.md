...
- The configured shared resource group and AKS cluster are never cleanup targets. The legacy resource-group sweep requires both the `radtest-canvas` prefix and the `radius-canvas-e2e=true` ownership tag, so it cannot match `ai_extensions_test`.
...
2. **A purpose-built Azure resource role scoped to the precreated resource group.** The runner no longer creates or deletes resource groups or AKS clusters. It needs only the resource and credential reads used for discovery, verification, deployment diagnostics, and `az aks get-credentials`. The role remains separate from RBAC delegation so their different blast radii stay visible.
...
| Precreated AKS validation fails                    | Fail before the product journey with the `az aks show` error attached. A missing cluster, inaccessible scope, or location mismatch is infrastructure failure, not a product regression.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Test crashes mid-run                               | `dispose()` runs from an `always()` teardown. If the runner dies outright, scheduled cleanup reclaims product-created Entra and GitHub state but deliberately preserves the shared resource group and AKS cluster.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
