...
Almost every step is an external side effect. Today no test performs any of them for real. Every test layer replaces `gh`, `az`, `rad`, and `kubectl` with a generated fake, so the suite proves the extension _sends_ the right commands, never that Azure and GitHub _accept_ them or that the resulting credential actually works.
...
for _CI_, where there is no human to be signed in. A developer running the command above
...
| `GH_TOKEN`         | A placeholder string (`PLACEHOLDER_SECRET`)             | Read from the environment (developer or machine-user PAT) |
...
A per-run cluster is the price of testing the Azure discovery path honestly. The cost is real: roughly 10-12 minutes per run plus a small nightly spend. The smallest node pool suffices, since the cluster is only a deploy target. If that proves too expensive the fallback is a second long-lived cluster dedicated to this suite — trading per-run cost for the shared-state weakness — as a deliberate later decision. [#789](https://github.com/radius-project/ai-extensions/issues/789) tracks establishing an observed cost envelope and alerting on abnormal spend; billing data is delayed diagnostic evidence, not a synchronous lifecycle assertion.

**The per-run resource group does not isolate cluster discovery, so the journey must select by name.** This is easy to get wrong, because every _other_ resource in this design is isolated by the resource group. Cluster listing is not. [`discoverResources()`](../../packages/adapter-canvas/src/server/services/discovery.ts) builds `az aks list` with a `--query` projection and an optional `--subscription`, and **no `--resource-group`**, so it returns every cluster in the shared subscription — including the Radius long-running cluster and anything another team is running. It also projects `id` from the cluster _name_ rather than the ARM resource id, so clusters are identified by name alone.

Three consequences the journey has to honour:

- **Select the per-run cluster by its exact `radtest-canvas-<uid>` name.** Never "the first result", never "the only result" — both are true on an empty subscription and false on the shared one, which is the failure mode that passes locally and fails at 04:00.
...
| Visibility                              | Private                               | The fixture is mutable test infrastructure rather than a sample application. The dedicated machine user is granted access only where the test needs it.                                                                                                                                                                                                                                                                                                                        |
...
##### GitHub machine-user access

Cloud E2E uses one dedicated GitHub machine user as the developer identity for every operation against the fixture repository and GHCR. The account has administrator access to the private fixture repository because the journey reads repository OIDC settings, manages environments and their secrets and variables, publishes workflow files, dispatches workflows, inspects deployments, and resets the default branch during cleanup. The account is not used for unrelated repository activity.

The account uses one classic personal access token:

| Scope             | Used for                                                                                                                                                          |
|-------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `repo`            | Accessing the private fixture; managing contents, refs, pull requests, Actions runs, deployments, environments, secrets, variables, and repository OIDC settings. |
| `workflow`        | Committing the generated deployment workflows under `.github/workflows/`.                                                                                         |
| `read:packages`   | Inspecting the GHCR state package that the extension bootstraps.                                                                                                  |
| `write:packages`  | Pushing the bootstrap manifest before the fixture deployment workflow runs.                                                                                       |
| `delete:packages` | Removing the disposable state package during test cleanup. Normal extension setup does not require this scope.                                                    |

The workflow maps this token to both `GH_TOKEN` and `GH_PACKAGES_TOKEN`, and maps the account login to `GH_PACKAGES_USER`. Keeping one credential matches the product's normal GitHub CLI model: one selected user performs repository operations and receives package scopes. The test keeps an explicit package command environment only to prevent another inherited token from changing which credential is exercised.
...
2. **A purpose-built Azure resource role at subscription scope.** Subscription scope is unavoidable for `Microsoft.Resources/subscriptions/resourceGroups/write` and `delete`: the per-run resource group does not exist when the runner creates it. The role is not `Contributor`; it contains only resource-group lifecycle, the AKS control-plane operations used by `az aks create`, `list`, `get-credentials`, and deletion through the resource group, and the read operations used by assertions and cleanup. Its assignment has no Azure RBAC write actions.
3. **Constrained RBAC delegation at subscription scope.** The product creates assignments inside a newly created resource group and on its AKS cluster, so a permission inherited from above the resource group is required. Use `Role Based Access Control Administrator` with an assignment condition that permits only `Contributor`, `Azure Kubernetes Service RBAC Cluster Admin`, and `Locks Contributor` assignments to service principals. Apply the same role-definition restriction to assignment deletion; cleanup may remove those three test-owned roles but reports any unexpected role for manual investigation instead of receiving arbitrary assignment-delete power. Do not grant `Owner`, `User Access Administrator`, or unconstrained RBAC administration. The resource role and RBAC delegation are separate so the condition and their different blast radii remain visible.
4. **`ai-extensions` added to the existing additional-secrets map**, which another repository already uses.
5. **An additional-variables map** — the only genuinely new capability in this list. Secrets already fan out to extra repositories, but variables are hard-bound to a single repository, so there is currently no way to publish `AIEXT_CLOUD_E2E_AZURE_LOCATION` and `AIEXT_CLOUD_E2E_FIXTURE_REPOSITORY` to `ai-extensions` at all.
...
- **Automated publication still requires the Terraform workflow's GitHub App to reach `ai-extensions`.** The current environment was configured manually, so `RADIUS_GHOPERATIONS_BOT` is not part of the Cloud E2E runtime. If secret and variable publication moves back into Terraform, the publishing App must be installed on `ai-extensions` with only its existing secrets and variables permissions.
...
| Token permission missing or direct commit rejected | A missing classic PAT `workflow` scope hard-fails workflow publication. Other protected-branch-style 403 or 409 responses may activate the pull-request fallback; the journey treats that as failure because the required workflows are absent from `main`, and cleanup closes only matching `radius/setup-*` PRs and deletes their branches.                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
This tier deliberately breaks a rule stated for every change in [the test plan](./2026-08-radius-canvas-test-plan.md): _"Keep tests local and repeatable. Do not use personal credentials, live cloud resources, mutable repositories, or public network assets."_ That rule is right for the eleven offline layers and is not weakened there. It is scoped to those layers with an explicit carve-out here, so the exception is recorded rather than implied.
...
| Long-lived cloud secrets in CI                      | Azure uses OIDC, so no Azure client secret is stored. GitHub requires one classic PAT because the extension performs both repository and GHCR operations outside the fixture workflow. The PAT belongs to a dedicated machine user, is stored as one Actions secret, and is never exposed to pull-request triggers.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Over-broad GitHub privilege                         | The machine user is dedicated to Cloud E2E and has administrator access only where the fixture lifecycle requires it. Its classic PAT is limited to `repo`, `workflow`, `read:packages`, `write:packages`, and the test-only `delete:packages`; the workflows verify the token's login before using it.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
- **Use a fixture GitHub App plus a separate package PAT.** Rejected: GitHub App installation tokens cannot perform the GHCR bootstrap and cleanup, so the split would exercise two identities where a developer uses one selected GitHub account.
