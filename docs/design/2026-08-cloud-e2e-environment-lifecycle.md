...
**The per-run resource group does not isolate cluster discovery, so the journey must select by name.** This is easy to get wrong, because every *other* resource in this design is isolated by the resource group. Cluster listing is not. [`discoverResources()`](../../packages/adapter-canvas/src/server/services/discovery.ts) builds `az aks list` with a `--query` projection and an optional `--subscription`, and **no `--resource-group`**, so it returns every cluster in the shared subscription — including the Radius long-running cluster and anything another team is running. It also projects `id` from the cluster *name* rather than the ARM resource id, so clusters are identified by name alone.
