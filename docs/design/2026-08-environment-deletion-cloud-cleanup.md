...
4. **GHCR state package.** Derive the dedicated package with `stateRegistryForEnvironment` and validate through the GitHub Packages API that it is private or internal. GitHub does not consistently populate the API's canonical repository field for packages first published with a classic PAT, so a missing link is not ownership proof or failure by itself. Instead, read the retained `bootstrap` tag from GHCR and require the exact immutable OCI manifest Radius creates for the target repository; if GitHub does report a canonical link, it must also match. Delete the package only after those checks and confirm it is absent. The existing Delete Environment confirmation covers this environment-exclusive artifact; no second prompt is shown. Missing `read:packages` or `delete:packages` access, missing or changed bootstrap provenance, a conflicting repository link, or any ambiguous result ends as a retryable partial failure after the earlier teardown remains complete. Each attempt reloads the active GitHub CLI package credential, so a retry uses the token updated by `gh auth refresh` instead of the credential cached by the failed attempt. Confirmed absence is idempotent success.
5. **App-registration review.** Record an informational "left in place" step and notify the user; never delete it.

Operation schema version 7 records the GHCR state-package stage. When the extension loads an unfinished or retryable version 6 deletion, it inserts that stage before app-registration review and durably rewrites the record so restart recovery cannot skip package cleanup. Completed version 6 deletions remain historical records and are not reopened.
