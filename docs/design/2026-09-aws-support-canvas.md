...
*Provenance authorizes reuse; selection authorizes sharing.* A role the canvas created for this repository is reused without asking — that is what lets a second environment join the first one's role. A role it did not create is never written to because the name happens to match: setup stops and hands back the trust policy, so the role's owner can apply it by hand. Azure draws the same line, testing caller ownership where AWS reads its provenance tags, and asks rather than guessing when more than one owned application matches the name. Selection covers the other case, deliberately sharing one identity across repositories: Azure's `use an existing application…` link pins the chosen one, disables the name field, is reversible through `Use a per-repo identity instead`, and warns that `Sharing one identity across repositories means every wired repository can use its Azure permissions. Only do this for repos that belong to the same product.` AWS applies the same rule to IAM roles.
