# Radius Copilot app integration exception scenarios
...
- **Present each exception in one of two channels, each with its own voice**. In the chat, the skill answers in the first person, as a direct response to what the user asked (for example, "I can't find a repository attached to this session. Add a repository and try again"), and avoids referring to Radius by name except when pointing to a UI label the user must find (for example, the Radius plugin in the Plugins settings page). In the Radius side panel, exceptions are GUI states shown where the user is working: inline form validation, a graph node in a failed state exposing its error, or a render error. A single failure may be acknowledged in chat and detailed in the side panel.
