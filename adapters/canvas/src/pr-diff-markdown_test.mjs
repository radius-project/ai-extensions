...
import { renderPrDiffMarkdown, renderDiffMermaid } from "./pr-diff-markdown.mjs";
...
const container = (id, connTargets = []) => ({
    ...
    connections: connTargets.map((t) => ({ id: t, name: t.split("/").pop(), direction: "Outbound" })),
});
...
describe("renderPrDiffMarkdown", () => {
    it("summarizes counts and reports no changes for identical graphs", () => {
        const base = [container("/app/api", ["/app/db"]), datastore("/app/db", "redisCaches")];
        const md = renderPrDiffMarkdown(computeGraphDiff(base, base), "main", "feature");
    ...
    it("draws an added edge as a solid green link", () => {
        const base = [container("/app/api"), datastore("/app/cache", "redisCaches")];
        const head = [container("/app/api", ["/app/cache"]), datastore("/app/cache", "redisCaches")];
        const md = renderPrDiffMarkdown(computeGraphDiff(base, head), "main", "feature");
    ...
    it("draws a removed edge between two still-present nodes as a dotted red link", () => {
        ...
        const base = [container("/app/api", ["/app/cache"]), datastore("/app/cache", "redisCaches")];
        const head = [container("/app/api"), datastore("/app/cache", "redisCaches")];
        const md = renderPrDiffMarkdown(computeGraphDiff(base, head), "main", "feature");
...
describe("renderDiffMermaid", () => {
    it("assigns a unique node id to every resource even when names collide", () => {
        const diff = [
            { id: "/a/db", name: "db", type: "T", diffStatus: "removed", connections: [] },
            { id: "/b/db", name: "db", type: "T", diffStatus: "added", connections: [] },
            { id: "/c/db", name: "db", type: "T", diffStatus: "unchanged", connections: [] },
        ];
