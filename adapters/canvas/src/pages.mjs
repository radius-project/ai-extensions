...
<div id="app-params-section" style="display:none; margin-bottom:16px; padding:12px 16px; background:var(--background-color-default, #f6f8fa); border:1px solid var(--border-color-default, #d1d9e0); border-radius:8px;">
  <h3 style="margin:0 0 4px 0; font-size:13px; font-weight:600;">⚙️ Application Parameters</h3>
  <p style="font-size:11px; margin:0 0 10px 0; color:var(--text-color-muted, #656d76);">Optional overrides for parameters declared in your <code>app.bicep</code>. Leave a field blank to use its Bicep default, or — for parameters with no default — let Radius auto-generate a value.</p>
  <div id="app-params-list" style="display:grid; grid-template-columns:1fr 1fr; gap:12px;"></div>
</div>
...
// Dynamic application parameters: fetch the params declared in the target
// repo's app.bicep and render an optional input for each. Values entered here
// override Bicep defaults; blanks fall back to the default (or are
// auto-generated by Radius when the param has no default). Re-fetched whenever
// the repo or branch selection changes.
var appParamsSection = document.getElementById('app-params-section');
var appParamsList = document.getElementById('app-params-list');
var appParamsToken = 0;

function paramFieldId(name) { return 'app-param-' + name; }

function renderAppParams(params) {
    appParamsList.innerHTML = '';
    if (!params || params.length === 0) { appParamsSection.style.display = 'none'; return; }
    for (var i = 0; i < params.length; i++) {
        var p = params[i];
        var wrap = document.createElement('div');
        wrap.style.cssText = 'display:flex; flex-direction:column; gap:4px;';
        var label = document.createElement('label');
        label.style.cssText = 'font-size:12px; font-weight:600; color:var(--text-color-muted, #656d76);';
        var hint = p.secure ? ' (secure)' : (p.hasDefault ? ' (optional)' : ' (auto-generated if blank)');
        label.textContent = p.name;
        var hintSpan = document.createElement('span');
        hintSpan.style.cssText = 'font-weight:400;';
        hintSpan.textContent = hint;
        label.appendChild(hintSpan);
        var input = document.createElement('input');
        input.id = paramFieldId(p.name);
        input.setAttribute('data-param-name', p.name);
        input.type = p.secure ? 'password' : 'text';
        input.autocomplete = 'off';
        input.placeholder = p.hasDefault ? ('default: ' + String(p.default).replace(/^'|'$/g, '')) : 'auto-generated if blank';
        input.style.cssText = 'padding:6px 10px; border:1px solid var(--border-color-default, #d1d9e0); border-radius:6px; font-size:13px; background:var(--background-color-default, #fff);';
        wrap.appendChild(label);
        if (p.description) {
            var desc = document.createElement('span');
            desc.style.cssText = 'font-size:11px; color:var(--text-color-muted, #656d76);';
            desc.textContent = p.description;
            wrap.appendChild(desc);
        }
        wrap.appendChild(input);
        appParamsList.appendChild(wrap);
    }
    appParamsSection.style.display = 'block';
}

function loadAppParams() {
    var repoSel = document.getElementById('deploy-repo-select');
    var branchSel = document.getElementById('deploy-branch-select');
    var repo = (repoSel && repoSel.value) || document.getElementById('target-repo').value || '';
    var branch = (branchSel && branchSel.value) || '';
    if (!repo || repo === '__custom__') { appParamsSection.style.display = 'none'; return; }
    var myToken = ++appParamsToken;
    fetch('/api/app-params', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ repo: repo, branch: branch })
    }).then(function(r) { return r.json(); }).then(function(result) {
        if (myToken !== appParamsToken) return; // a newer request superseded this one
        renderAppParams(result && result.params);
    }).catch(function() {
        if (myToken !== appParamsToken) return;
        appParamsSection.style.display = 'none';
    });
}

function collectDeployParams() {
    var out = {};
    var inputs = appParamsList.querySelectorAll('input[data-param-name]');
    for (var i = 0; i < inputs.length; i++) {
        var v = inputs[i].value.trim();
        if (v !== '') out[inputs[i].getAttribute('data-param-name')] = v;
    }
    return out;
}

(function wireAppParams() {
    var repoSel = document.getElementById('deploy-repo-select');
    var branchSel = document.getElementById('deploy-branch-select');
    if (!repoSel || !branchSel) { setTimeout(wireAppParams, 150); return; }
    repoSel.addEventListener('change', loadAppParams);
    branchSel.addEventListener('change', loadAppParams);
    setTimeout(loadAppParams, 400);
})();
...
document.getElementById('deploy-btn').addEventListener('click', function() {
    ...
    envData.deployParams = collectDeployParams();
    if (provider === 'azure') {
