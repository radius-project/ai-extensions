            ...
            handler: async (args) => {
                ...
                try {
                    ...
                    if (!baseContent && !headContent) {
                        return `.radius/app.bicep does not exist on ${baseBranch} or ${headBranch} yet. Author it with the Radius app-bicep skill (run the radius_generate_app tool) and follow the skill through committing and pushing it to the branch (the diff reads the pushed branch), then re-run this tool.`;
                    }
...
1. First, check if .radius/app.bicep (or app.bicep) exists on the branch the graph reads (committed and pushed to the remote).
2. If it does not, author it using the radius_generate_app tool (the radius-app-bicep skill owns namespaces, types, structure, and committing + pushing the file) and follow that skill to completion.
3. Only AFTER the skill has committed and pushed app.bicep to that branch, open: open_canvas({ canvasId: "radius", instanceId: "radius-panel", input: { page: "graph", repo: "<current-repo>" } }). The graph renders from the pushed branch, so a local save is not enough.

The same rule applies to the "planned" and "graph-diff" pages: they render from the pushed .radius/app.bicep, so if it is not on the branch, first author it with the radius_generate_app tool (radius-app-bicep skill) and let the skill commit and push it before opening those pages.
