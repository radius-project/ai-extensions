...
(function setupDevSelfReload() {
    try {
        ...
        const maybeReload = () => {
            if (triggered || shuttingDown) return;
            let deploying = false;
            try { deploying = deployInFlight(); } catch {}
            const panelActive = (Date.now() - getLastWebviewActivityAt()) < RELOAD_IDLE_WINDOW_MS;
            const waited = Date.now() - reloadRequestedAt;
            if (deploying || (panelActive && waited < RELOAD_MAX_DEFER_MS)) {
                try { console.error(`[radius][dev] reload deferred (${deploying ? "deploy in flight" : "canvas panel active"}); retrying in ${RELOAD_DEFER_POLL_MS}ms…`); } catch {}
