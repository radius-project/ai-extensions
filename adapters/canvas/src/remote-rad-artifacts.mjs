 ...
 */
export async function stageRemoteRadArtifacts(github, repo, branch, bicepRepoPath, { log = () => {} } = {}) {
    if (!github?.getContentBytes || !repo || !branch) return null;
    const normalized = String(bicepRepoPath || ".radius/app.bicep").replace(/\\/g, "/");
    const bicepDir = path.posix.dirname(normalized);
    const radiusDir = bicepDir && bicepDir !== "." ? bicepDir : "";
    const configRepoPath = radiusDir ? `${radiusDir}/bicepconfig.json` : "bicepconfig.json";

    let configText;
    try {
        configText = await github.getContent(`/repos/${repo}/contents/${configRepoPath}?ref=${branch}`);
    } catch (err) {
        log(`Warning: could not fetch ${configRepoPath} on ${branch}: ${String(err?.message ?? err)}`);
        return null;
    }
    ...
    try {
        ...
        log(`Warning: ${configRepoPath} on ${branch} is not valid JSON; ignoring: ${String(err?.message ?? err)}`);
        return null;
    }
    ...
    try {
        ...
        for (const [alias, ref] of Object.entries(extensions)) {
            ...
            const rel = safeLocalRef(ref);
            if (!rel) {
                log(`Warning: skipping non-local extension artifact reference for ${alias}: ${ref}`);
                continue;
            }
            const artifactRepoPath = radiusDir ? `${radiusDir}/${rel}` : rel;
            let bytes;
            try {
                bytes = await github.getContentBytes(`/repos/${repo}/contents/${artifactRepoPath}?ref=${branch}`);
            } catch (err) {
                log(`Warning: could not fetch extension artifact ${artifactRepoPath} on ${branch}: ${String(err?.message ?? err)}`);
                continue;
            }
            if (bytes && bytes.tooLarge) {
                log(`Warning: extension artifact ${artifactRepoPath} on ${branch} exceeds the GitHub contents API inline limit; the ${alias} extension will not resolve for this branch.`);
                continue;
            }
            if (!bytes) {
                log(`Warning: extension artifact ${artifactRepoPath} not found on ${branch}; the ${alias} extension will not resolve for this branch.`);
                continue;
            }
        ...
        log(`Warning: could not stage remote .radius artifacts for ${repo}@${branch}: ${String(err?.message ?? err)}`);
        return null;
    }
}
 ...
 */
export async function radArtifactsDirForSelection({ isLocal, state, github, repo, branch, bicepRepoPath, log }) {
    if (isLocal) {
        return { dir: workspaceRadArtifactsDir(state, bicepRepoPath), remote: false };
    }
    const dir = await stageRemoteRadArtifacts(github, repo, branch, bicepRepoPath, { log });
