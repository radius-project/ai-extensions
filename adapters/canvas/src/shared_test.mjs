...
describe("credential profiles", () => {
    beforeEach(() => {
        for (const p of listCredentialProfiles(REPO)) deleteCredentialProfile(REPO, p.name);
    });
    ...
    it("saves an azure profile with normalized fields and 'verified' status", () => {
        const saved = saveCredentialProfile(REPO, {
            name: "azure-staging", provider: "azure", user: "u@d.com",
            tenantId: "t1", subscriptionId: "s1",
        });
        expect(saved).toMatchObject({ name: "azure-staging", provider: "azure", status: "verified", user: "u@d.com" });
    ...
    it("defaults an unknown provider to azure and keeps aws when given", () => {
        expect(saveCredentialProfile(REPO, { name: "a", provider: "gcp" }).provider).toBe("azure");
        expect(saveCredentialProfile(REPO, { name: "b", provider: "aws", accountId: "123" }).provider).toBe("aws");
    });
