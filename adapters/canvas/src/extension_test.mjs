...
describe("default canvas page", () => {
    ...
    it("resolves both the canvas open handler and the HTTP page router from the shared default", () => {
        expect(source).toContain("const page = ctx.input?.page || DEFAULT_CANVAS_PAGE");
        expect(serverSource).toContain("requestedPage || entry?.page || DEFAULT_CANVAS_PAGE");
    });

    it("sources the default from hooks.mjs so the page vocabulary has a single owner", () => {
        expect(source).toMatch(/import \{[^}]*DEFAULT_CANVAS_PAGE[^}]*\} from "\.\/hooks\.mjs"/);
        expect(serverSource).toMatch(/import \{[^}]*DEFAULT_CANVAS_PAGE[^}]*\} from "\.\/hooks\.mjs"/);
    });
});
