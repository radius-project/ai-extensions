...
export function selectDeployEntry(servers, attemptId) {
    if (attemptId) {
        for (const entry of servers.values()) {
            if (entry?.baseUrl && entry.state?.deployAttempt?.id === attemptId) return entry;
        }
        return null;
    }
    let found = null;
...
export function validateDeployAttempt(args = {}, state = {}) {
    ...
    const mismatched = ATTEMPT_FIELDS
        .filter(([arg, snap]) => args[arg] && args[arg] !== snapshot[snap])
        .map(([arg, snap]) => `${arg} (asked for "${args[arg]}", attempt used "${snapshot[snap]}")`);
...
export function buildDeployPayload(args = {}, state = {}) {
    ...
    return {
        environment: args.environment || snapshot.environment || last.environment || "",
        provider: args.provider || snapshot.provider || last.provider || "azure",
        targetRepo: args.repo || snapshot.targetRepo || last.targetRepo || state.contextRepo || "",
        branch: args.branch || snapshot.branch || last.branch || "",
        appFile: args.appFile || snapshot.appFile || last.appFile || ".radius/app.bicep",
        agentInitiated: true,
    };
}
...
export function summarizeDeployStatus(d = {}, logLines) {
    ...
    if (tail.length) sections.push(`last ${tail.length} log line(s):\n${tail.join("\n")}`);
    ...
    return {
        ...
        ...(diagnostic ? { diagnosticNote: DEPLOY_DIAGNOSTIC_NOTE, diagnostic } : {}),
    };
}
