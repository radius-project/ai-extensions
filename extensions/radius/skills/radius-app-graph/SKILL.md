...
4. The planned graph, auto-open graph diff, and `radius_generate_pr_diff_markdown` prefer the committed `app-graph.json` beside the selected branch's model and fall back to building it from `app.bicep` when the artifact is absent. PR diff mode compares base and head branch graphs and tags resources `added | removed | modified | unchanged`.
