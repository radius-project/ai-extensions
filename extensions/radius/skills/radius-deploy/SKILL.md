...
8. Deploys an environment-only `Radius.Core/environments` resource from the app file's directory (e.g. `.radius/`), then deploys a pack-only provider recipe pack with that environment, resolves the exact pack resource ID, and explicitly attaches it with `rad env update --recipe-packs --preview`. **Azure** downloads the `azure-avm` pack from `radius-project/resource-types-contrib`; **AWS** generates an inline `aws-terraform` pack bundling the Kubernetes recipes (`containers`, `containerImages`, `persistentVolumes`, `routes`, `postgreSqlDatabases`, `secrets`) plus a provider-gated `mySqlDatabases` recipe (AWS RDS). Both Bicep files live beside the app so the repo's own `bicepconfig.json` resolves the `radius` extension.
