...
7. Authenticates to GHCR with the repository `GITHUB_TOKEN`, exports environment-scoped `RADIUS_STATE_BACKEND`, `RADIUS_STATE_REGISTRY`, and `RADIUS_STATE_ARCHIVE`, creates the CLI workspace, then runs `rad startup` to restore the control-plane databases and Terraform recipe-state Secrets saved by the previous run (a no-op on the first run). It then selects the Radius resource group to deploy into from what the restore produced: a never-deployed environment gets a group derived from the repository and environment name, so its cloud resources cannot collide with another environment's, while an environment whose resources are already in `default` keeps that group. The `Radius.Compute/containerImages` type ships with the published `radius` Bicep extension, so no resource-type registration or local Bicep-extension build happens at deploy time.
