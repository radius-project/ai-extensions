...
7. Authenticates to GHCR with the repository `GITHUB_TOKEN`, exports environment-scoped `RADIUS_STATE_BACKEND`, `RADIUS_STATE_REGISTRY`, and `RADIUS_STATE_ARCHIVE`, creates the CLI workspace, then runs `rad startup` to restore the control-plane databases and Terraform recipe-state Secrets saved by the previous run (a no-op on the first run). It then deploys into a Radius resource group keyed to the repository and environment, so two environments backed by the same cloud scope cannot resolve to the same cloud resources. The `Radius.Compute/containerImages` type ships with the published `radius` Bicep extension, so no resource-type registration or local Bicep-extension build happens at deploy time.
