...
Keep two authoring modes separate. An explicit user modeling request or a legacy `radius_generate_app` bootstrap uses the [legacy staged workflow](#legacy-staged-workflow). A canonical `definition.author` assignment uses the guarded coordinator workflow below. Never switch an accepted canonical operation to legacy after a failure, missing approval, cancellation, or unavailable capability.

For canonical authoring, start only from a trusted operation/action/staging handoff. It must bind the operation, outstanding agent action, approved workspace source and fingerprint, guarded `stagingRef`, and host-resolved staging location. A prompt, GitHub identity, legacy bootstrap, `session.send`, or public completion field is not that authority. Never supply `approved: true`, claim human approval, invent an assignment, or use a public error report as an authenticated agent outcome.

The current SDK supports the retained legacy bootstrap, but does not supply trusted source-bound approval and authenticated assignment/outcome delivery for canonical authoring. Canonical requests therefore return `CAPABILITY_UNAVAILABLE` before authoring. Supported injected hosts can provide these capabilities; their tests do not qualify the current SDK. A canonical source contract must represent a caller-selected subdirectory; otherwise report it as unavailable rather than widening the scope.

During a canonical authoring or repair operation, write only to its assigned staging location and return outcomes through authenticated `operation.respond`, using the exact operation/action identity and staged output manifest. Never invoke standalone begin, compile, seal, promotion, or abort commands for that operation, including read-only checker modes intended for the coordinator. Never manufacture `validatedOutputs` or alter approval, source, action, repair-budget, or validation records. The coordinator validates the actual staged bytes once per accepted proposal, rechecks authority and source freshness, and alone decides whether to promote. An agent's completed outcome is a proposal, not proof of validation or promotion. A failed proposal needs a separately authorized linked repair or an already authorized finite automatic policy; a new staging directory does not reset the shared five-cycle ceiling.
...
Then stop, and write nothing: no `.radius/app.bicep`, no `.radius/bicepconfig.json`, no origin record, no branch, no commit. Do not guess a directory on the user's behalf or model one candidate to "make progress" — a directory you chose yourself is not an answer to the question you just asked. Preserve the user's directory selection on the next legacy request. In canonical mode, it must instead be represented in a new trusted source assignment; if the host cannot represent that scope, report `CAPABILITY_UNAVAILABLE`.

## Response

When asked to model a repository:

1. Generate only into the staging directory for the selected mode (see [Staged runs](#staged-runs)). Never write generated files directly into their final locations. Canonical authoring never stages files in git; legacy promotion stages only its managed outputs. Neither mode commits, pushes, opens a pull request, publishes a registry recipe, or deploys without separate authorization.
2. In your chat reply, give a one-line intro naming the app (e.g. "I'll create an application definition for `todo-list-app`."), then a short, natural summary of the resources you identified, a brief list such as "Container: `todo-list-app`", "MySQL database", "Secret for DB credentials". A sentence or two of reasoning is fine; don't dump raw source analysis or the full file contents. If external-client ingress was considered but omitted, state why. Describe only observed results: proposal submitted, validation incomplete or failed, cancellation, or confirmed promotion through the selected mode. Never equate queued, running, action-required, or agent-completed status with promotion. Do not claim the application graph or canvas is rendering, since you cannot observe that. If a graph view shows an error or empty state, report that honestly. Keep the reply about the user's application and its resources; do not name internal skill or reference files.

## Radius CLI execution boundary

Never invoke `rad` or `rad.exe` directly from PowerShell, a shell, a subprocess, or a delegated agent. Use `node "<loaded-skill-base>/scripts/show-radius-type.mjs"` to resolve predefined Radius type definitions. Compile the staged application definition with `node "<loaded-skill-base>/scripts/validate-bicep.mjs" <staging-dir>/app.bicep`; the checker uses only the extension-managed Bicep and fails on every compiler warning or error. Canonical `definition.validate` is read-only and requires no agent; it validates supported captured sources and cannot grant authoring, promotion, or deployment authority. Graph reads stay read-only and are not authoring completion signals. The extension honors an existing executable `RADIUS_RAD_BINARY` as a developer-owned override; otherwise it runs its managed binary from `%USERPROFILE%\.radius\ai-extensions\bin\rad.exe` on Windows or `$HOME/.radius/ai-extensions/bin/rad` on macOS/Linux, downloading it when absent and attempting a best-effort upgrade when older than the latest release (offline/API failures keep the installed binary; set `RADIUS_RAD_SKIP_VERSION_CHECK` to skip the version check). It does not resolve `rad` from `PATH` or `.rad/bin`. Diagnose graph failures only from the Radius extension log; never reproduce them with a direct CLI command or through another agent.

If `show-radius-type.mjs` fails while locating, querying, or validating the selected Radius CLI or while deriving its extension reference, stop and report the actionable error using the selected mode's failure path below. This includes a missing binary, invalid or incomplete version JSON, a noncanonical commit, and an unsupported development, edge, or pull-request version. A CLI-resolution failure is not permission to repair the toolchain: never download, install, upgrade, downgrade, copy, move, rename, back up, delete, or replace a `rad` binary; never change or unset `RADIUS_RAD_BINARY` or `RADIUS_RAD_SKIP_VERSION_CHECK`; and never search `PATH`, `.rad/bin`, or another location for a released fallback. The extension alone owns its managed binary lifecycle. Use a different binary only after the user explicitly changes their configuration and requests a new modeling run.

## Workflow

1. Establish the selected mode and staging directory (see [Staged runs](#staged-runs)). Canonical mode requires the trusted assignment; legacy mode starts its own run with `--begin`. Then select one runnable deployment profile. Treat explicit user, scenario, and target-repository deployment requirements for Radius types, resource-name parameters, workload roles/count, native configuration keys, secret bindings, provider profile, protocol values, and connection names as acceptance criteria. Verify that the pinned source supports that profile; do not silently replace it with an easier default or optional backend.
...
11. In canonical mode, submit the staged manifest through authenticated `operation.respond`; never invoke standalone promotion. In legacy mode, seal the completed proposal and invoke the promotion command below. Report only the observed outcome.
...
For legacy staged runs, the checker counts its own compiles in `run.json`. It records the attempt before compiling, so an interrupted compile still counts, and refuses compilation if that record cannot be read or updated. The count covers that separately authorized legacy modeling run. The limit is six compiles: the first compile plus five repairs. Do not switch to read-only verification to evade that budget.

Canonical staging is marked coordinator-owned, and the standalone agent checker refuses it without spending or resetting a local compile counter. Follow the canonical exception above instead of the compile-and-repair steps below: submit one staged proposal and let the coordinator validate it. The initial authoring proposal plus at most five linked repair proposals allow at most six coordinator authoring compilations; a stricter policy reduces that limit. All linked repairs inherit the consumed family budget, including repeated requests against the original failure. Manual repair is the default. An explicitly authorized automatic policy advances only after an authenticated failed response and current authority checks, never from status polling or graph reads.
...
- **If the checker refuses to compile, stop.** Whether the budget is spent or the run's bookkeeping is broken, do not edit and retry. Do not write the origin record and do not publish the run. Report the resource, property, and last compiler diagnostic through the selected mode's failure path, redacting secrets and private host details. State that no definition was promoted only after observing that outcome.
...
When a deploy fails because of a modeling or schema error in an existing `.radius/app.bicep` (unknown type or API version, unknown or missing property, invalid reference between resources, wrong credential shape, or a Bicep parse or compile error), repair that model rather than regenerating it from scratch. A repair starts a new run in the selected authoring mode and edits staged copies only (see [Staged runs](#staged-runs)). Canonical repair requires a new trusted assignment and must not fall back to legacy. Editing the final file in place would leave a half-repaired model behind if the repair failed partway. This assumes the deploy error and relevant logs have been provided; otherwise ask for them first.
...
5. Write a new [origin record](#origin-record-apporiginjson) after the checker passes, then finish through the selected mode's promotion path and explain the fix. Redeployment requires separate authorization after observed promotion. If the same error recurs, try a different fix within the repair budget rather than reapplying the one that failed. If no different fix can be found, report a redacted failure and stop the run.

## Staged runs

A modeling run that stops partway must preserve existing repository content. The following coordinator rules apply to canonical mode. Legacy mode uses the separate commands below.

The coordinator owns staging lifetime, captured effective-input manifest, approved source fingerprint, and original output baselines. Use only the location from the trusted assignment; do not discover, create, adopt, remove, or resume a canonical staging directory yourself.

Write every generated file into that directory and nowhere else: `app.bicep`, `bicepconfig.json`, the origin record, and any custom-type artifacts (`custom-types.yaml`, `custom-types.tgz`, `custom-recipe-pack.bicep`, and `<type>-recipe.bicep`). Pass the assigned directory to the local `radius_publish_custom_type_extension` tool as `stagingDir`; local extension compilation is not a registry push. Run the Bicep checker against staged `app.bicep`, then submit an authenticated completed outcome with the assigned `stagingRef` and output manifest. Do not submit validation authority: the coordinator inspects actual inputs and outputs, validates their exact bytes, and guards promotion against source drift, changed effective dependencies, stale action ownership, cancellation, and concurrent user edits.

The standalone script is not a canonical authorization path. Never create its validation record or invoke it to bypass the coordinator. Lifecycle promotion does not stage files in git.

Rules:

- Never write, copy, or move generated files into final `.radius/` locations yourself.
- Never re-run modeling "to finish the job" after refusal without a new trusted assignment. A retry uses new coordinator-owned staging and authority.
- Report every generated artifact in the output manifest. Unexpected, missing, or changed outputs must fail validation rather than being silently omitted.
- If promotion refuses because the source or a file changed during the run, preserve that version. Report the coordinator's actual result; do not merge, restore, overwrite, or force promotion.

### Legacy staged workflow

Use this sequence only for a separately requested legacy modeling run, never as a fallback for an accepted canonical operation. Run commands from the selected repository root. Capture the directory printed by `--begin` and use that exact directory throughout:

```text
node "<loaded-skill-base>/scripts/promote-app-model.mjs" --begin
```

Author all outputs in that directory. For a repair, copy the existing model and required artifacts there before editing. Resolve types and perform the workflow and checklist above. Complete the run in this order:
...
node "<loaded-skill-base>/scripts/promote-app-model.mjs" --seal --staging <staging-dir>
node "<loaded-skill-base>/scripts/promote-app-model.mjs" --staging <staging-dir>
```

The seal command performs independent compiler/static verification against captured local inputs and binds the complete output set, including the origin record. Promotion verifies again and rejects changed inputs, outputs, or run records. The existing begin, validate, origin, promote sequence also remains supported: promotion creates the first seal through real verification when no seal has been created yet. Once sealed, a missing record is an error, not permission to silently replace it. Neither verification reserves nor resets an agent repair attempt. Do not edit a seal, manufacture hashes, change bookkeeping, or use independent verification to continue repairing after the six-compile limit.

An origin hash alone is insufficient: promotion must complete the actual compiler/static verification and source/output checks.

Legacy verification retains the shipped compiler warnings and static build-reference, source-reference, runtime-variable, and secret-target rules. It is not a passed canonical validation report and does not establish runtime/Recipe evidence or authenticated approval. Legacy Bicep registry resolution and cache behavior remain available; this path does not claim canonical external-dependency isolation or an OS sandbox. Canonical validation continues to report required missing evidence as incomplete.

On success, the script publishes and stages its managed files in git; it does not commit or push. On failure or cancellation, report the actual error and discard only this run with `node "<loaded-skill-base>/scripts/promote-app-model.mjs" --abort --staging <staging-dir>`. If recovery files are retained, stop and report them rather than deleting them or claiming rollback succeeded.

### When a step fails

For canonical failures, send a failed outcome through authenticated `operation.respond` with redacted, actionable diagnostics; the coordinator releases its own run. For legacy failures, follow the abort instructions above. Cancellation must not be disguised as success or a permanent modeling error. Never report cleanup or rollback as successful before observing it. Distinguish "proposal written in staging" from "model promoted".
...
- **Looks permanent** — no Dockerfile, a required backing service with no Radius type that cannot be provisioned on Azure, no source that resolves to a runnable profile. Report it and do not offer an unchanged retry. The retained `radius_report_modeling_failure` tool records only a legacy Canvas attempt diagnostic, using its exact instance, repository, branch, and attempt token; it cannot complete a lifecycle action, authenticate an outcome, approve changes, or promote outputs. Use it only if that legacy handoff explicitly supplied those arguments and the failure is permanent, never for a transient failure, cancellation, user-declined action, or a run that wrote `app.bicep`. Lifecycle failures still require authenticated `operation.respond`.
...
- Write the record on **every** generation and every repair, as the last staged-authoring step, and only after `validate-bicep.mjs` exits successfully with no warnings. The record identifies the exact model bytes and source; it does not replace promotion verification or grant canonical approval.
...
- Write it into the selected run's staging directory beside the `app.bicep` it describes and include both in its output manifest (see [Staged runs](#staged-runs)). Both promotion paths reject missing or mismatched provenance rather than trusting an agent's assertion.
...
Graph reads do not request or authorize a refresh. If a model is stale because its source or generator changed, or its origin record is missing, report that state. Regeneration requires a separate explicit request through the selected authoring mode:
...
- **A missing origin record is not approval to regenerate.** A separately requested legacy refresh starts a new staged run and records its baseline; canonical refresh requires a new trusted assignment.
- Source or generator drift also requires a new run against the current source. In canonical mode, an earlier run's approval cannot authorize changed source; obtain a new approved assignment.
...
| Data store secret                 | `<engine>Secret` when the type's schema defines a secret-reference credential property; app secrets use `appSecrets`                                                             |
...
| Field                              | Value                                                                                                                                                                                                                                                                                                                                                                                      |
|------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Data store admin username          | The administrator username you author for the provisioned database. It is not derived from the source. Set it wherever the schema puts credentials — `username` on the resource, or `USERNAME` in the authored Secret when the schema takes a secret reference instead. Use `myadmin` when the source does not require a login; it fits the Azure PostgreSQL, MySQL, and SQL safe subsets. |
...
5. Secret resources (app secrets, or credentials the schema takes as a secret-resource reference)
...
- Building from a complete Dockerfile context: add a `Radius.Compute/containerImages` resource with `build.source` set to the repo git URL (`git::https://github.com/<org>/<repo>.git//<subdir>?ref=<sha-or-tag>`); the container references the built image via `<serviceName>Image.properties.imageReference` (no separate connection needed). When the push registry is authenticated (e.g. `ghcr.io`), also add the registry-push Secret whose `name` property is exactly `radius-ghcr-registry-creds` (`username`/`password` from `@secure() registryUsername` + `@secure() registryPassword` params — both are `@secure()` because `data.<key>.value` is sensitive) and `dependsOn` it — its `name` value must match the recipe pack's `containerImagesRegistrySecretName`. Immediately above that Secret, emit the exact two-line warning comment shown in [containerImages structure](references/bicep-structure-rules.md#radiuscomputecontainerimages-structure); do not vary its wording. Omit the Secret and warning for an unauthenticated registry.
- Credential inputs follow the type's schema, classified by sensitivity and never by property name: a property marked `x-radius-sensitive: true` takes the `@secure() param` value directly (`Radius.Data/mySqlDatabases.password`); a plain, non-sensitive `string` property whose schema description identifies it as the resource ID of a `Radius.Security/secrets` resource takes `<secret>.id` from an authored or reused Secret and never a `@secure() param` (`Radius.Messaging/rabbitMQ.password`); if the schema defines neither, the type takes no credential input and you must not invent one. A property named `password` may be either kind, and a reference property may be named `password`, `passwordSecret`, or `secretName` — the name carries no information, so read the schema. Assigning a raw credential to a reference property makes the password the Kubernetes Secret name the Recipe looks up and fails the deployment. Whenever you supply a credential value, it must come from a `@secure() param` and never a literal, whether it is assigned inline to a sensitive property or placed in an authored Secret's `data`. That governs where a supplied value comes from; it does not require the type to take a credential input at all.
- That classification reads the envelope's own properties, but sensitivity belongs to the schema **node**, so read it recursively. A marked **writable** node takes a secure value at whatever depth it sits: a leaf inside an open map (`Radius.Security/secrets.data.<key>.value`, whose enclosing `data` is not itself marked — this leaf is what "placed in an authored Secret's `data`" above refers to), a leaf inside a nested object, or a whole object that compiles to a `secureObject` and takes one `@secure() param object`. A node marked sensitive **and** `readOnly: true` is a Recipe-populated output: never set it and never invent a parameter for it; read it through the Recipe's `result.secrets` contract. Generated `Radius.Resources/*` types carry both flags too, because `x-radius-sensitive` in `custom-types.yaml` is compiled into `custom-types.tgz`. Only a `@secure() param` referenced by name — directly, or through a `var` that aliases it — is a secure value; a literal, a parameter declared without `@secure()`, and any string interpolation (even of secure operands) are not, so a credential-bearing string such as a connection URL is never assembled in Bicep: bind the parts separately and compose them only through a path the pinned application source proves it supports, reporting the contract gap when it supports none. Give a `@secure() param` no default value. Bicep enforces this as `use-secure-value-for-secure-inputs`, which Radius emits with no severity: it prints as a `warning` and still fails the build. See [secrets-handling.md](references/secrets-handling.md#sensitivity-marks-a-schema-node-not-a-top-level-property).
...
In canonical mode, the coordinator applies the required validation policy to the actual captured source and staged proposal. Every compiler, type, source, secret, runtime, reference, and recipe warning is required: fix it or report failure. Missing or skipped required evidence makes validation incomplete, not passed. Policy-defined advisory classifications are fixed before execution; never downgrade a failed required check to advisory after seeing its result. Compilation alone, an origin hash, an agent-supplied validation report, and a graph render do not prove the complete runtime contract or authorize promotion.
...
- [ ] When a `Radius.Compute/containerImages` resource pushes to an authenticated registry, it is paired with a `Radius.Security/secrets` whose `name` property is exactly `radius-ghcr-registry-creds` (keys `username`/`password` from `@secure() registryUsername` + `@secure() registryPassword` params) and `dependsOn`s it; no registry is set on the resource. The Secret's `name` value matches the recipe pack's `containerImagesRegistrySecretName`, and the exact two-line warning from [containerImages structure](references/bicep-structure-rules.md#radiuscomputecontainerimages-structure) appears immediately above the Secret. For an unauthenticated registry, no such Secret or warning is authored.
...
- [ ] Every credential input was classified from the schema rather than from the property's name: each `x-radius-sensitive: true` property is assigned a `@secure() param` directly, and each plain, non-sensitive `string` property documented as the resource ID of a `Radius.Security/secrets` resource is assigned `<secret>.id` — no `@secure() param` is assigned to a reference property, whatever it is named, and no credential input is invented where the schema defines none. Each authored Secret referenced that way exposes the value under the exact case-sensitive data key the consuming schema names (`password`, not `PASSWORD`), and every `secretKeyRef.key` reading it uses that same key. `database`/`topic`/`queue`/etc. derived from source.
- [ ] The resolved schema was read recursively for sensitivity, and every **writable** node marked `x-radius-sensitive: true` receives a `@secure() param` by name — including a nested leaf such as `Radius.Security/secrets.data.<key>.value`, a leaf inside a nested object, a `secureObject` property, and the same nodes on generated `Radius.Resources/*` types. A node marked sensitive **and** `readOnly: true` is left unset like any other Recipe output and has no parameter invented for it. No sensitive node is assigned a literal, a parameter declared without `@secure()`, or a string interpolation; no `@secure() param` declares a default value; and no credential-bearing string is assembled in Bicep.
...
- [ ] Apart from the required `radius-ghcr-registry-creds` warning, the generated Bicep contains no explanatory comments. The staged `bicepconfig.json` preserves unrelated settings and resolves every declared extension. The resolver owns its `radius` alias; any model-authored compiler setting or generated custom-type alias preserves that value.
