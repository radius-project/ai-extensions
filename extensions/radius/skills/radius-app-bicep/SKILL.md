...
After type resolution succeeds, `show-radius-type.mjs` reads an existing `<staging-dir>/bicepconfig.json`; if none exists, it reads the current `.radius/bicepconfig.json`; if neither exists, it starts with an empty object. It preserves unrelated settings and extension aliases, enables `experimentalFeaturesEnabled.extensibility`, and fills an absent or blank `extensions.radius` with the extension derived from the managed Radius CLI's stamped release identity. A different nonblank `radius` alias is a version conflict: the command exits `1`, leaves the configuration unchanged, and prints no contract JSON. Successful partial and all-missing results still update the staged configuration.
...
Do not upgrade or downgrade an existing compatible pinned extension merely because another release is newer. Use `radius:latest` only when the CLI reports the `edge` release identity; the resolver warns that this mutable tag may not match the configured binary.
