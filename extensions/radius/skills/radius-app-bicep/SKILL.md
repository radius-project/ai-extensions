...
- Sensitivity is a property of the schema node, so read it recursively rather than only across the properties envelope. Every node marked `x-radius-sensitive: true` takes a secure value at whatever depth it sits: a top-level string (`Radius.Data/mySqlDatabases.password`), a leaf inside an open map (`Radius.Security/secrets.data.<key>.value`, whose enclosing `data` is not itself marked), a leaf inside a nested object, or a whole object that compiles to a `secureObject` and takes one `@secure() param object`. Generated `Radius.Resources/*` types carry the flag too, because `x-radius-sensitive` in `custom-types.yaml` is compiled into `custom-types.tgz`. Only a `@secure() param` referenced by name — directly, or through a `var` that aliases it — is a secure value; a literal, a parameter declared without `@secure()`, and any string interpolation (even of secure operands) are not, so a credential-bearing string such as a connection URL is never assembled in Bicep — bind the parts separately and compose them only through a path the pinned application source proves it supports, reporting the contract gap when it supports none. Give a `@secure() param` no default value. Bicep enforces this as `use-secure-value-for-secure-inputs`, which Radius emits with no severity: it prints as a `warning` and still fails the build. See [secrets-handling.md](references/secrets-handling.md#sensitivity-marks-a-schema-node-not-a-top-level-property).
...
- [ ] The resolved schema was read recursively for sensitivity, and every node marked `x-radius-sensitive: true` receives a `@secure() param` by name — including a nested leaf such as `Radius.Security/secrets.data.<key>.value`, a leaf inside a nested object, a `secureObject` property, and the same nodes on generated `Radius.Resources/*` types. No sensitive node is assigned a literal, a parameter declared without `@secure()`, or a string interpolation; no `@secure() param` declares a default value; and no credential-bearing string is assembled in Bicep.
