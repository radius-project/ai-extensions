...
During a canonical operation, write only to its assigned staging location and return outcomes through authenticated `operation.respond`, using the exact operation/action identity and staged output manifest. Never invoke standalone begin, seal, promotion, or abort commands for that operation. Never manufacture `validatedOutputs` or alter approval, source, action, repair-budget, or validation records. The coordinator validates the actual staged bytes, rechecks authority and source freshness, and alone decides whether to promote. An agent's completed outcome is a proposal, not proof of validation or promotion.
...
The checker enforces this whenever the model it compiles is inside a staging directory, by counting its own compiles in that run's `run.json`. It records the attempt before compiling, so an interrupted compile still counts, and it refuses to compile at all if that record cannot be read or updated — a budget it cannot count is one it cannot enforce. The count covers exactly one modeling run, and a later authorized run starts fresh. You do not track attempts yourself, and you cannot compile your way past the limit. The limit is six compiles: the first compile plus five repairs. The coordinator's `--validate-json`/read-only validation does not alter run repair state, but each agent compile still counts. Do not switch to that read-only mode to evade the agent repair budget.
