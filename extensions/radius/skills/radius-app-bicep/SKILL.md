...
Never invoke `rad` or `rad.exe` directly from PowerShell, a shell, a subprocess, or a delegated agent. Use `node "<loaded-skill-base>/scripts/show-radius-type.mjs"` to resolve predefined Radius type definitions. Compile the generated application definition with `node "<loaded-skill-base>/scripts/validate-bicep.mjs" <staging-dir>/app.bicep`; the checker uses only the extension-managed Bicep and fails on every compiler warning or error. Graph validation must go through the Radius canvas and its tools: open `canvasId: "radius"` with `instanceId: "radius-panel"`, pass the current session repository as `repo` in `owner/repo` form, and use the current Copilot worktree branch. The extension honors an existing executable `RADIUS_RAD_BINARY` as a developer-owned override; otherwise it runs its managed binary from `%USERPROFILE%\.radius\ai-extensions\bin\rad.exe` on Windows or `$HOME/.radius/ai-extensions/bin/rad` on macOS/Linux, downloading it when absent and attempting a best-effort upgrade when older than the latest release (offline/API failures keep the installed binary; set `RADIUS_RAD_SKIP_VERSION_CHECK` to skip the version check). It does not resolve `rad` from `PATH` or `.rad/bin`. Diagnose graph failures only from the Radius extension log; never reproduce them with a direct CLI command or through another agent.

`validate-bicep.mjs` has this exit-code contract:

| Exit code | Meaning                                                                                                                  | Required action                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
|-----------|--------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `0`       | Validation completed with valid compiler output and no compiler warnings, compiler errors, or checker policy violations. | Continue the validation checklist. Only after it is complete, write the origin record and publish the staged run.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `1`       | Validation completed and found actionable compiler or checker policy diagnostics about the model.                        | Repair the staged model and rerun the checker within the repair budget. Only exit `1` permits a model edit in response to checker output.                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `2`       | Validation was unavailable, incomplete, or refused before a reliable verdict.                                            | Run `node "<loaded-skill-base>/scripts/promote-app-model.mjs" --abort --staging "<staging-dir>"`, report the exact checker failure, and state that no application model was written. Do not edit the model based on this result, write `app.origin.json`, publish, or claim that the model compiled. Do not download, install, replace, relocate, or search for another Bicep binary. The Radius extension owns the managed binary lifecycle. Never continue or manually restart the current staged run; after the underlying failure is corrected, only a user request may start a fresh modeling run. |

These codes apply only to `validate-bicep.mjs`. The promote script has a separate exit `2` meaning documented under [Staged runs](#staged-runs): it published the files, but `git add` failed.
...
10. Only after the checker returns exit `0`, write the [origin record](#origin-record-apporiginjson) into the staging directory.
...
The checker enforces this whenever the model it validates is inside a staging directory, by counting reserved validation attempts in that run's `run.json`. It records the attempt before invoking Bicep, so an unavailable or interrupted check still counts, and it refuses to validate at all if that record cannot be read or updated. A budget it cannot count is one it cannot enforce. The count covers exactly one modeling run, and a later run starts fresh. You do not track attempts yourself, and you cannot validate your way past the limit.

- **Five repairs per run.** The first compile is free, because it is what reveals the problem; after five repair-and-recompile cycles, a completed boundary compile that returns model diagnostics still exits `1`, consumes the final attempt, and says the budget is now spent. A later invocation is refused before validation and exits `2`. That is the same number of repair cycles the deploy-failure repair loop allows after a failed deploy, so the product has one answer to "how many times do we retry a repair on `app.bicep`".
- **The checker tells you when a model failure repeats.** It fingerprints only exit `1` output, with line numbers and diagnostic ordering normalized out, and says so when a model failure is the one you just saw. Treat that as proof the last fix was wrong: make a materially different fix rather than varying it, or use the remaining budget to establish why the schema cannot express what the source needs.
- **An unavailable check still consumes its reserved attempt.** The checker records an attempt before starting Bicep, so exit `2` does not roll that reservation back. Follow the exit `2` action in the [checker contract](#radius-cli-execution-boundary): abort the staged run and do not retry, continue, or manually restart it.
- **If the checker refuses to compile, stop.** A spent budget or broken run record is exit `2`, because no new validation occurred. Follow the [checker contract](#radius-cli-execution-boundary), report the exact refusal, and say that no application model was written.
...
4. Re-run the [validation checklist](#validation-checklist) against the whole file; a change in one resource can ripple to connections or references elsewhere. Follow the [checker exit-code contract](#radius-cli-execution-boundary): only exit `1` permits another model edit, while exit `2` requires aborting the staged run with the exact command in that contract and reporting that no application model was written.
5. After checker exit `0`, write a new [origin record](#origin-record-apporiginjson) into the staging directory so the repaired bytes are the ones recorded, publish the run, and return the corrected file with a short note of what changed and why before handing it back to `radius-deploy` to redeploy. If an exit `1` error recurs, treat the previous fix as insufficient and try a different fix rather than reapplying the one that just failed. If a couple of different fixes still do not resolve it, or no different fix can be found, report that to `radius-deploy` so it can stop the retry loop and surface the problem to the user.
...
For checker exit `2`, follow the [checker exit-code contract](#radius-cli-execution-boundary). Never continue or manually restart that staged run. A fresh modeling run starts only when the user requests it after the underlying failure is corrected.
...
- Write the record on **every** generation and every repair, as the last step, and only after `validate-bicep.mjs` returns exit `0` with no warnings. Never write it after checker exit `1` or `2`. The record holds a hash of the exact bytes that compiled, which is what lets the canvas treat the model as known-valid instead of recompiling it on every graph open. Recording a file that has not passed the checker asserts a validity that was never proven.
...
- [ ] `node "<loaded-skill-base>/scripts/validate-bicep.mjs" <staging-dir>/app.bicep` returned exit `0` and printed no errors or warnings. Exit `1` was treated only as permission to repair the staged model within the budget; exit `2` aborted the run as required by the [checker exit-code contract](#radius-cli-execution-boundary). Every predefined type and API version matches the resolver output. The resolver wrote the staged `bicepconfig.json`; no direct `rad` command or manually selected Radius extension alias was used.
...
- [ ] `.radius/app.origin.json` was written by `write-app-origin.mjs` only after checker exit `0`, describes the final `app.bicep` byte-for-byte, and is staged with it (see [Origin record](#origin-record-apporiginjson)).
