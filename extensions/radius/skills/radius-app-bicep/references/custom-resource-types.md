...
- Mark every sensitive input or sensitive output `x-radius-sensitive: true`. The flag is compiled into `custom-types.tgz`, so Bicep enforces it on the generated type exactly as on a predefined one: every marked node — a string, a whole object (which becomes a `secureObject`), or a leaf nested inside a plain object — must be assigned a `@secure() param` by name in `app.bicep`, or the compile fails with `use-secure-value-for-secure-inputs`. See [secrets-handling.md](secrets-handling.md#what-counts-as-a-secure-value).
