...
As a fail-closed backstop, `validate-bicep.mjs` rejects a direct Recipe-managed aggregate secret key such as `url`, `uri`, `dsn`, or `connectionString` when the target environment name identifies an address part such as `ADDR`, `ADDRESS`, `HOST`, or `PORT`. This `aggregate-secret-alias` check catches a contradiction visible in the compiled model; it is not evidence that other direct bindings are compatible. Do not rename the environment variable or secret key to evade it.
