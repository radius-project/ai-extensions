...
// - A directive is recognized only as the first token on its line, and its
//   codes end where a `//` or `/*` comment starts, even with no space before it.
...
// 1-based line it is on and the security rules it names.
export function securitySuppressions(source) {
  const suppressions = [];
  for (const match of source.matchAll(SUPPRESSION_DIRECTIVE)) {
    const rules = match[2]
      .split(/\/[/*]/u, 1)[0]
      .trim()
      .split(/\s+/u)
...
function remedy(file, stagingDir) {
  const outside =
    stagingDir !== null && !isInside(stagingDir, file) ?
      ` ${file} is outside this modeling run's staging directory, so do not edit it in place: make the change in the run's staged copy.`
    : "";
  return (
  ...
  for (const file of new Set(filePaths)) {
    ...
    if (isBicepConfig(file)) {
      ...
      for (const problem of configurationProblems(parsed.config)) {
        findings.push(
          `${file}: error security-rule-disabled: ${problem}. ${remedy(file, stagingDir)}`
        );
      }
      continue;
    }
    for (const { line, directive, rules } of securitySuppressions(text)) {
      findings.push(
        `${file}:${line}: error security-rule-suppressed: #${directive} suppresses ${rules.join(", ")}. ${remedy(file, stagingDir)}`
      );
    }
  }
