  ...
  return async (req, res) => {
    ...
    if (pathname === "/api/create-environment" && req.method === "POST") {
      ...
      try {
        ...
        const deployWorkflow = generateDeployWorkflow2(envName, provider, ".radius/app.bicep", null);
        ...
        if (entry2) {
          ...
          (async () => {
            ...
            if (!dRunId) {
              ...
              entry2.state.deployError = "The deploy workflow (radius-deploy.yml) did not start. Check that the workflow exists on branch " + branch + " and that Actions are enabled for " + repo + ".";
              entry2.state.deployStatus = "failed";
              return;
            }
...
`);
      ...
      try {
        sendEvent2("info", "Generating GitHub Actions deploy workflow...");
        const workflow = generateDeployWorkflow2(env, provider, appFile, oidc);
        ...
        sendEvent2("info", "Committing workflow to " + targetRepo + "...");
        ...
        if (commitResult.code !== 0) {
          sendEvent2("error", "Failed to commit workflow to repository. Check repo permissions.");
          sendEvent2("done", "failed");
          res.end();
          return;
        }
        sendEvent2("stdout", "Workflow committed to " + targetRepo);
        try {
        ...
        if (provider === "azure") {
          ...
          if (namespace) await runCmd2("gh", ["variable", "set", "RADIUS_K8S_NAMESPACE", "--body", namespace, "--env", env, "--repo", targetRepo]);
        ...
        const triggerResult = await runCmd2("gh", ["workflow", "run", "radius-deploy.yml", "-f", "environment=" + env, "--repo", targetRepo]);
        if (triggerResult.code === 0) {
          sendEvent2("success", "Workflow triggered! Waiting for run to start...");
          await new Promise((r) => setTimeout(r, 5e3));
          const runsResult = await runCmd2("gh", ["run", "list", "--workflow=radius-deploy.yml", "--limit", "1", "--json", "databaseId,status", "--repo", targetRepo]);
          try {
