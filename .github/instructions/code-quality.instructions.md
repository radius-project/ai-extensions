...
Build scripts, configuration, and workflow generators are verified by artifact, configuration, or command-level checks rather than a forced unit test. Generated output such as `.artifacts/radius/` is never hand-edited or tested directly; rebuild it from source and confirm it is in sync.
