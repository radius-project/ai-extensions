...
case "${resource}" in
    ...
    customresourcedefinition)
        ...
        if [[ "${name}" == "httpproxies.projectcontour.io" ||
            ...
            "${name}" == "contourdeployments.projectcontour.io" ]]; then
    ...
    jq -nc '{
      resources:{
        app:{
          type:"Radius.Compute/routes@2025-08-01-preview",
          existing:false,
          properties:{name:"route",properties:{}}
        }
      }
    }' >"${COMPILED}"
...
jq -nc '{
  resources:{
    route:{
      type:"Radius.Compute/routes@2025-08-01-preview",
      properties:{name:"route",properties:{kind:"TCP"}}
    }
  }
}' >"${COMPILED}"
BYO_GATEWAY_JSON="$(
...
export BYO_GATEWAY_JSON
run_action ensure "" true byo byo-system
assert_success "BYO TCP listener"
assert_output "BYO Gateway byo-system/byo is ready"
assert_no_call "kubectl create"
assert_no_call "helm "
...
run_action ensure "" true byo byo-system
assert_failure "BYO incompatible listener"
assert_output "compatible listeners"
assert_no_call "kubectl create"
assert_no_call "helm "
...
# The managed Contour Gateway accepts only the protocols its fixed listeners
# support. TCP and UDP require a compatible user-managed Gateway.
for route_kind in TCP UDP; do
    reset_case
    jq -nc --arg kind "${route_kind}" '{
      resources:{
        route:{
          type:"Radius.Compute/routes@2025-08-01-preview",
          properties:{name:"route",properties:{kind:$kind}}
        }
      }
    }' >"${COMPILED}"
    run_action ensure
    assert_failure "managed ${route_kind} route"
    assert_output "Radius-managed Gateway supports only HTTP and TLS routes"
    assert_output "${route_kind}"
    assert_no_call "kubectl"
    assert_no_call "helm "
done
...
jq -nc '{
  resources:{
    route:{
      type:"Radius.Compute/routes@2025-08-01-preview",
      properties:{name:"route",properties:{kind:"SMTP"}}
    }
  }
}' >"${COMPILED}"
run_action ensure
assert_failure "invalid route kind"
assert_output "compiled application template has invalid resources data"
assert_no_call "rad env show"
assert_no_call "kubectl"
...
for contour_resource in \
    httpproxies.projectcontour.io \
    tlscertificatedelegations.projectcontour.io \
    extensionservices.projectcontour.io \
    contourconfigurations.projectcontour.io \
    contourdeployments.projectcontour.io; do
    reset_case
    CONTOUR_CRDS_STATE='present'
    CONTOUR_SHARED_OBJECTS='true'
    CONTOUR_OBJECT_RESOURCE="${contour_resource}"
    export CONTOUR_CRDS_STATE CONTOUR_SHARED_OBJECTS CONTOUR_OBJECT_RESOURCE
    run_action cleanup
    assert_success "retain shared Contour ${contour_resource} objects"
    assert_output "Ingress or Contour objects use the controller"
    assert_no_call "kubectl delete"
    assert_no_call "helm uninstall"
done
...
for workflow in \
    ...
    "${AWS_WORKFLOW}" \
    "${REPO_ROOT}/.github/extension/delete-azure.yml" \
    "${REPO_ROOT}/.github/extension/delete-aws.yml" \
    "${REPO_ROOT}/.github/extension/delete-environment-azure.yml"; do
    # shellcheck disable=SC2016
    grep -qF 'group: radius-routes-gateway-${{ github.repository }}' \
        "${workflow}" ||
        fail "${workflow}: missing shared routes Gateway concurrency group"
    grep -qF 'cancel-in-progress: false' "${workflow}" ||
        fail "${workflow}: routes Gateway concurrency must not cancel in-progress runs"
done
