      ...
      def route_kinds:
        if type != "object" and type != "array" then
          ...
          [
            ...
            if type != "object" then
            ...
            elif is_routes_resource then
              (
                (.properties // {}) |
                if type != "object" then
                  error("routes resource properties must be an object")
                else
                  (.properties // {}) as $body |
                  if ($body | type) != "object" then
                    error("routes resource body must be an object")
                  else
                    ($body.kind // .kind // "HTTP")
                  end
                end
              ...
              if (
                ...
                ($kind == "HTTP" or $kind == "TLS" or
                 $kind == "TCP" or $kind == "UDP")
              ) then
                $kind
              else
                error("routes resource kind must be HTTP, TLS, TCP, or UDP")
              end
            elif (
              ...
              if (.properties.template | type) != "object" then
                ...
                .properties.template.resources | route_kinds[]
              end
            else
              empty
            end
...
gateway_supports_route_kinds() {
    local gateway_json="$1"
    local route_kinds="$2"
    jq -e --argjson route_kinds "${route_kinds}" '
      (.spec.listeners // []) as $listeners |
      [
        $route_kinds[] as $kind |
        any(
          $listeners[]?;
          ($kind == "HTTP" and
            (.protocol == "HTTP" or .protocol == "HTTPS")) or
          ($kind == "TLS" and .protocol == "TLS" and
            .tls.mode == "Passthrough") or
          ($kind == "TCP" and .protocol == "TCP") or
          ($kind == "UDP" and .protocol == "UDP")
        )
      ] | all
    ' <<<"${gateway_json}" >/dev/null
}
...
gateway_valid() {
    ...
    gateway_supports_route_kinds "${gateway_json}" "${route_kinds}" ||
        return 1
...
validate_byo() {
    ...
    gateway_valid "${GATEWAY_NAME}" "${GATEWAY_NAMESPACE}" "" \
        "${route_kinds}" ||
        fail "BYO Gateway validation failed: Gateway, compatible listeners, GatewayClass, or controller is not ready"
    echo "BYO Gateway ${GATEWAY_NAMESPACE}/${GATEWAY_NAME} is ready; no resources changed."
}

validate_managed_route_kinds() {
    local route_kinds="$1"
    local unsupported
    unsupported="$(
        jq -r '[.[] | select(. != "HTTP" and . != "TLS")] | join(", ")' \
            <<<"${route_kinds}"
    )"
    [[ -z "${unsupported}" ]] ||
        fail "Radius-managed Gateway supports only HTTP and TLS routes; configure a compatible BYO Gateway for: ${unsupported}"
}
...
shared_contour_consumers_exist() {
    ...
    for resource in "${CONTOUR_OBJECTS[@]}"; do
        if optional_contour_objects_exist \
            "${resource}" "${resource}" \
            "cluster-wide Contour ${resource} objects"; then
            return 0
        fi
    done
    return 1
...
ensure_gateway() {
    ...
    route_kinds="$(app_route_kinds "${compiled}")" ||
    ...
    validate_gateway_inputs
    if [[ "${GATEWAY_EXPLICIT}" != "true" ]]; then
        validate_managed_route_kinds "${route_kinds}"
    fi
    require_gateway_tools
    if [[ "${GATEWAY_EXPLICIT}" == "true" ]]; then
        validate_byo "${route_kinds}"
        return 0
    fi
