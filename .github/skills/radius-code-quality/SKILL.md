...
- Keep server modules decomposed. A production server file above 750 lines requires an explicit decomposition review and a recorded exception. Route family names assign ownership and do not justify one large file per family.
...
- The 80% line, 80% function, and 70% branch thresholds that apply to newly extracted runtime, route, renderer, and browser modules are minimum migration gates, not the target for new work under this skill.
