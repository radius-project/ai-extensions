...
- Preserve one loadable generated extension bundle at `.artifacts/radius/com.github.copilot/extensions/radius/extension.mjs`, with Copilot SDK imports externalized. The assembled plugin is published at `plugins/radius/` on release branches, so its published bundle is `plugins/radius/com.github.copilot/extensions/radius/extension.mjs`. Never hand-edit generated output.
...
| Selected stable visual state                                                             | Owning behavioral tests plus current canonical light- and dark-theme baselines. `Visual comparisons` checks pull requests that change visual inputs; for an intended change, add the `pr:update-visual-baselines` label or run `pnpm test:visual:canonical:update`, then review every changed image                     |
...
| Generated output such as `.artifacts/radius/`                                            | Rebuild from source, run built-extension smoke, and confirm the artifact is in sync; never hand-edit generated output                                                                                                                                                                                                   |
