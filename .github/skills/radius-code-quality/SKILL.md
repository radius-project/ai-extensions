...
Both documents track their own delivery status; the test plan's phase table is the sole source of truth for which infrastructure is delivered. Do not copy phase completion status into this skill. Apply every pull-request gate that the plan marks as delivered. Real-host qualification is a scheduled release gate and does not block pull requests.
...
Every pull request must apply the following matrix using the test plan's current delivery status. The rows are cumulative: when a change matches multiple rows, include the evidence from every matching row whose infrastructure is delivered. Across those levels, cover the changed behavior's success case, error and failure cases, and boundary and edge conditions without duplicating the same scenario. A higher-level test complements focused lower-level tests by protecting a distinct boundary contract or cross-layer risk; it does not repeat or replace them.
...
| Selected stable visual state                                                             | Owning behavioral tests plus reviewed light- and dark-theme visual baselines; exercise representative success, empty/loading, and error states when they are part of the selected visual inventory                                                       |
| Async work, external call, subprocess, polling, cancellation, timeout, or cleanup        | Tests at the owning unit and boundary levels plus the reliability suite when the regression involves races, repetition, multiple instances, platform paths, timeout limits, or cleanup under failure                                                     |
...
Documentation-only changes do not require product tests unless they change executable examples, generated inputs, test configuration, or claimed behavior. For all other pull requests, omitting a matrix row requires a written explanation that the row is structurally inapplicable to the change; inconvenience, implementation size, or coverage at another layer is not sufficient.
...
These are the required pull-request gates. The Windows process integration command runs in the Windows CI job; component and Chromium commands run in the Playwright CI container. Run them locally when the required environment is available, and otherwise require their CI jobs to pass without claiming they ran locally. `pnpm run test:visual` and `pnpm run test:reliability` are scheduled gates, not pull-request gates; run their focused cases while changing selected visual baselines or the resilience behavior they protect. Do not replace any required gate with manual validation or imply real-host coverage from another suite.
