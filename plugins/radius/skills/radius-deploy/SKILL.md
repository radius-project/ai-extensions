...
1. Checks the Repo Radius actions already pinned in the repository's deploy workflows against the version this build of the extension requires. When they match, the deploy proceeds with no prompt. When they are older, the dispatch is withheld and the developer is shown what would change; on confirmation the new commit SHAs are committed to the default branch, or opened as a pull request when that branch is protected. Nothing is written to the repository without that confirmation.
...
- `plugins/radius/extension.mjs` — deploy workflow template generation (`generateDeployWorkflow`) and repo commit of the dispatcher + provider workflows to `.github/workflows/`.
- `plugins/radius/extension.mjs` — deploy dispatch + run polling (uses `gh workflow run run-rad-commands.yml` and `gh run list`).
- The deploy workflow templates are canonical in `radius-project/radius` at `.github/extension/` — `run-rad-commands.yml` (dispatcher), `run-rad-commands-{azure,aws}.yml` (provider `workflow_call` workflows), and `actions/*` (shared composite actions: `setup-control-plane`, `restore-state`, `apply-custom-recipe-packs`, `run-rad-commands`, `teardown`). The extension fetches these at the commit SHA pinned in `radius-core/src/workflows/pinset.ts` (never a moving branch) and commits the dispatcher + provider workflows into the user repo at `.github/workflows/`; the composite actions are referenced in place from `radius-project/radius`, not copied, with every `uses:` rewritten to that exact SHA.
