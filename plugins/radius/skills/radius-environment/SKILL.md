...
- The verification workflow templates are the canonical `verify-aws.yml` / `verify-azure.yml` (both named `Radius - Verify Credentials`) hosted in `radius-project/ai-extensions` at `.github/extension/`. The extension fetches the matching provider template from there at commit time (there is no bundled copy, so a fetch failure is a hard error) and commits it into the target user repo at `.github/workflows/verify-<provider>.yml`, then dispatches it.
