...
- **In `app.bicep`** — set `codeReference: '<path>[#L<line>]'` inside a resource's `properties`. `rad app graph` preserves it and the canvas renders the link. When authoring or updating `app.bicep` (see the `radius-app-modeling` skill), populate `codeReference` for every non-application resource you can locate.
