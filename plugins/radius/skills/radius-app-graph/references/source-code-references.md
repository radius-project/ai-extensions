...
7. **Last resort** — use a `docker-compose.*` declaration when it exists; otherwise use the resource's own declaration line in `.radius/app.bicep`. The generated Bicep declaration is the authoritative definition site when the application has no separate initialization file.
...
| image/container | `Dockerfile`; `server`,`app`,`main`,`index` (incl. under `src/`)                                                                           |
