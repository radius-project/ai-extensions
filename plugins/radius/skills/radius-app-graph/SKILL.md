...
- **Graph build fails**: verify `rad app graph <app.bicep> --include-icons` succeeds locally, or check that the cached/downloaded `rad` binary is executable. Do not add `--preview` to this modeled command.
