...
- The delete workflow templates are canonical in `radius-project/radius` at `.github/extension/` — `delete-application.yml` (dispatcher), `delete-azure.yml` / `delete-aws.yml` (provider `workflow_call` workflows), and `actions/*` (shared composite actions: `setup-control-plane`, `restore-state`, `delete-resource`, `teardown`). The extension fetches them at the commit SHA pinned in `radius-core/src/workflows/pinset.ts` — the same pin the deploy workflows use — and commits the dispatcher + the Azure provider workflow into the user repo at `.github/workflows/`, with the composite actions referenced in place from `radius-project/radius` at that exact SHA. A stale pin is reported but never blocks a delete: refusing to tear down an application because its workflow needs an upgrade would strand the cloud resources it is holding.
