...
- **Delete a deployment** dispatches the committed `delete-application.yml` GitHub Actions workflow, which spins up an ephemeral k3d Radius control plane, connects to the target AKS cluster, restores persisted state, runs `rad app delete`, and persists the updated state again before tearing the control plane down. Deleting the deployment also deletes that application's resources (running their recipes' delete path against the target cluster and cloud). The confirmation dialog lists the deployed resources the deletion will remove. Every delete dispatch carries a unique correlation id that the workflow echoes into its run name, so the canvas tracks the exact run it started — a delete of another environment running at the same time is never mistaken for it, and a retried delete reports its own outcome rather than the previous attempt's.
- **Delete a removed resource** dispatches the committed `delete-resource.yml` workflow. Deployment is incremental, so a resource dropped from `.radius/app.bicep` stays deployed; the deployed graph lists those resources and each can be deleted individually. The canvas only accepts a request whose resource identity, type, application, environment, repository, branch, and deployed-graph revision match the removed set it derived itself from resources the application verifiably owns, and it reloads the application definition and re-derives the removal immediately before dispatching — so a branch switch, a re-modeled definition, or a newer deployed inventory refuses instead of deleting. While the cleanup runs it blocks every other mutation of that deployment, including one started from another canvas instance. It uses its own dispatcher rather than `delete-application.yml` so a successful single-resource cleanup is never read as a whole-application teardown, and on success it republishes the application's resource inventory from the live control plane so the deployed graph stops offering the resource it just removed. If the cleanup run could not persist Radius state, the Deployments list keeps showing the application as deployed and annotates its row with the orphan warning and a link to the cleanup run, so the warning survives a page reload.
...
6. Runs `rad app delete <name> --yes --preview` (`--preview` switches the CLI to the deployed-application API path) via the `delete-resource` composite action, which writes a `rad-delete-result` artifact (JSON: `outcome`, `exitCode`, `resourceType`, `name`, `application`, `environment`, `radiusResourceType`, `output`). For a single removed resource the action instead verifies ownership with `rad resource show <radius-resource-type> <name> -o json` and then runs `rad resource delete <radius-resource-type> <name> --yes`; that command is not preview-wired and has no `--application` flag, so the explicit ownership check is what scopes the delete to one application, and it refuses whenever ownership cannot be established.
7. `rad shutdown` (`if: always()`, retried up to three times with a bounded backoff) persists the post-delete control-plane databases and Terraform recipe-state Secrets back to the state archive — the OCI-backed archive by default (pushed to the GHCR repository in `RADIUS_STATE_REGISTRY` under the `RADIUS_STATE_ARCHIVE` tag, default `radius-state`), or the `radius-state` git orphan branch when `RADIUS_STATE_BACKEND=git`. When every attempt fails, the run publishes a `radius-state-save-failure-attempt-<run_attempt>` artifact and the canvas warns that orphaned resources may exist and that recovery is redeploy, then delete and redeploy — including on a delete that otherwise succeeded, whose row is retained with that warning instead of disappearing. On failure, logs are uploaded as the `radius-logs` artifact; the k3d cluster is always deleted.
