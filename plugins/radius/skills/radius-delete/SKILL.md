...
- **`dial tcp: lookup dynamic-rp.radius-system ... no such host` during `rad app delete`**
  → The application has more than one page of resources of a single type, and the CLI followed the listing's absolute cluster-internal `nextLink` directly instead of going through the Kubernetes API-server proxy. The `delete-resource` composite action aliases the control-plane services onto loopback before the delete to work around this. If the alias step logs `Port ... is already in use` or `Could not port-forward ...`, that service was not aliased and the same failure returns. The underlying leak is tracked in radius-project/radius#12837.
