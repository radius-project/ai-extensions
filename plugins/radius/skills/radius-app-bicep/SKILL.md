...
- [ ] A developer-supplied credential consumed through connection projection is stored in an authored or reused `Radius.Security/secrets` and connected through `<secret>.id`; a Recipe-generated credential is obtained by connecting only to `<producer>.id`. Each generated `CONNECTION_<CONNECTION>_<SECRETKEY>` suffix is the uppercased authored data key or Recipe `result.secrets` key; case-normalized collisions fail rather than choosing a value. `secretKeyRef` with `<producer>.properties.secrets.name` is used only for an explicitly required custom Kubernetes environment name. Explicit `env` precedence and `disableDefaultEnvVars` do not suppress a required generated credential. No secret is hardcoded, copied from a Recipe result, or moved into plain state; runtime composition preserves ordering, escaping, and required encoding.
