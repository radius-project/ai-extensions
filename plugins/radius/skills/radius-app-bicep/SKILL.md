...
- Generic projection can be a `CONNECTION_<NAME>_PROPERTIES` JSON value, individual `CONNECTION_<NAME>_<PROPERTY>` values, or another version-specific shape. Secret connections generate secret-backed `CONNECTION_<CONNECTION>_<SECRETKEY>` variables. `<CONNECTION>` is the connection map key uppercased without inserting separators (`postgresSecret` becomes `POSTGRESSECRET`); `<SECRETKEY>` is the uppercased authored Secret data key or Recipe `result.secrets` key. Case-normalized collisions are errors. Verify the configured extension/runtime contract; do not invent or rename a segment.
- Secret-backed `CONNECTION_*` projection is Kubernetes Container Recipe behavior and requires compatible Radius control-plane and Container Recipe versions. Read the control-plane version with `rad version --output json`; inspect the configured Recipe/template metadata with `rad recipe show <name> --resource-type <type> --output json`; and verify the resolved schema configuration plus the pinned Recipe tag or digest. These signals do not automatically prove secret-connection projection. Rely on it only when the resulting control-plane version and exact template contract are known compatible; otherwise preserve an existing schema-supported `env`, `secretKeyRef`, `envFrom`, native variable, or equivalent binding. Do not migrate a working `app.bicep` to secret-backed connections without explicit user intent, and do not recommend this Kubernetes projection for Azure Container Instances (ACI).
...
- Explicit native variables may coexist with generic projection. An explicit `env` entry takes precedence when it has the same name as a generated connection variable. Avoid conflicting values, and set `disableDefaultEnvVars: true` on the connection only when all of that connection's generated variables must be suppressed; never set it when the workload relies on a generated secret-backed variable.
...
- [ ] A developer-supplied credential consumed through connection projection is stored in an authored or reused `Radius.Security/secrets` and connected through `<secret>.id`; a Recipe-generated credential is obtained by connecting only to `<producer>.id`. Each generated `CONNECTION_<CONNECTION>_<SECRETKEY>` suffix is the uppercased authored data key or Recipe `result.secrets` key; case-normalized collisions fail rather than choosing a value. An explicit native or compatibility-fallback `secretKeyRef` for an authored Secret uses `<secret>.name` and its declared data key; `secretKeyRef` with `<producer>.properties.secrets.name` is reserved for an explicitly required custom Kubernetes environment name for a Recipe result. Explicit `env` precedence and `disableDefaultEnvVars` do not suppress a required generated credential. No secret is hardcoded, copied from a Recipe result, or moved into plain state; runtime composition preserves ordering, escaping, and required encoding.
