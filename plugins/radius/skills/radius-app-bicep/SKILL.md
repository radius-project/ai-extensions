...
8. Generate the Bicep into `<staging-dir>/app.bicep` using [naming-conventions.md](references/naming-conventions.md). Keep Azure-bound names, administrator logins, and model names within the rules in [azure-provider-value-rules.md](references/azure-provider-value-rules.md). Use string literals or parameters with literal defaults for these values. If the source requires a value that the selected Recipe cannot deploy, report the conflict and abort the run; do not rename the source-required database, container, topic, or model. Then run `node "<loaded-skill-base>/scripts/validate-bicep.mjs" <staging-dir>/app.bicep`. Repair every compiler error and warning until the checker exits successfully and prints no warnings, within the [repair budget](#repair-budget) it enforces. Never make compilation pass by deleting a required backend activation, native configuration value, secret binding, or dependency edge.
...
2. Locate the implicated resource, property, or workload setting, then re-resolve the exact configured type schema and Recipe contract (see [Resource Type Resolution](#resource-type-resolution)) to confirm property names, required fields, credential shape, API version, resource reference paths, and provider restrictions. For the canonical Azure Recipe Pack, check [azure-provider-value-rules.md](references/azure-provider-value-rules.md). If the application requires the rejected value, report the application/Recipe conflict instead of silently changing it.
...
| Data store `database` name         | Derived from source (e.g., `MYSQL_DATABASE`/`POSTGRES_DB`, or the database segment of a connection string). For the canonical Azure Recipe Pack, it must also satisfy [azure-provider-value-rules.md](references/azure-provider-value-rules.md). Do not rename a source-required database to make it pass.                                                         |
...
- [ ] When the selected Environment uses the canonical Azure Recipe Pack, every provider-bound database name, administrator login, container name, Kafka topic, MongoDB database, and AI model satisfies [azure-provider-value-rules.md](references/azure-provider-value-rules.md). Each value is a string literal or has a literal parameter default. A source-required incompatible value stops the run instead of being renamed. Azure-only rules were not applied to another provider or an unverified custom Recipe.
