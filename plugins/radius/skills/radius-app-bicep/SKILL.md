...
- [ ] Read-only properties are never **set**. Every referenced read-only value is present in the returned schema and has separate evidence that the target Recipe populates it in a form compatible with the application. Direct references provide dependency ordering.
