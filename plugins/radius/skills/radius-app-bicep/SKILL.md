...
Never invoke `rad` or `rad.exe` directly from PowerShell, a shell, a subprocess, or a delegated agent. Run the bundled `show-radius-type.mjs` for predefined type definitions; that script performs the supported read-only managed-CLI identity query internally. Compile the generated application definition with `node "<loaded-skill-base>/scripts/validate-bicep.mjs" .radius/app.bicep`; the checker uses only the extension-managed Bicep and fails on every compiler warning or error. Graph validation must go through the Radius canvas and its tools: open `canvasId: "radius"` with `instanceId: "radius-panel"`, pass the current session repository as `repo` in `owner/repo` form, and use the current Copilot worktree branch. The extension honors an existing `RADIUS_RAD_BINARY`; otherwise it runs its managed binary from `%USERPROFILE%\.radius\ai-extensions\bin\rad.exe` on Windows or `$HOME/.radius/ai-extensions/bin/rad` on macOS/Linux, downloading it when absent and attempting a best-effort upgrade when older than the latest release (offline/API failures keep the installed binary; set `RADIUS_RAD_SKIP_VERSION_CHECK` to skip the version check). It does not resolve `rad` from `PATH` or `.rad/bin`. Diagnose graph failures only from the Radius extension log; never reproduce them with a direct CLI command or through another agent.

When Canvas starts this workflow, `radius_generate_app` returns `skillBase` and `skillVersion`. Substitute those exact values for `<loaded-skill-base>` and `<loaded-skill-version>` throughout this skill; do not rediscover or omit them.

## Workflow

1. Start the run with `node "<loaded-skill-base>/scripts/promote-app-model.mjs" --begin`, which prints the staging directory to write everything into (see [Staged runs](#staged-runs)). Then select one runnable deployment profile. Treat explicit user, scenario, and target-repository deployment requirements for Radius types, resource-name parameters, workload roles/count, native configuration keys, secret bindings, provider profile, protocol values, and connection names as acceptance criteria. Verify that the pinned source supports that profile; do not silently replace it with an easier default or optional backend. Before inventorying the repository, decide external-client ingress using only the selected profile's startup and configuration chain and the [route authoring rule](#appbicep-structure-mandatory-order). Ask and stop if it is ambiguous; do not revisit the decision with files from an unselected profile.
2. Build an internal requirement ledger that maps every acceptance criterion and planned resource property reference to source evidence, its exact JIT schema path and access mode, and the workload setting that consumes it. Use it for reasoning and validation; do not print it or add it as Bicep comments. Follow [runtime-contract.md](references/runtime-contract.md).
...
2. Locate the implicated resource, property, or workload setting, then rerun the JIT lookup (see [Resource Type Resolution](#resource-type-resolution)) to confirm property names, required fields, credential shape, API version, and resource reference paths.
...
It removes any staging directory a previous interrupted run left behind, records the fingerprints of the files in `.radius/` this run may replace, and prints the staging directory. It writes nothing outside that directory, which is what lets a failed run leave `.radius/` byte-identical without having to undo anything. Write `app.bicep`, the origin record, and any authored custom-type files into that directory and nowhere else. `show-radius-type.mjs` writes `bicepconfig.json`; `radius_publish_custom_type_extension` writes `custom-types.tgz` and maintains its config alias when passed the printed directory's `.staging-<runId>` basename as `stagingDir`. Run the Bicep checker against the staged `app.bicep`, so what is verified is exactly what will be published.
...
- **Canonical ordering within each group.** Order `param` declarations and resource types using plain ASCII ordering, then resources of the same type by their `name` value. The one exception is a resource that must be declared after something it references.
...
Use [component-catalog.md](references/component-catalog.md) only to choose candidate types. Before authoring, run `show-radius-type.mjs` once with every distinct predefined type that will be declared or referenced:
...
The command returns compact JSON with an ordered `resources` array. Each entry contains the exact `type`, `apiVersion`, and one recursive `schema`. Names listed in an object's `required` array must be authored; other properties are optional. A property marked `readOnly` may be referenced but not authored, one marked `writeOnly` may be authored but not referenced, and an unmarked property supports both. Follow nested `properties`, array `items`, map `additionalProperties`, `enum`, `oneOf`, constraints, and `sensitive` directly. Never follow raw compiler references or infer flag meanings—the script has already converted them.

If a type has multiple API versions, rerun the batch with an exact `@<api-version>` for that selector. If a required type or property is absent, stop; do not fetch contrib schemas, defaults, Recipe packs, OCI metadata, or another release as fallback. Do not substitute a similarly named type. Generate a `Radius.Resources/*` custom type only when no predefined type returned by this lookup fits the required Azure-provisionable backing service; follow [custom-resource-types.md](references/custom-resource-types.md).

## Extension

Declare `extension radius`. It provides every predefined Radius type (`Radius.Core/*`, `Radius.Compute/*`, `Radius.Data/*`, `Radius.Messaging/*`, `Radius.AI/*`, `Radius.Storage/*`, `Radius.Security/*`). Do NOT declare per-namespace or per-type extensions (`radiusCompute`, `containers`, `kafka`, etc.). When the application uses a generated custom type (see [custom-resource-types.md](references/custom-resource-types.md)), also declare the local custom-types extension published into `.radius/` (for example `extension customTypes`). The resolver and custom-type publisher maintain their aliases in the staged config.

## bicepconfig.json

`show-radius-type.mjs` writes `<staging-dir>/bicepconfig.json` only after every requested type resolves. It preserves compatible repository settings, enables extensibility, and fills an absent or blank `radius` alias from the managed CLI version. A different nonblank alias is a version conflict and stops the run without changing the config. Do not create, edit, or replace this file yourself, and do not select another extension tag to clear a diagnostic. When custom types are needed, `radius_publish_custom_type_extension` adds the local `customTypes` alias itself (see [custom-resource-types.md](references/custom-resource-types.md)).
...
- Reference the peer's host from `<peer>.properties.hosts`, never from the resource `name` or a hand-built `<resource-name>-<containerKey>` string. Use dot access for a known static key that is a valid Bicep identifier (`identityApi.properties.hosts.identity`). Use indexed access only for a dynamic key or one that is not a valid identifier (`peer.properties.hosts['identity-api']`). Do not suppress `prefer-unquoted-property-names` for a known identifier. The containers recipe populates `hosts` with each port-exposing container's actual Service FQDN, so the reference is stable and creates a deploy-time dependency edge.
...
- **Inputs** (credentials you supply): follow the returned JIT schema — set a sensitive property (e.g. `password`) from a `@secure()` parameter; author a referenced `Radius.Security/secrets` only when the schema uses `secretName`; or none. When the app container also needs a supplied credential, assign the same `@secure()` parameter directly to its `env.value` — Radius encrypts and injects it, so do NOT wrap it in a `Radius.Security/secrets` or route it through `secretKeyRef`.
- **Outputs** (values a recipe generates): use only readable paths present in the returned JIT schema, then verify target-specific population during deployment readiness. When the contract declares managed-secret metadata, bind the workload directly with `secretName: <resource>.properties.secrets.name` and its exact key. Never copy a recipe secret through an authored wrapper or guess `<resource>.properties.<key>`; if the managed-secret contract is absent, report the gap.
...
- [ ] One `show-radius-type.mjs` call covered every predefined type. Every planned property read/write has its verbatim path and access mode in the returned JIT schema. An absent path blocks generation rather than being replaced by a guessed property, alias, or wrapper.
- [ ] When a target Environment is selected, deployment readiness verifies Recipe registration and consumed output population. Without one, describe the model as schema-validated and make no target-specific readiness claim.
- [ ] Exactly one `Radius.Core/applications` uses its returned API version. `extension radius` is always declared; a local custom-types extension (for example `extension customTypes`) is also declared when the app uses a generated `Radius.Resources/*` custom type. No per-namespace or per-type extensions.
- [ ] `node "<loaded-skill-base>/scripts/validate-bicep.mjs" <staging-dir>/app.bicep` exits successfully and prints no errors or warnings. Every predefined `Radius.*` type and API version matches the JIT response, and the resolver wrote the staged `bicepconfig.json`. No direct `rad` command was used.
- [ ] `param environment string` is declared. Every expression supplied to a JIT node marked `sensitive: true` originates from a `@secure()` parameter, including nested map values whose key name does not look secret.
...
- [ ] When a `Radius.Compute/containerImages` resource pushes to an authenticated registry, it is paired with a `Radius.Security/secrets` named exactly `radius-ghcr-registry-creds` (keys `username`/`password` from `@secure()` `registryUsername`/`registryPassword` params) and `dependsOn`s it; no registry is set on the resource. The Secret name matches the recipe pack's `containerImagesRegistrySecretName`. For an unauthenticated registry, no such Secret is authored.
- [ ] Every source build pins the exact modeled revision and authors only build inputs supported by the returned schema; any explicit platform matches the Dockerfile and target runtime.
- [ ] Credentials match the returned schema: `username`+`password` on the resource, or `secretName`+secret, or none. Every `sensitive: true` input uses a `@secure()` parameter; `database`/`topic`/`queue`/etc. is derived from source.
...
- [ ] Read-only properties are never **set**. Every reference is readable in the returned schema; target-specific output population remains a deployment-readiness check. Direct references provide dependency ordering.
...
- [ ] The credential shape the returned schema and exact target Recipe expose matches what the pinned client parses. Address outputs stand alone only where the exact target Recipe provably generates no credential and the reply names that dependence; an unresolved Recipe counts as generating one. No undeclared discrete property or secret key is invented; no aggregate credential is split in Bicep, in an authored secret, or by a wrapper the pinned image cannot run. A shape mismatch with no proven path is reported and the run is not published.
...
- [ ] The generated Bicep contains no explanatory comments. The staged config resolves every declared extension and was changed only by the resolver or custom-type publisher.
