...
- Secret connections generate secret-backed `CONNECTION_<CONNECTION>_<SECRETKEY>` variables. `<CONNECTION>` is the uppercased connection map key without inserting separators (`postgresSecret` becomes `POSTGRESSECRET`); `<SECRETKEY>` is the uppercased authored Secret data key or Recipe `result.secrets` key. An explicit `env` entry wins over a generated variable with the same name. `disableDefaultEnvVars: true` suppresses all generated variables for that connection. Normalized generated-name collisions fail validation rather than selecting a value.
