...
- Generic projection can be a `CONNECTION_<NAME>_PROPERTIES` JSON value, individual `CONNECTION_<NAME>_<PROPERTY>` values, or another version-specific shape. Secret connections generate secret-backed `CONNECTION_<CONNECTION>_<SECRETKEY>` variables; the suffix is the uppercased authored Secret data key or Recipe `result.secrets` key. Case-normalized collisions are errors. Verify the configured extension/runtime contract; do not invent or rename a suffix.
