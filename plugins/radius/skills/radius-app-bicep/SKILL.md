...
2. Commit both files to that branch, then push it to the remote, setting the upstream when the branch has none (for example, `git push -u origin <branch>`). Confirm the push succeeds before relying on any downstream view: the application graph reads `.radius/app.bicep` from the pushed branch, so a local save or an unpushed commit is not enough.
