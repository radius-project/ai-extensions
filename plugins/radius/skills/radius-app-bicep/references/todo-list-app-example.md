...
- **Image**: the pinned [`Dockerfile`](https://github.com/dockersamples/todo-list-app/blob/5a6fbf5caf982f1d928fe6c1c32aa74f1e95e063/Dockerfile) is the complete source-build context. It is a single unpinned `FROM node:22-alpine` stage whose only build step is `yarn install --production`, so it cannot silently copy build-platform output into an image for another architecture: when the stage can execute, it installs the native `sqlite3` dependency for the requested platform; when it cannot, the build fails before publishing an image rather than publishing one with mismatched binaries. Both tests in [Choosing build.platforms](bicep-structure-rules.md#choosing-buildplatforms) hold, so `build.platforms` is omitted and the multi-arch default stands without claiming every builder can execute every requested platform. Tag omission still requires validation against the exact current `containerImages` Recipe; this example makes no claim that omission is broken.
