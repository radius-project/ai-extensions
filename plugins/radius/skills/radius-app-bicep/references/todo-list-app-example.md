...
- **Image**: the pinned [`Dockerfile`](https://github.com/dockersamples/todo-list-app/blob/5a6fbf5caf982f1d928fe6c1c32aa74f1e95e063/Dockerfile) is the complete source-build context. It is a single unpinned `FROM node:22-alpine` stage whose only build step is `yarn install --production`, so it is not fixed to one architecture and everything it installs — including the native `sqlite3` dependency — is installed as the requested platform. Both tests in [Choosing build.platforms](bicep-structure-rules.md#choosing-buildplatforms) hold, so `build.platforms` is omitted and the multi-arch default stands. Tag omission still requires validation against the exact current `containerImages` Recipe; this example makes no claim that omission is broken.
...
6. Pin `build.source` to the modeled commit, validate tag omission against the exact current Recipe, omit `build.platforms` because the single unpinned stage builds correctly for both defaults, and consume the build through verified `properties.imageReference`.
