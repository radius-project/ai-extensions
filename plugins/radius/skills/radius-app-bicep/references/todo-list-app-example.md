...
- **Image**: the pinned [`Dockerfile`](https://github.com/dockersamples/todo-list-app/blob/5a6fbf5caf982f1d928fe6c1c32aa74f1e95e063/Dockerfile) is the complete source-build context. It is a single unpinned `FROM node:22-alpine` stage whose build step is `yarn install --production`. Although that step would install the native `sqlite3` dependency for the requested platform if it could execute, the amd64 builder has no QEMU/binfmt emulation and therefore cannot run the arm64 stage. Test C in [Choosing build.platforms](bicep-structure-rules.md#choosing-buildplatforms) fails, so set `build.platforms: ['linux/amd64']`; this image will not run on an arm64-only cluster until its Dockerfile adopts a cross-build strategy. Tag omission still requires validation against the exact current `containerImages` Recipe; this example makes no claim that omission is broken.
