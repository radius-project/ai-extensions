...
Before writing Bicep, enumerate every planned resource property read and write as a separate ledger row. Record the verbatim path, recursive type, and access mode from the batch JIT response. The schema does not prove defaults or target-specific Recipe output population. For a managed secret, require the readable nested secret-name path and exact key; key declarations are metadata, not readable secret values. The consumer must use that managed secret directly through `secretKeyRef`; any row that reads the key as a resource property or copies it into an authored secret fails preflight.

Reject the model before generation if any required type or property path is absent from the JIT response. Do not repair a missing output by guessing a direct convenience property, choosing a similarly named alias, copying it through an authored secret wrapper, or retaining only an unconsumed connection. Compilation is downstream confirmation, not property-path discovery. Recipe registration and output population are deployment-readiness checks when a target Environment is selected.

If compilation disagrees with the commit-derived JIT response, stop and report a managed release or extension mismatch rather than querying another schema source or deleting feature-critical wiring.
...
- Inspect the exact image Recipe's default platforms. If the Dockerfile cannot safely build them with a verified cross-build strategy or builder emulation, set an explicit platform supported by the deployment target. Require direct Dockerfile, builder, and target-runtime evidence rather than inferring incompatibility from an unrelated tool.
...
6. Confirm every source build pins the modeled revision, validates optional Recipe paths, selects compatible platforms, and preserves required Git metadata.
