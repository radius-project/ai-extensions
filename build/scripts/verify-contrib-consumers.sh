...
fetch_defaults_catalog() {
    local defaults="${TMP_ROOT}/radius-defaults.yaml"
    local url="https://raw.githubusercontent.com/${CATALOG_REPO}/${CATALOG_REF}/deploy/manifest/defaults.yaml"
    curl --proto '=https' --tlsv1.2 -fsSL --retry 5 --retry-connrefused \
        "${url}" -o "${defaults}"
    [[ -s "${defaults}" ]] || fail "downloaded defaults catalog is empty."
    printf '%s' "${defaults}"
}
...
main() {
    ...
    local defaults="${DEFAULTS_YAML}"
    if [[ -z "${defaults}" ]]; then
        defaults="$(fetch_defaults_catalog)"
    fi
    [[ -f "${defaults}" ]] || fail "defaults catalog not found: ${defaults}"
    export RADIUS_DEFAULTS_YAML="${defaults}"

    # shellcheck disable=SC1090 # The helper path is configurable for tests.
