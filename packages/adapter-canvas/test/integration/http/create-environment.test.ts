    ...
    expect(harness.finished).toEqual([
      {
        ...
        options: {
          terminal: {
            ...
            baseBranch: "main",
            userMessage:
              "I couldn't push the workflow files to the default branch of `octo/app` directly, so I committed them to `radius/setup-dev-workflows-op-http` and opened a pull request: https://github.com/octo/app/pull/7. Merge it to finish setting up the repository."
          }
        }
      }
    ]);
