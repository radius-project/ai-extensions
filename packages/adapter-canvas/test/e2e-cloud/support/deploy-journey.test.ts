...
describe("findDeleteEnvironmentSuccessProblems", () => {
  ...
  it("reports a refused delete, quoting the reason the product gave", () => {
    expect(
      findDeleteEnvironmentSuccessProblems({
        status: 409,
        payload: { error: "still deployed", code: "app-deployed" },
        environmentName: "radtest-env"
      })
    ).toEqual([
      'Deleting environment "radtest-env" answered 409, not 200: still deployed'
    ]);
  });

  it("reports a failed delete that gave no reason at all", () => {
    expect(
      findDeleteEnvironmentSuccessProblems({
        status: 503,
        payload: null,
    ...
    ).toEqual(['Deleting environment "radtest-env" answered 503, not 200.']);
  });

  it("reports a 200 that did not actually report success", () => {
