...
/**
 * Everything wrong with how the product accepted a legitimate environment delete.
 *
 * The mirror of the refusal above, and the pair is the point: stage four proves
 * the Environment survives a *deployment* delete, stage five proves it does not
 * survive an *environment* delete. Neither means much alone.
 */
export function findDeleteEnvironmentSuccessProblems(input: {
  readonly status: number;
  readonly payload: unknown;
  readonly environmentName: string;
}): string[] {
  ...
  if (input.status !== 200) {
    ...
    problems.push(
      `Deleting environment "${input.environmentName}" answered ${input.status}, not 200${detail}`
    );
    return problems;
  }
  const record = asRecord(input.payload);
  if (record?.success !== true)
    problems.push(
      `Deleting environment "${input.environmentName}" answered 200 but did not report success: ` +
