...
<div id="graph-guidance" style="margin-top:8px; font-size:12px; color:var(--rad-text-tertiary);">
  Click a node to view source code links.
    ...
    : `<div id="graph-status" class="status info">Select a branch to read the application graph. Missing or unsupported definitions are reported as unavailable; source is never generated by a graph read.</div>
<div id="graph-container-wrapper"></div>`;
  ...
  return pageShell(
    ...
    `
${graphHeader("graph")}
${renderGraphEvidence(state.graphReadEvidence?.graph)}
...
${renderPageState(GRAPH_PAGE_STATE_ID, {
  ...
  evidence: state.graphReadEvidence?.graph
})}
