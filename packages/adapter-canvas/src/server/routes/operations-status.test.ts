...
describe("operation resume and abandon actions", () => {
  ...
  it.each([[true], [false]])(
    "records an app-registration delete decision=%s on a delete operation's flat request",
    async (decision) => {
      // Delete operations carry a flat request with no `azure` block, so this
      // path must be handled before the azure-request guard rejects it.
      const operation = actionRecord({
        inputRequired: {
      ...
      const recording = await runAction(
        "/api/operations/op-action/resume/delete-app-registration-decision",
        JSON.stringify({ deleteAppRegistration: decision }),
        handleResumeOperation,
        actionDependencies({
          getOperation: () => operation,
          canResumeInput: () => true,
      ...
      expect(recording.status).toBe(202);
      expect(
        (operation.request as Record<string, unknown>).deleteAppRegistration
      ).toBe(decision);
      expect(scheduled).toHaveLength(1);
    }
  );

  it("synthesizes a request to persist the delete decision when none was saved", async () => {
    // A delete op whose persisted request was lost still records the decision:
    // it synthesizes a minimal request from the prompt metadata (clientId,
    // appDisplayName) and the resume body so the runner can act instead of
    // re-prompting on the same unanswered decision.
    const operation = actionRecord({
      inputRequired: {
        code: "delete-app-registration-decision",
        checkpoint: "delete-app-registration-decision",
        metadata: { clientId: "app-9", appDisplayName: "radius-deploy-octo" }
      }
    });
    Object.defineProperty(operation, "request", {
      configurable: true,
    ...
    expect(operation.request).toEqual({
      deleteAppRegistration: true,
      clientId: "app-9",
      appDisplayName: "radius-deploy-octo",
      repo: "octo/app",
      environment: "dev",
      provider: "azure"
    });
  ...
  it("resumes a delete decision with no metadata or identifiers to carry", async () => {
    // With neither a saved request, prompt metadata, nor resume-body identifiers
    // the synthesized request still records the decision alone.
    const operation = actionRecord({
      inputRequired: {
        code: "delete-app-registration-decision",
        checkpoint: "delete-app-registration-decision"
    ...
    const recording = await runAction(
      "/api/operations/op-action/resume/delete-app-registration-decision",
      JSON.stringify({ deleteAppRegistration: false }),
      handleResumeOperation,
      actionDependencies({
        getOperation: () => operation,
        canResumeInput: () => true,
    ...
    expect(recording.status).toBe(202);
    expect(operation.request).toEqual({ deleteAppRegistration: false });
  });
