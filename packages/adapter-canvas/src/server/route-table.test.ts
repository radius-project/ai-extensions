...
describe("server route ownership boundary", () => {
  ...
  // Four families are deliberately split, and each is named here so no later
  ...
  it("owns the liveness-source, repositories, identity-profile, and identity-auth families, the operations-status GETs, the azure-discovery reads and discover write, the graphs-planning reads, and every deployments route but POST /api/deploy, and leaves 12 routes on the legacy fallback", () => {
    ...
    expect(LEGACY_ROUTE_INVENTORY).toHaveLength(12);
  ...
  it("keeps the residual legacy dispatcher exactly equal to the inventory", () => {
    ...
    // Cross-checked against the inventory, and independently pinned: 12 of 38
    ...
    expect(residualLegacyCount).toBe(12);
