...
// Assembled here, then published at plugins/radius. It cannot be assembled in
// place because tracked plugin metadata already lives at that path.
...
// The complete workflow contract ships in the plugin root so a release contains
// the exact templates, actions and scripts it was built with.
...
function installToLocal() {
  try {
    ...
    if (existsSync(manifestFrom)) {
      ...
      const manifest = JSON.parse(readFileSync(manifestFrom, "utf8"));
      manifest.main = basename(installPath);
