...
async function assembleDist(bundleInputs) {
  ...
  copyExtensionAssets();
  writeCanvasCompatibilityEntryPoint();
  ...
  materializeExternalCanvasManifest(distManifest);
...
function writeCanvasCompatibilityEntryPoint() {
...
function materializeExternalCanvasManifest(manifestPath) {
  const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
  const logo = manifest.extensions?.["com.github.copilot"]?.logo;
  if (logo !== "assets/preview.png") {
    throw new Error(
      `${manifestPath} must declare extensions.com.github.copilot.logo as "assets/preview.png".`
    );
  }
  manifest.logo = logo;
