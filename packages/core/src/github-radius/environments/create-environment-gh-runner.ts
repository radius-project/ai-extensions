...
export { needsWorkflowScope } from "../deployments/workflow-credential-fallback.js";
...
export function createWorkflowScopeGhRunner(
  ...
  const { providerMutationRecord } = ports.operationDomain;
  const { executeRecoverableMutation } = createProviderMutationRecovery(
  ...
  const setEnvironmentVariable = async (
    ...
    const valueSha256 = ports.hashString(value);
    ...
    > => {
      ...
      return {
        state: "present",
        value: body.value,
        valueSha256: ports.hashString(body.value)
      };
    };
          ...
          const predecessorMatches =
            ...
            ((previousValue === null && current.state === "absent") ||
              (previousValue !== null &&
                ...
                current.valueSha256 === ports.hashString(previousValue)));
