// UI-independent environment-deletion coordinator.
//
...
import { stateRegistryForEnvironment } from "../../workflows/state.js";
import {
...
async function deleteGitHubEnvironmentStage(
  ...
  const { enterStage, addStep, setStageState, finish } = ports.operationDomain;
...
async function deleteEnvironmentCredentials(
  ...
  const { addStep, setStageState, finish } = ports.operationDomain;
  const stopCredentialCleanup = (
    operation: DeletionOperation,
    code: string,
    message: string
  ) =>
    stopCredentialCleanupWithDomain(
      operation,
      code,
      message,
      ports.operationDomain
...
function stopCredentialCleanupWithDomain(
  op: DeletionOperation,
  code: string,
  message: string,
  operationDomain: OperationDomain
): false {
  const { setStageState, finish } = operationDomain;
  setStageState(op, STAGE_DELETE_CREDENTIAL, "failed");
  finish(op, "failed_partial", {
    failure: {
      code,
