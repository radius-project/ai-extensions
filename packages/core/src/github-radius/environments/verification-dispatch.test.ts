...
describe("verification dispatch", () => {
  it("recognizes explicit registration rejection on stdout", () => {
    expect(
      isWorkflowRegistrationRejection(
        command({
          code: 1,
          stdout: "HTTP 422 Unexpected inputs provided: radius_operation"
        }),
        true
      )
    ).toBe(true);
  });
  it.each([new Error("disk unavailable"), "disk unavailable"])(
    "does not dispatch when saving intent fails: %s",
    async (error) => {
      const test = harness({ dispatches: [] });
      test.ports.persist = async () => {
        throw error;
      };
  ...
  it("reconciles a non-Error dispatch rejection without replaying it", async () => {
    const test = harness({ dispatches: [], lists: [[exactRun()]] });
    test.sendDispatch.mockRejectedValueOnce("connection closed");
  ...
  it.each([
    ...
    command({ code: "1", stdout: "HTTP 403 Forbidden" }),
    command({ stdout: "unreadable" })
  ])(
    "hands off when reconciliation cannot read the run list",
    async (listed) => {
      const test = harness({
        dispatches: [command({ code: 1, timedOut: true })],
        lists: [listed]
      });
      await expect(test.run()).resolves.toMatchObject({
        state: "manual_required",
        guidance: expect.stringMatching(/could not read|unreadable/)
      });
      expect(test.sendDispatch).toHaveBeenCalledOnce();
      expect(test.sleeps).toEqual([]);
      expect(test.op.providerRecovery.mutations[0].status).toBe(
        "manual_required"
      );
    }
  );
  ...
  it.each([
    { runs: [exactRun(42), exactRun(43)] },
    {
      runs: [
        {
          ...exactRun(),
          displayTitle: "Radius verify another [different-operation]"
        }
      ]
    }
  ])("never adopts competing or unmarked new runs", async ({ runs }) => {
    const test = harness({
      dispatches: [command({ code: 1, timedOut: true })],
      lists: [runs]
    });
    await expect(test.run()).resolves.toMatchObject({
      state: "manual_required",
      guidance: expect.stringContaining("could not identify exactly one")
    });
    expect(test.op.verification?.runId).toBeNull();
    expect(test.sendDispatch).toHaveBeenCalledOnce();
    expect(test.sleeps).toEqual([]);
  });

  it.each(["[null]", '["run"]'])(
    "rejects malformed baseline entries: %s",
    (json) => {
      expect(() => parseVerificationBaselineRunId(json)).toThrow(
        "without a valid databaseId"
      );
    }
  );

  it("chooses the maximum baseline id even when the newest run is first", () => {
    expect(
      parseVerificationBaselineRunId('[{"databaseId":9},{"databaseId":2}]')
    ).toBe(9);
  });

  it.each([null, false, "operation"])(
    "rejects non-record workflow provenance: %s",
    (value) => {
      expect(
        hasFreshWorkflowWriteProvenance(value, WORKFLOW_PATH, "main")
      ).toBe(false);
    }
  );

  it("does not mistake an empty rejected response for workflow registration lag", () => {
    expect(isWorkflowRegistrationRejection(command({ code: 1 }), true)).toBe(
      false
    );
  });
  ...
  it.each([
    ["{}", "GitHub returned a non-array workflow run list."],
    ["[{}]", "GitHub returned a workflow run without a valid databaseId."]
  ])(
    "retains the baseline parser's exact rejection for %s",
    async (stdout, detail) => {
      const test = harness({ dispatches: [] });
      test.runGh.mockResolvedValueOnce(command({ stdout }));
      ...
      expect(test.sendDispatch).not.toHaveBeenCalled();
      expect(test.op.providerRecovery.mutations).toEqual([]);
    }
  );
  ...
  it("persists the URL parser's original diagnostic before reconciling malformed success", async () => {
    const test = harness({
      dispatches: [command({ stdout: "not-a-url" })],
      lists: [[exactRun()]]
    });
    const diagnostics: Array<string | null | undefined> = [];
    const persist = test.ports.persist;
    test.ports.persist = async () => {
      diagnostics.push(test.op.providerRecovery.mutations[0]?.finalDiagnostic);
      await persist();
    };
    ...
    expect(diagnostics).toContain(
      "GitHub CLI exited with 0: not-a-url; GitHub CLI returned a malformed workflow run URL."
    );
    expect(test.sendDispatch).toHaveBeenCalledOnce();
  });
