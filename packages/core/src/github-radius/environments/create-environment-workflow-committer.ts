...
import { parseBranchDeleteTarget } from "./recovered-branch-delete.js";
import {
...
export interface WorkflowContentIdentityPort {
  workflowContentDigest(contentB64: string): string;
  workflowBlobSha(contentB64: string): string;
}
...
export function recordedSetupBranchCreate(
  ...
  for (const mutation of operationDomain.providerMutationsByKind(
    operation,
...
export function createWorkflowFileCommitter(
  ...
  { workflowContentDigest, workflowBlobSha }: WorkflowContentIdentityPort
): WorkflowFileCommitter {
  const { providerMutationRecord, providerMutationsByKind } =
    ports.operationDomain;
  const { executeRecoverableMutation, providerMutationWillWrite } =
    createProviderMutationRecovery(ports.operationDomain);
  ...
  const beginPrFallback = async (): Promise<PullRequestBranchState> => {
    ...
    if (ports.mutationRecovery) {
      ...
      if (
        ...
        // No cancellation gate is supplied. A rejected delete throws through
        // rejectionGuidance; reconciliation either proves removal or throws.
        // Consequently this await completes only after a confirmed removal.
