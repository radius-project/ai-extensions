...
function requireRecoveryRollback(
  ...
  const { unresolvedProviderMutations, requestStop } = operations;
  if (
...
async function confirmProviderMutation<T>({
  ...
  const { settleProviderMutation } = operationDomain;
  try {
  ...
  if (recovered) requireRecoveryRollback(operation, mutation, operationDomain);
...
async function reconcileMutation<T>(
  ...
  requireRecoveryRollback(operation, mutation, operationDomain);
...
export function providerMutationWillWrite(
  ...
  const journaled = operationDomain.providerMutationRecord(
    operation,
    kind,
    target
  );
...
export async function executeRecoverableMutation<T>(
  ...
  // Missing and not-applied entries take the forward path above; only manual
  // quarantine remains. An unexpected journal state must also fail closed,
  // never masquerade as proof that the provider did not apply the request.
  throw new ProviderMutationRecoveryError(
    mutation.evidence ||
...
export function createProviderMutationRecovery(
  operationDomain: OperationDomain
) {
  function execute<T>(
    input: Omit<RecoverableMutationInput<T>, "operationDomain"> & {
  ...
  function execute<T>(
    ...
    return executeRecoverableMutation({ ...input, operationDomain });
  }
  return {
    executeRecoverableMutation: execute,
    providerMutationWillWrite: (
      operation: object & { operationId: string },
      kind: string,
      target: string
    ) => providerMutationWillWrite(operation, kind, target, operationDomain),
