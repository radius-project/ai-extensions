...
describe("describeWorkflowCommitFailure", () => {
  it("explains missing workflow scope without inventing an unavailable CLI command", () => {
    const failure = describeWorkflowCommitFailure(
      ...
      "missing workflow scope",
      {
        kind: "unavailable",
        shell: "posix",
        installationNote: "Install the approved CLI."
      }
    );
    expect(failure.error).toContain("Install the approved CLI.");
    expect(failure.error).not.toContain("Run `");
  });
...
describe("publishWorkflowFiles", () => {
  ...
  ])("stops immediately when the committer cancels %s", async (path) => {
    const recorder = publisherRecorder({
      commits: {
        [path]: { ok: false, changed: false, cancelled: true, viaPr: false }
      }
    });
    expect(await publishWorkflowFiles(recorder.ports, recorder.target)).toEqual(
      { outcome: "cancelled" }
    );
    expect(recorder.commitCalls.at(-1)).toBe(path);
    expect(recorder.committed.map((entry) => entry.path)).not.toContain(path);
  });

  it("honors Stop after a refused deploy write without proceeding to deletion", async () => {
    const recorder = publisherRecorder({
      gateFalseOnCall: 2,
      commits: {
        ".github/workflows/run-rad-commands.yml": {
  ...
  it.each([true, false])(
    "preserves legacy-delete failure unless Stop wins: %s",
    async (stopped) => {
      const recorder = publisherRecorder({
        gateFalseOnCall: stopped ? 3 : undefined
      });
      const error = new Error("legacy delete outcome is unknown");
      recorder.ports.deleteLegacyDeployWorkflow = async () => {
        throw error;
      };
      const result = publishWorkflowFiles(recorder.ports, recorder.target);
      if (stopped)
  ...
  it("propagates delete workflow recovery blockers rather than treating them as optional warnings", async () => {
    const recorder = publisherRecorder();
    const error = new ProviderMutationRecoveryError(
      "manual ownership proof required",
      "provider-mutation-manual-required"
    );
    recorder.ports.generateDeleteWorkflow = async () => {
      throw error;
    };
    await expect(
    ...
    expect(
      recorder.steps.some((step) => step.includes("Could not generate/commit"))
    ).toBe(false);
  });
