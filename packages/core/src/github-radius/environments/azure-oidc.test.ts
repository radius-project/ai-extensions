...
describe("fetchGitHubJson retry", () => {
  ...
  describe("GitHub OIDC retry boundaries", () => {
    it("uses the production timer for bounded retry delays", async () => {
      vi.useFakeTimers();
      try {
        const failure: GitHubJsonResponse = { ok: false, status: 503 };
        const success: GitHubJsonResponse = { ok: true, status: 200, json: {} };
        const runner = vi
          .fn()
          .mockResolvedValueOnce(failure)
          .mockResolvedValueOnce(success);
        const result = fetchGitHubJson(runner, "/repos/octo/app", {
          baseDelayMs: 50
        });
    ...
    it.each([
      ...
      { header: "Fri, 18 Sep 2026 00:00:02 GMT", expectedDelay: 2000 },
      { header: "Thu, 17 Sep 2026 23:59:59 GMT", expectedDelay: 0 },
      { header: "invalid-date", expectedDelay: 300 },
      { header: "0", expectedDelay: 0 }
    ])(
      "honors Retry-After $header without exceeding the retry budget",
      async ({ header, expectedDelay }) => {
        ...
        const runner = vi
          ...
          .mockResolvedValueOnce({
            ok: false,
            status: 429,
            stderr: `Retry-After: ${header}`
          })
          .mockResolvedValueOnce(REPO_OK);
        await expect(
          fetchGitHubJson(runner, "/repos/octo/app", {
            now: () => Date.parse("2026-09-18T00:00:00Z"),
            sleepFn
          })
        ).resolves.toBe(REPO_OK);
        expect(sleepFn).toHaveBeenCalledExactlyOnceWith(expectedDelay);
        expect(runner).toHaveBeenCalledTimes(2);
      }
    );

    it("does not issue a read when no attempts were allowed", async () => {
      const runner = vi.fn(async () => REPO_OK);
      expect(
        await fetchGitHubJson(runner, "/repos/octo/app", { retries: 0 })
      ).toBeUndefined();
      expect(runner).not.toHaveBeenCalled();
    });

    it("does not retry a status-free failure with no transport diagnostic", async () => {
      const response: GitHubJsonResponse = { ok: false };
      const runner = vi.fn(async () => response);
      expect(await fetchGitHubJson(runner, "/repos/octo/app")).toBe(response);
      expect(runner).toHaveBeenCalledTimes(1);
    });

    it.each([
      {
        response: { ok: false, stderr: "Authentication unavailable" },
        detail: "Authentication unavailable"
      },
      { response: { ok: false }, detail: "request failed" }
    ])(
      "preserves actionable status-free access errors ($detail)",
      async ({ response, detail }) => {
        await expect(
          resolveOidcSubject(
            {
              targetRepo: "octo/app",
    ...
    it("reports an exhausted attempt policy as failed access, never guessed identity", async () => {
      const runner = vi.fn(async () => REPO_OK);
      await expect(
        resolveOidcSubject(
          {
    ...
    it.each([null, undefined])(
      "rejects a successful repository read with absent JSON (%s)",
      async (json) => {
        await expect(
          resolveOidcSubject(
            {
        ...
        ).rejects.toMatchObject({ code: "repo-metadata" });
      }
    );

    it.each([null, undefined])(
      "rejects a successful customization read with absent JSON (%s)",
      async (json) => {
        ...
        ).rejects.toMatchObject({ code: "customization-malformed" });
      }
    );
  });

  describe("OIDC parser and selection edge contracts", () => {
    it("accepts empty iterable tag and subject inputs", () => {
      expect(parseRadiusAppProvenanceTags("")).toEqual({ managed: false });
      expect(parseServedReposFromSubjects("")).toEqual([]);
      expect(
        selectMissingFederatedCredentials(
          [{ name: "dev", subject: "repo:octo/app:environment:dev" }],
          ""
        )
      ).toEqual([{ name: "dev", subject: "repo:octo/app:environment:dev" }]);
    });

    it("does not classify missing owner-add diagnostics as an existing assignment", () => {
      expect(isAppOwnerAlreadyAssignedError(undefined)).toBe(false);
      expect(isAppOwnerAlreadyAssignedError("")).toBe(false);
    });

    it("does not synthesize a subject while parsing a partial Azure credential", () => {
      expect(
        parseFederatedCredentials([{ id: "credential", name: "dev" }])
      ).toEqual([
        {
          id: "credential",
          name: "dev",
      ...
      expect(
        federatedCredentialListUnreadable([{ id: "credential", name: "dev" }])
      ).toBe(true);
    });
    ...
    it.each(
      [
        [
          { appId: "missing-date" },
          { appId: "dated", createdDateTime: "2026-01-01" }
        ],
        [
          { appId: "dated", createdDateTime: "2026-01-01" },
          { appId: "missing-date" }
        ...
        [
          { appId: "invalid-date", createdDateTime: "not-a-date" },
          { appId: "dated", createdDateTime: "2026-01-01" }
        ]
      ].map((ownedMatches) => ({ ownedMatches }))
    )(
      "prefers known creation dates over missing or unreadable dates: $ownedMatches",
      ({ ownedMatches }) => {
        expect(decideAppSelection({ ownedMatches })).toMatchObject({
    ...
    it("keeps only valid unique tags from untrusted JSON", () => {
      expect(
        parseAppTags(
          JSON.stringify([" managed ", 7, null, "", "managed", "custom"])
        )
      ).toEqual(["managed", "custom"]);
      expect(parseAppTags(undefined)).toBeNull();
      expect(parseAppTags("{}")).toBeNull();
      expect(parseDirectoryObjectIds(undefined)).toEqual([]);
      expect(buildRadiusAppProvenanceTags({})).toEqual(["radius-managed"]);
      expect(parseRadiusAppProvenanceTags(["unrelated"])).toEqual({
        managed: false
      });
    });

    it.each([null, 7, []].map((entry) => ({ entry })))(
      "refuses an unreadable credential entry $entry",
      ({ entry }) => {
        expect(federatedCredentialListUnreadable([entry])).toBe(true);
      }
    );

    it("does not infer served repositories from subjects with a missing owner or repository", () => {
      expect(
        parseServedReposFromSubjects([
          "repo:/app:environment:dev",
          "repo:octo/:environment:dev",
          "repo:octo/app:environment:dev"
        ])
      ).toEqual(["octo/app"]);
    });

    it("falls back to the oldest owned app if a wired client is not among the candidates", () => {
      const decision = decideAppSelection({
        ownedMatches: [
          { appId: "new", createdDateTime: "2026-01-01" },
          { appId: "old", createdDateTime: "2025-01-01" }
        ],
    ...
    it("renders an empty AWS discovery without inventing failures", () => {
      expect(discoverStatusText({}, "aws")).toBe(
        "Found 0 cluster(s), 0 VPC(s)"
      );
    });
  });
