...
import {
  ...
  ensureGitHubEnvironment as ensureWithDomain,
...
function ensureGitHubEnvironment(
  input: Omit<Parameters<typeof ensureWithDomain>[0], "operationDomain">
) {
  return ensureWithDomain({ ...input, operationDomain });
}
...
describe("ensureGitHubEnvironment", () => {
  it("reconciles an existing environment after restart without reading the repository or replaying PUT", async () => {
    const operation = createOperation({ operationId: "op-restart" });
    prepareProviderMutation(operation, {
      kind: "github_environment.put",
      target: "octo/app:production"
    });
    const readGitHubJson = vi.fn(async () =>
      readResult({ json: { name: "Production", id: 7 } })
    );
    ...
    await expect(
      ...
      message: expect.stringContaining("never recorded an id")
    });
    expect(readGitHubJson.mock.calls).toHaveLength(2);
    expect(readGitHubJson).toHaveBeenNthCalledWith(
      1,
  ...
  it.each([
    ...
    ["invalid", null],
    [{ id: " 123 " }, "123"],
    [{ id: "", node_id: " node-123 " }, "node-123"],
    [{ node_id: "" }, null],
    [{ node_id: 123 }, null]
  ])("reads only a usable immutable provider id from %j", (body, id) => {
    expect(parseEnvironmentProviderId(body)).toBe(id);
  });

  it.each(["not JSON", "null"])(
    "retains an acknowledged create with malformed body %s",
    async (stdout) => {
      const operation = createOperation({ operationId: "op-malformed" });
      await expect(
        ensureGitHubEnvironment({
          repo: "octo/app",
          requestedName: "production",
          readGitHubJson: async (path) =>
            readResult({ ok: path === "/repos/octo/app", status: 404 }),
  ...
  it.each(["not_applied", "manual_required"] as const)(
    "does not replay a %s PUT when the environment now exists",
    async (status) => {
      const operation = createOperation({ operationId: "op-existing" });
      const mutation = prepareProviderMutation(operation, {
        kind: "github_environment.put",
        target: "octo/app:production"
      });
      settleProviderMutation(operation, mutation.mutationId, status, null);
      const runGh = vi.fn(async () => result());
      const pending = ensureGitHubEnvironment({
        repo: "octo/app",
        requestedName: "production",
      ...
      if (status === "not_applied") {
        ...
        await expect(pending).rejects.toMatchObject({
          code: "provider-mutation-manual-required",
          message: expect.stringContaining("cannot prove who created")
        });
      }
      expect(runGh).not.toHaveBeenCalled();
    }
  ...
  it("preserves recorded manual guidance instead of retrying the PUT", async () => {
    const operation = createOperation({ operationId: "op-guidance" });
    const mutation = prepareProviderMutation(operation, {
      kind: "github_environment.put",
      target: "octo/app:production"
    });
    ...
    ).rejects.toThrow("Inspect the existing environment.");
  });

  it.each(["", "state unavailable"])(
    "keeps an unreadable post-PUT state unresolved: %s",
    async (stderr) => {
      const operation = createOperation({ operationId: "op-unreadable" });
      ...
      await expect(
        ensureGitHubEnvironment({
          ...
          requestedName: "production",
          readGitHubJson: async (path) =>
            path === "/repos/octo/app" ? readResult() : (
              readResult({
                ok: false,
                status: ++environmentReads === 1 ? 404 : 403,
                stderr
              })
            ),
          runGh: async () => result({ timedOut: true }),
          mutationRecovery: { operation, persist: async () => {} }
        })
      ...
      expect(operation.providerRecovery.mutations[0]).toMatchObject({
        ...
        evidence: expect.stringContaining(
          stderr || "GitHub environment state could not be read."
        )
      });
    }
  );

  it("hands off a post-PUT environment without a canonical identity", async () => {
    const operation = createOperation({ operationId: "op-no-name" });
    let environmentReads = 0;
    await expect(
      ensureGitHubEnvironment({
        repo: "octo/app",
        requestedName: "production",
        readGitHubJson: async (path) =>
          path === "/repos/octo/app" || ++environmentReads > 1 ?
            readResult()
      ...
      code: "provider-mutation-manual-required",
      message: expect.stringContaining("not its canonical identity")
    });
  });

  it("honors Stop arriving during intent persistence", async () => {
    const operation = createOperation({ operationId: "op-stop" });
    const runGh = vi.fn(async () => result());
    let stopChecks = 0;
    await expect(
      ensureGitHubEnvironment({
        ...
        readGitHubJson: async (path) =>
          readResult({ ok: path === "/repos/octo/app", status: 404 }),
        beforeCreate: async () => ++stopChecks === 1,
        runGh,
        mutationRecovery: {
          operation,
          persist: async () => {
    ...
    expect(runGh).not.toHaveBeenCalled();
    expect(stopChecks).toBe(2);
    expect(operation.providerRecovery.mutations[0].status).toBe("not_applied");
  });

  it("surfaces a conclusive journaled PUT rejection with its diagnostic", async () => {
    const operation = createOperation({ operationId: "op-rejected" });
    await expect(
      ensureGitHubEnvironment({
        repo: "octo/app",
        requestedName: "production",
        readGitHubJson: async (path) =>
          readResult({ ok: path === "/repos/octo/app", status: 404 }),
        runGh: async () => result({ code: 1, stderr: "HTTP 403 forbidden" }),
        mutationRecovery: { operation, persist: async () => {} }
      })
    ).rejects.toMatchObject({
  ...
  it("names an inaccessible repository when its lookup has no diagnostic", async () => {
    await expect(
      ensureGitHubEnvironment({
        repo: "octo/app",
        requestedName: "production",
        readGitHubJson: async () => readResult({ ok: false, status: 404 }),
        runGh: async () => {
          throw new Error("must not write");
        }
      })
      ...
      message: expect.stringContaining("repository is missing or inaccessible")
    });
  });

  it("reports a failed unjournaled PUT even without a diagnostic", async () => {
    await expect(
      ensureGitHubEnvironment({
        repo: "octo/app",
        requestedName: "production",
        readGitHubJson: async (path) =>
          readResult({ ok: path === "/repos/octo/app", status: 404 }),
        runGh: async () => result({ code: 1 })
      })
    ).rejects.toMatchObject({
      code: "github-environment-create-failed",
      message: expect.stringContaining("The GitHub API request failed.")
    });
  });

  it("uses the built-in retry clock for a malformed Retry-After value", async () => {
    vi.useFakeTimers();
    try {
      const readGitHubJson = vi
        .fn()
        .mockResolvedValueOnce(
          readResult({ ok: false, status: 503, stderr: "Retry-After: invalid" })
        )
        .mockResolvedValueOnce(readResult({ json: { name: "production" } }));
      const pending = ensureGitHubEnvironment({
        repo: "octo/app",
        requestedName: "production",
        readGitHubJson,
      ...
      expect(readGitHubJson).toHaveBeenCalledTimes(1);
      await vi.advanceTimersByTimeAsync(1);
      await expect(pending).resolves.toMatchObject({ state: "reused" });
      expect(readGitHubJson).toHaveBeenCalledTimes(2);
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  describe("readEnsuredGitHubEnvironment", () => {
    ...
    it.each(["reused", "invalid-state"])(
      "validates the saved artifact state %s",
      (state) => {
        const record = {
          ...resolved,
          setupArtifacts: {
            githubEnvironment: {
              ...resolved.setupArtifacts.githubEnvironment,
              state,
              providerId: "7"
            }
          }
        ...
        expect(
          readEnsuredGitHubEnvironment(record, "octo/app", "Production")
        ).toEqual(
          state === "reused" ?
            { name: "Production", state, providerId: "7" }
          : null
        );
      }
    );

    it("does not reuse an empty saved provider id as identity evidence", () => {
      const record = {
        ...resolved,
        setupArtifacts: {
          githubEnvironment: {
