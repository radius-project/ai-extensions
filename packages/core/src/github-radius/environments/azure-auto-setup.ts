import { buildEnvironmentSuffix } from "../../platforms/oidc-subject.js";
...
export function validateAzureAutoSetupDependencies(
...
function respond(
  context: EnvironmentSetupCompletion,
  status: number,
...
export interface AzureAutoSetupRequest {
  repo?: string;
  environment?: string;
...
export async function runAzureAutoSetup(
  data: AzureAutoSetupRequest,
  dependencies: AzureAutoSetupDependencies
): Promise<EnvironmentSetupResult> {
  let result: EnvironmentSetupResult | undefined;
  await executeAzureAutoSetup(data, dependencies, {
    complete(status, body) {
      result = environmentSetupResult(status, body);
    }
  });
  if (!result) throw new Error("Azure setup completed without an outcome.");
  return result;
}

async function executeAzureAutoSetup(
  data: AzureAutoSetupRequest,
  dependencies: AzureAutoSetupDependencies,
  context: EnvironmentSetupCompletion
): Promise<void> {
  ...
  try {
    ...
    const requestedAppName = data.appName ?? "";
