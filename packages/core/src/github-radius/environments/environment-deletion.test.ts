...
function makeOp(
  ...
  const op = createOperation({
    ...
    stages: [
      STAGE_DELETE_RADIUS_ENV,
      ...(includeAzureCleanup ? [STAGE_DELETE_CREDENTIAL] : []),
      STAGE_DELETE_GITHUB_ENV,
      STAGE_DELETE_STATE_PACKAGE,
      ...(includeAzureCleanup ? [STAGE_REVIEW_APP_REGISTRATION] : [])
...
describe("runEnvironmentDeletion — azure happy path", () => {
  it("does not repeat any cleanup after all stages completed", async () => {
    const op = makeOp();
    const ports = makePorts();
    await runEnvironmentDeletion(op, ports);
    const steps = structuredClone(op.steps);
    await runEnvironmentDeletion(op, ports);
    expect(op.state).toBe("succeeded");
    expect(op.steps).toEqual(steps);
    expect(ports.deleteRadiusEnvironment).toHaveBeenCalledOnce();
    expect(ports.deleteGitHubEnvironment).toHaveBeenCalledOnce();
    expect(ports.deleteStatePackage).toHaveBeenCalledOnce();
    expect(ports.runAz).toHaveBeenCalledOnce();
  });

  it.each(["clientId", "tenantId"])(
    "keeps GitHub state when an older environment has no %s",
    async (key) => {
      const op = makeOp({ request: { [key]: "" } });
      const ports = makePorts();
      await runEnvironmentDeletion(op, ports);
      expect(op.failure?.code).toBe("federated-credential-app-unknown");
      expect(ports.readAzureIdentity).not.toHaveBeenCalled();
...
describe("runEnvironmentDeletion — GHCR state package", () => {
  ...
  it("keeps prior teardown complete and makes package failure retryable", async () => {
    ...
    op.failure = null;
    op.terminal = null;
    for (const current of op.stages) {
...
describe("runEnvironmentDeletion — best-effort warnings", () => {
  it.each([true, false])(
    "fails closed before cleanup on a Radius delete failure with logging=%s",
    async (logging) => {
      const op = makeOp({ provider: "aws", includeAzureCleanup: false });
      const ports = makePorts({
        deleteRadiusEnvironment: vi.fn(async () => ({
...
describe("runEnvironmentDeletion — credential provenance gate (#331)", () => {
  it.each(["not JSON", "{}", ""])(
    "retains credentials when immediate revalidation is unreadable: %s",
    async (stdout) => {
      const op = makeOp();
      const ports = makePorts({
        readCredentialProvenance: () => createdProvenance()
      });
      ports.runAz = vi.fn(async (args) => {
        ...
        if (args.includes("show")) return { code: 0, stdout };
        throw new Error("unverified credential must not be deleted");
      });
      await runEnvironmentDeletion(op, ports);
  ...
  it("fails closed when active Azure identity cannot be read", async () => {
    const op = makeOp();
    const ports = makePorts({
      readAzureIdentity: async () => {
        throw new Error("Azure session expired");
      }
    });
    await runEnvironmentDeletion(op, ports);
    expect(op.failure?.code).toBe("federated-credential-identity-unavailable");
    expect(op.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          warning: expect.objectContaining({ message: "Azure session expired" })
        })
      ])
    );
    expect(ports.runAz).not.toHaveBeenCalled();
  ...
  it("retains a custom-subject credential that does not prove exclusive ownership", async () => {
    const op = makeOp();
    const ports = makePorts({
      runAz: async () =>
        ok(
      ...
      expect.arrayContaining([
        expect.objectContaining({
          warning: expect.objectContaining({
            code: "federated-credential-retained-custom-subject",
            message: expect.stringContaining(
              "historical configuration does not prove"
            )
          })
        })
      ])
  ...
  it("warns when consumer provenance cannot be retired after deleting GitHub state", async () => {
    const op = makeOp();
    const ports = makePorts({
  ...
  it("passes a heartbeat that refreshes activity and persists while the run is polled", async () => {
    ...
    const deleteRadiusEnvironment = vi.fn(
      async (_input, onHeartbeat?: () => void | Promise<void>) => {
        ...
        op.lastActivityAt = "2026-08-21T00:00:00.000Z";
