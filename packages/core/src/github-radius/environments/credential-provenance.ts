import type { OidcSubjectConfig } from "../../platforms/index.js";
...
export function createCredentialProvenanceRegistry(
  now: () => string
): CredentialProvenanceRegistry {
  ...
  return {
    ...
    async record(input) {
      const candidate = buildRecord(input, now);
