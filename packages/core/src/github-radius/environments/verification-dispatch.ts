...
export async function runVerificationDispatch(
  ...
  try {
    ...
    return {
      state: "baseline_failed",
      // This local parser only throws JSON syntax errors or explicit Errors.
  ...
  for (let attempt = 1; ; attempt += 1) {
    ...
    if (result.code === 0 || result.code === "0") {
      ...
      try {
        ...
        // The URL parser throws Errors; no external callback runs in this try.
        const malformed = `${diagnostic}; ${(error as Error).message}`;
        recordProviderMutationDiagnostics(operation, mutation.mutationId, {
          final: malformed
        });
        settleProviderMutation(
