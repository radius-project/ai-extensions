...
import {
  ...
  pendingBranchDelete as pendingWithDomain,
  reconcileRecoveredBranchDelete as reconcileWithDomain,
  REF_PAGE_SIZE,
  type BranchRefReadResult,
  type RecoveredBranchDeleteOutcome
} from "./recovered-branch-delete.js";

const pendingBranchDelete = (
  operation: Parameters<typeof pendingWithDomain>[0]
) => pendingWithDomain(operation, operationDomain);
const reconcileRecoveredBranchDelete = (
  input: Omit<Parameters<typeof reconcileWithDomain>[0], "operationDomain">
) => reconcileWithDomain({ ...input, operationDomain });
...
describe("settling a recovered setup-branch delete", () => {
  ...
  it("refuses to delete a branch that has moved on", async () => {
    ...
    const outcome = await reconcile({
      ...
      readBranchRef: async () =>
        refRead({
          stdout: JSON.stringify({ object: { sha: "someone-else" } })
        })
    });
