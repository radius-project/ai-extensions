...
import { createProviderMutationRecovery } from "./provider-mutation-recovery.js";
...
async function readAzureWithPropagationRetry(
  ...
  retrySuccessfulRead?: (result: AzureAutoSetupCommandResult) => boolean
  ...
  for (let attempt = 0; attempt < AZURE_PROPAGATION_ATTEMPTS; attempt++) {
    ...
    if (
      !(
        (succeeded && retrySuccessfulRead?.(last)) ||
...
async function createFederatedCredentials({
  ...
  | "appName"
>): Promise<boolean> {
  const { providerMutationRecord, unresolvedProviderMutations } =
...
async function resolveServicePrincipalObjectId(
  ...
  const result = await readAzureWithPropagationRetry(
    ...
    (candidate) => candidate.stdout.trim() === ""
  );
...
async function assignRole(
  input: RoleAssignmentInput & { assignmentId: string },
...
> {
  ...
  const { providerMutationRecord } = operationDomain;
  let last: AzureAutoSetupCommandResult = {
    code: 1,
    stdout: "",
    stderr: ""
  ...
  const mutationTarget = input.assignmentId;
  for (let attempt = 0; attempt < AZURE_PROPAGATION_ATTEMPTS; attempt++) {
    ...
    const mutation =
      await executeRecoverableMutation<AzureAutoSetupCommandResult>({
        ...
        providerIdempotencyKey: input.assignmentId,
        persist,
        beforeMutation: () =>
          stopBoundary(
            `before-role-assignment:${input.role}:attempt-${attemptNumber}`
...
export async function configureAzureAutoSetupCredentials({
  ...
  const contributorAssignmentId = dependencies.deterministicProviderUuid(
  ...
  const clusterAssignmentId = dependencies.deterministicProviderUuid(
  ...
  const locksContributorAssignmentId = dependencies.deterministicProviderUuid(
