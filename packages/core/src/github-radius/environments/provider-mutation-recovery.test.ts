...
import {
  ...
  createProviderMutationRecovery,
...
describe("provider mutation recovery", () => {
  it("reconciles a thrown mutation response and persists before adopting its identity", async () => {
    const operation = createOperation({ operationId: "op-lost-response" });
    const events: string[] = [];
    ...
    await expect(
      executeRecoverableMutation({
        ...
        mutate,
        accept: () => {
          throw new Error("there was no response to accept");
        },
        reconcile: async () => {
          ...
          return {
            ...
            value: "dev",
            providerId: "7",
            evidence: "Exact provider identity matched."
          };
        }
      })
    ).resolves.toEqual({ state: "applied", value: "dev", recovered: true });
    expect(events).toEqual([
      "persist:prepared",
      "persist:outcome_unknown",
      "reconcile",
      "persist:confirmed"
    ]);
  ...
  it.each([
    ["Conflict", "HTTP request failed for CONFLICT"],
    ["CONFLICT", "HTTP request failed for conflict"],
    ["ab/Conflict", "HTTP request failed for CONFLICT"],
    ["ab/\ud800Conflict", "HTTP request failed for \ud800Conflict"]
  ])(
    "never takes the resource name %s as a provider rejection",
    (target, stderr) => {
      expect(
        classifyProviderMutationFailure(command({ code: 1, stderr }), target)
      ).toBe("outcome_unknown");
    }
  );

  it("classifies a timeout before interpreting an explicit provider rejection", () => {
    expect(
      classifyProviderMutationFailure(
        command({
          code: 1,
  ...
  it("records failed independent reconciliation without a callback", async () => {
    const operation = createOperation({ operationId: "op-independent" });
    ...
    expect(operation.providerRecovery.mutations[0]).toMatchObject({
      ...
      evidence: "Provider state could not be read: provider unavailable"
    });
    expect(persist).toHaveBeenCalledOnce();
  });

  it.each([new Error("disk full"), "disk full"])(
    "stops before mutation when persistence rejects: %s",
    async (error) => {
      const operation = createOperation({ operationId: "op-persist" });
  ...
  it("fails closed when a Stop boundary reports unfinished reconciliation", async () => {
    const operation = createOperation({ operationId: "op-stop-unresolved" });
    const mutate = vi.fn(async () => command());
    await expect(
      executeRecoverableMutation({
        operation,
  ...
  it("retains a non-Error identity validation rejection without writing", async () => {
    const operation = createOperation({ operationId: "op-validation-string" });
    const mutate = vi.fn(async () => command());
    await expect(
      executeRecoverableMutation({
        operation,
  ...
  it("hands off when acknowledged provenance recording rejects without an Error", async () => {
    const operation = createOperation({ operationId: "op-provenance-string" });
    await expect(
      executeRecoverableMutation({
        operation,
        kind: "github_environment.put",
      ...
      message: expect.stringContaining("ledger unavailable")
    });
  ...
  it("records a conclusively absent reconciliation without inventing evidence", async () => {
    const operation = createOperation({ operationId: "op-absent" });
    const mutation = prepareProviderMutation(operation, {
      kind: "github_environment.put",
      target: "octo/app:dev"
    });
    ...
    ).resolves.toEqual({ state: "not_applied" });
    expect(operation.providerRecovery.mutations[0]).toMatchObject({
      status: "not_applied",
      evidence: null
    });
  });
  it("exposes bounded failed-read settlement to independent reconciliation callers", async () => {
    const operation = createOperation({
      operationId: "op-independent-reconciliation"
    });
    const mutation = prepareProviderMutation(operation, {
      kind: "github_environment.put",
      target: "octo/app:dev"
