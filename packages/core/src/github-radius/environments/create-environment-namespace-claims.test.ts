...
describe("the pinned-account claims ports", () => {
  it.each([
    [{ code: 1, stdout: "denied", stderr: "" }, "denied"],
    [
      { code: 1, stdout: "", stderr: "" },
      'the variables for environment "dev" could not be read.'
    ],
    [
      { code: 1, stdout: " ", stderr: "" },
      'the variables for environment "dev" could not be read.'
    ...
    "preserves failure diagnostics or names the unreadable environment",
    async (response, reason) => {
      const ports = createSelectedNamespaceClaimsPorts({
        run: async () => response
      });
      await expect(
        ports.readEnvironmentVariables("octo/app", "dev")
      ).resolves.toEqual({
        ok: false,
        reason
      });
  ...
  it("treats an empty successful list as no environments", async () => {
    const ports = createSelectedNamespaceClaimsPorts({
      run: async () => ({ code: "0", stdout: "", stderr: "" })
    });
    await expect(ports.listEnvironmentNames("octo/app")).resolves.toEqual({
      ok: true,
      names: []
    });
  });

  it("fails closed when the selected executor rejects without an Error", async () => {
    const ports = createSelectedNamespaceClaimsPorts({
      run: async () => {
...
describe("establishing the repository's namespace claims", () => {
  ...
  it("reports the first failure in name order, not in completion order", async () => {
    ...
    const ports: NamespaceClaimsPorts = {
      ...
      readEnvironmentVariables: async (_repo, environment) => {
        if (environment === "beta") {
          completions.push("beta");
          finishBeta();
          return { ok: false, reason: "beta failed first" };
        }
        await betaFinished;
        completions.push("alpha");
