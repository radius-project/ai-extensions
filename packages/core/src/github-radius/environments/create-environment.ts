...
import type {
  ...
  WorkflowScopeGhRunnerTarget,
  WorkflowFileCommitter,
  WorkflowFileCommitterPorts,
  WorkflowFileCommitterTarget,
  WorkflowTempFilePort
} from "./workflow-execution-ports.js";
...
import { selectedEnvironmentReader } from "./github-environment.js";
...
// Checkpoints retain their original order: each persists operation identity
// before checking cancellation. A persistence failure closes the operation
// before any further provider mutation.

export interface CreateEnvironmentDependencies extends AdmissionPorts {
  operationDomain: OperationDomain;
  ghCommandPresentation?: GhCommandPresentation;
  legacyDeployWorkflowFile: string;
...
export async function runCreateEnvironment(
  data: CreateEnvironmentRequestData,
  dependencies: CreateEnvironmentDependencies
): Promise<EnvironmentSetupResult> {
  let result: EnvironmentSetupResult | undefined;
  await executeCreateEnvironment(data, dependencies, (status, body) => {
    result = environmentSetupResult(status, body);
  });
  if (!result)
    throw new Error("Environment setup completed without an outcome.");
  return result;
}
...
async function executeCreateEnvironment(
  ...
  const { executeRecoverableMutation, providerMutationWillWrite } =
    createProviderMutationRecovery(dependencies.operationDomain);
  ...
  try {
    ...
    const runner = dependencies.createWorkflowScopeGhRunner(
      {
        targetRepo,
        envName,
    ...
    const fail = async (
      ...
      const failure = await dependencies.finalizeSetupFailure(operation, {
        ...
        extra,
        steps,
        evidence: typeof extra.ghError === "string" ? extra.ghError : null,
        runAz:
          provider === "azure" ?
            (args: string[]) => dependencies.runAzCommand(args)
          : null,
    ...
    const committer = dependencies.createWorkflowFileCommitter(
      {
        operationDomain: dependencies.operationDomain,
    ...
    const [defaultVerify, defaultDispatcher, defaultLegacyDeploy] =
      await Promise.all([
        ...
        dependencies.fetchFileFromRepoResult(
          targetRepo,
          `.github/workflows/${dependencies.legacyDeployWorkflowFile}`,
          defaultBranch,
          selectedExecutor
        )
      ]);
      ...
      if (pr.ok) {
        ...
        steps.push(
          '⚠️ Committed workflows to branch "' +
            ...
            "GitHub confirmed the pull request was not created." +
    ...
    if (verifyPlan.trigger === "none") {
      verifySkipReason = verifyPlan.skipReason;
      steps.push(
        `⏭️ Skipping credential verification until the pull request is merged — ${verifyPlan.skipReason}.`
      );
    ...
    // (newly-triggered) runs rather than any stale runs.
    {
      ...
      if (!prState) {
        dependencies.recordCommitState(operation, {
          ...
          baseBranch: verifyPlan.defaultBranch,
          pullRequestUrl: pullRequestUrl || null
        });
      }
      if (!(await checkpoint("after-verification-dispatch"))) return;
      dependencies.verificationDispatched({
        dispatchedAt,
        runId: verifyRunId,
        runUrl: verifyRunUrl
      });
    ...
    if (actionRequired) {
      ...
      dependencies.finish(operation, "action_required", {
        terminal: {
          ...
          baseBranch: prState?.base || verifyPlan.defaultBranch,
    ...
    respond(200, {
      ...
      pullRequestBaseBranch:
        actionRequired ? prState?.base || verifyPlan.defaultBranch : null,
