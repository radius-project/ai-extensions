...
import {
  createWorkflowFileCommitter as createCommitter,
  isProtectedBranchFailure,
  readWorkflowCommitProvenance,
  recordedSetupBranchCreate as recordedWithDomain,
...
const workflowContentDigest = (value: string) =>
  createHash("sha256").update(Buffer.from(value, "base64")).digest("hex");
const createWorkflowFileCommitter = (
  ports: Parameters<typeof createCommitter>[0],
  target: Parameters<typeof createCommitter>[1]
) =>
  createCommitter(ports, target, {
    workflowContentDigest,
    workflowBlobSha: (value) => {
...
const recordedSetupBranchCreate = (
  operation: Parameters<typeof recordedWithDomain>[0],
  repo: string
) => recordedWithDomain(operation, repo, operationDomain);
const providerRecoveryManualGuidance =
  operationDomain.providerRecoveryManualGuidance;
...
describe("workflow publication recovery boundaries", () => {
  const branch = "radius/setup-dev-workflows-workflow";
  const branchTarget = `octo/app\0${branch}\0sha-1`;
  const marker = "radius-operation:op_workflow:workflow:p:fff71b97a5a94949";
  const absent = { code: 1, stderr: "HTTP 404: Not Found" };

  function recoveringBranch() {
  ...
  it("honors Stop for a concurrent fallback after the first branch workflow has completed", async () => {
    function signal() {
      let resolve: () => void = () => {
    ...
    const secondDirect = signal();
    const releaseSecondDirect = signal();
    const firstFallback = signal();
    const secondFallback = signal();
    const releaseFirstRead = signal();
    const releaseSecondRead = signal();
    const operation = createOperation({ operationId: "op_workflow" });
    const h = harness({
      defaultBranch: "main",
      headSha: "sha-1",
    ...
    h.ports.mutationRecovery = {
      operation,
      persist: async () => {},
      beforeMutation: async () => !stopped
    };
    h.ports.runGh = async (args) => {
      if (args[1].includes("first?ref=")) {
        firstFallback.resolve();
        await releaseFirstRead.promise;
      } else if (args[1].includes("second?ref=")) {
        secondFallback.resolve();
        await releaseSecondRead.promise;
      }
      return { code: 1, stdout: "", stderr: "HTTP 404: Not Found" };
    };
    h.ports.runGhWorkflow = async () => {
      writes++;
      if (writes === 1) {
        await secondDirect.promise;
        return { code: 1, stdout: "", stderr: "protected branch" };
      }
      if (writes === 2) {
        secondDirect.resolve();
        await releaseSecondDirect.promise;
        return { code: 1, stdout: "", stderr: "protected branch" };
      }
      return { code: 0, stdout: PUT_RESPONSE, stderr: "" };
    };
    const committer = createWorkflowFileCommitter(h.ports, target);
    const first = committer.commitWorkflowFileSmart("first", CONTENT, "m");
    const second = committer.commitWorkflowFileSmart("second", CONTENT, "m");
    await firstFallback.promise;
    releaseSecondDirect.resolve();
    await secondFallback.promise;
    releaseFirstRead.resolve();
    expect(await first).toMatchObject({ ok: true, viaPr: true });
    stopped = true;
    releaseSecondRead.resolve();
    expect(await second).toMatchObject({ cancelled: true, viaPr: true });
    expect(writes).toBe(3);
    expect(h.tempRemovals).toHaveLength(h.tempWrites.length);
  });

  it("reuses a fallback branch established while another direct write was in flight", async () => {
    ...
    const h = harness({
      defaultBranch: "main",
      headSha: "sha-1",
      runGh: [absent, absent, absent, absent]
    });
    let writes = 0;
    let branches = 0;
    h.ports.createBranchRef = async () => {
      branches++;
    ...
    expect(await first).toMatchObject({ ok: true, viaPr: true });
    finishSecond();
    expect(await second).toMatchObject({ ok: true, viaPr: true });
    expect(branches).toBe(1);
    expect(writes).toBe(4);
  });

  it.each(["workflow", "branch"])(
    "does not send a %s mutation when Stop arrives without a host cancellation callback",
    async (stage) => {
      const operation = createOperation({ operationId: "op_workflow" });
      const h = harness({
        defaultBranch: "main",
            ...
            operation.providerRecovery.mutations.some(
              (mutation) =>
                mutation.kind ===
                  (stage === "workflow" ?
                    "github_workflow.put"
                  : "github_branch.create") && mutation.status === "prepared"
            )
          )
      ...
      ).toHaveLength(stage === "workflow" ? 0 : 1);
      expect(h.tempRemovals).toHaveLength(h.tempWrites.length);
    }
  );

  it("uses stdout when a destructive branch rejection has no stderr", async () => {
    const operation = recoveringBranch();
    operation.recoveryState = "provider_reconciliation_pending";
    const h = harness({
      defaultBranch: "main",
      runGh: [{ stdout: '{"object":{"sha":"sha-1"}}' }],
      runGhWorkflow: [
        { code: 1, stdout: "HTTP 403: protected branch", stderr: "" }
      ]
    });
  ...
  it("refuses a second delete when the recovered branch gained replacement work", async () => {
    const operation = recoveringBranch();
    operation.recoveryState = "provider_reconciliation_pending";
    const h = harness({
      defaultBranch: "main",
      runGh: [
        { stdout: '{"object":{"sha":"sha-1"}}' },
        { stdout: JSON.stringify([{ ref: `refs/heads/${branch}` }]) },
        { stdout: '{"object":{"sha":"replacement-work"}}' }
      ],
      runGhWorkflow: [{ code: 1, stderr: "timeout", timedOut: true }]
    });
    h.ports.mutationRecovery = { operation, persist: async () => {} };
    await expect(
      createWorkflowFileCommitter(h.ports, target).commitWorkflowFileSmart(
  ...
  it.each([
    { code: 1, stderr: "HTTP 500: unavailable", stdout: "" },
    { code: 1, stderr: "", stdout: "provider unavailable" },
    { code: 1, stderr: "", stdout: "" },
    { code: 0, stdout: "invalid JSON" },
    { code: 0, stdout: "{}" },
    { code: 0, stdout: '{"object":{"sha":42}}' }
  ])(
    "does not replay a branch create when its identity cannot be read: %j",
    async (response) => {
      const operation = recoveringBranch();
      const h = harness({ defaultBranch: "main", runGh: [response] });
      h.ports.mutationRecovery = { operation, persist: async () => {} };
      await expect(
        createWorkflowFileCommitter(h.ports, target).commitWorkflowFileSmart(
          "p",
  ...
  it("refuses to adopt an operation-specific branch that moved to unrelated work", async () => {
    const operation = recoveringBranch();
    const h = harness({
      defaultBranch: "main",
      runGh: [{ stdout: '{"object":{"sha":"other-work"}}' }]
    });
    h.ports.mutationRecovery = { operation, persist: async () => {} };
    await expect(
      createWorkflowFileCommitter(h.ports, target).commitWorkflowFileSmart(
  ...
  it.each([true, false])(
    "requires complete branch absence proof after an interrupted create: %s",
    async (confirmed) => {
      const operation = recoveringBranch();
      const h = harness({
        defaultBranch: "main",
        runGh: [
          absent,
          confirmed ? { stdout: "[]" } : { code: 1, stderr: "HTTP 403" },
          ...(confirmed ? [absent] : [])
        ]
      });
      h.ports.mutationRecovery = { operation, persist: async () => {} };
      const result = createWorkflowFileCommitter(
        h.ports,
        target
      ).commitWorkflowFileSmart("p", CONTENT, "m");
      if (confirmed)
        await expect(result).rejects.toThrow("could not create branch");
      else
        await expect(result).rejects.toMatchObject({
          code: "provider-mutation-outcome-unknown"
        });
      expect(operation.providerRecovery.mutations[0].status).toBe(
        confirmed ? "not_applied" : "outcome_unknown"
      );
      expect(h.calls.every((call) => call.kind === "runGh")).toBe(true);
    }
  );

  it.each([
    "before workflow",
    "after workflow checkpoint",
    "before branch",
    "after branch checkpoint"
  ])("honors Stop %s without issuing the gated mutation", async (stage) => {
    const operation = createOperation({ operationId: "op_workflow" });
    const h = harness({
      defaultBranch: "main",
      headSha: "sha-1",
    ...
    h.ports.mutationRecovery = {
      ...
      beforeMutation: async (kind) => {
        ...
        return !(
          ...
          (stage === "before workflow" && kind === "github_workflow.put") ||
          (stage === "before branch" && kind === "github_branch.create")
        );
      },
      persist: async () => {
        const kind =
          stage === "after workflow checkpoint" ? "github_workflow.put"
          : stage === "after branch checkpoint" ? "github_branch.create"
          : "";
        if (
          operation.providerRecovery.mutations.some(
            (mutation) =>
              mutation.kind === kind && mutation.status === "prepared"
          )
        ) {
          requestStop(operation);
        }
    ...
    expect(
      await createWorkflowFileCommitter(
        h.ports,
        target
      ).commitWorkflowFileSmart("p", CONTENT, "m")
    ).toMatchObject({
    ...
    expect(
      h.calls.filter((call) => call.kind === "runGhWorkflow")
    ).toHaveLength(stage.includes("branch") ? 1 : 0);
    expect(boundaryCalls).toBeGreaterThan(0);
    expect(h.tempRemovals).toHaveLength(h.tempWrites.length);
  });

  it("cancels a later PR-branch write without touching its workflow", async () => {
    const operation = createOperation({ operationId: "op_workflow" });
    const h = harness({
      defaultBranch: "main",
      headSha: "sha-1",
      createBranch: { ok: true, stderr: "" },
      runGh: [absent, absent, absent],
      runGhWorkflow: [
    ...
    ).toMatchObject({ ok: true, viaPr: true });
    stopped = true;
    expect(
      await committer.commitWorkflowFileSmart("second", CONTENT, "m")
    ).toMatchObject({ cancelled: true, viaPr: true });
    expect(
      h.calls.filter((call) => call.kind === "runGhWorkflow")
    ).toHaveLength(2);
  });
  ...
  it.each([
    ...
    { code: 1, stderr: "HTTP 500", stdout: "" },
    { code: 1, stderr: "", stdout: "provider unavailable" },
    { code: 1, stderr: "", stdout: "" },
    { code: 0, stdout: "not JSON" },
    { code: 0, stdout: '{"sha":"blob"}' },
    { code: 0, stdout: JSON.stringify({ content: CONTENT }) },
    { code: 0, stdout: JSON.stringify({ sha: "", content: CONTENT }) }
  ])(
    "keeps unreadable or absent workflow evidence from authorizing a replay: %j",
    async (response) => {
      const operation = createOperation({ operationId: "op_workflow" });
      prepareProviderMutation(operation, {
        kind: "github_workflow.put",
        target: "octo/app:<default>:p",
        intent: {
          branch: "<default>",
          path: "p",
      ...
      const h = harness({ runGh: [absent, response] });
      h.ports.mutationRecovery = { operation, persist: async () => {} };
      const result = createWorkflowFileCommitter(
        h.ports,
        target
      ).commitWorkflowFileSmart("p", CONTENT, "m");
      if (response.stderr === "HTTP 404") {
      ...
      expect(h.calls.every((call) => call.kind === "runGh")).toBe(true);
      expect(h.tempRemovals).toHaveLength(1);
    }
  );
  ...
  it.each([
    ...
    "refuses to adopt equal workflow bytes with %s provenance",
    async (variant) => {
      const operation = createOperation({ operationId: "op_workflow" });
      const intent =
        variant === "missing intent" ? undefined : (
          {
            branch: variant === "wrong branch" ? "other" : "<default>",
            path: variant === "wrong path" ? "other" : "p",
            operationMarker: marker,
            contentSha256:
              variant === "wrong digest" ? "different" : CONTENT_DIGEST,
            ...(variant === "missing predecessor" ?
              {}
            : { previousBlobKnown: true })
          }
        );
      ...
      const h = harness({
        runGh: [
          absent,
          { stdout: JSON.stringify({ sha: "blob", content: CONTENT }) }
        ]
      });
      h.ports.mutationRecovery = { operation, persist: async () => {} };
      await expect(
...
describe("what survives a crash during a recovered branch delete", () => {
  ...
  it("keeps the blocker after the refused record is reloaded", async () => {
    ...
    const reloaded = structuredClone(operation);
...
describe("the predecessor blob a workflow retry records", () => {
  ...
  it("captures the blob the retry actually read, not the refused attempt's", async () => {
    ...
    expect(
      operation.providerRecovery.mutations[0].intent?.previousBlobSha
    ).toBe("blob-after");
  });
...
describe("recovering a setup branch after the default branch moved", () => {
  ...
  it.each(["prepared", "outcome_unknown"] as const)(
    "reconciles a %s branch workflow after that workflow advanced the branch",
    async (status) => {
      const operation = createOperation({ operationId: "op_workflow" });
      operation.recoveryState = "provider_reconciliation_pending";
      ...
      const marker = "radius-operation:op_workflow:workflow:p:fff71b97a5a94949";
      const workflowMutation = prepareProviderMutation(operation, {
        kind: "github_workflow.put",
