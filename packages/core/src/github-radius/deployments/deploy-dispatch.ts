...
} from "../environments/gh-command-display.js";
import type { DeploymentState, DeployErrorKind } from "./types.js";
import { buildEnvironmentSuffix } from "../../platforms/index.js";
import { remediationView } from "../../remediations.js";
...
export function createDeployDispatchService(
  ...
  const withStrippedToken = async (
    ...
    return second.code === 0 ?
      ...
      : {
          result: second.timedOut ? { ...first, timedOut: true } : first,
  ...
  const appendRadCommands = async (
    ...
    const selectedAppFile =
      entry.state.deployAttempt?.appFile || entry.state.appFile;
    try {
      let bicepPath = selectedAppFile || ".radius/app.bicep";
      ...
      if (!isCurrent()) return null;
      if (!bicepSource && !selectedAppFile) {
      ...
      if (!bicepSource) {
        if (selectedAppFile) {
          return `Could not read the selected application file "${selectedAppFile}" on ${repo}@${branch}; deployment was not started.`;
        }
        log(
      ...
      if (selectedAppFile) {
        return `Could not prepare the selected application file "${selectedAppFile}": ${dependencies.errorMessage(e)}. Deployment was not started.`;
      }
      log(
  ...
  return {
    async prepareAndDispatch(request) {
      ...
      const isCurrent = () =>
        entry.state.deployAttempt?.id === attemptId &&
      ...
      const missingBranch = await branchIsMissingOnRemote(repo, deployRef);
      if (!isCurrent()) return { dispatched: false };
      if (missingBranch) {
      ...
      if (dispatchDeployRes.code !== 0) {
        ...
        const dispatchKind =
          dispatchDeployRes.timedOut ? "run-unconfirmed" : (
            dependencies.classifyDeployDispatchFailure(de)
          );
        ...
        const installationNote = presentation.installationNote;
        ...
        const scopeHint =
          ...
          : needsWorkflowScope(de) ?
            ...
            : ` Your stored GitHub CLI credential is missing the "workflow" scope. ${installationNote}`
          : " Ensure " +
            deployWorkflowFile +
            ' exists on branch "' +
            deployRef +
