...
export interface DeployRequestDependencies {
  ...
  canvasGraphResources(values: unknown[]): DeploymentResource[];
  beginDeployAttempt(
    state: DeploymentState,
    input: {
      repo: string;
  ...
  onSettled(entry: DeployRequestInstanceEntry): void;
  monitor: DeployMonitorService;
  unconfirmedRunKind: DeploymentState["deployErrorKind"];
  repairAttemptCap: number;
  errorMessage(error: unknown): string;
  redactDiagnostics(text: string): string;
}
...
const REQUIRED_DEPENDENCIES: readonly (keyof Omit<
...
export function assertDeployRequestPorts(
  dependencies: Omit<DeployRequestDependencies, "onSettled">
): void {
...
export function createDeployRequestService(
  ...
  assertDeployRequestPorts(dependencies);
  assertDeployDependencies("createDeployRequestService", dependencies, [
    "onSettled"
  ]);
  return {
    async deploy({ state, target, source, attemptId }) {
      let reservation: DeploymentReservation | null = null;
      let reservationOwner: DeploymentState | null = null;
      ...
      try {
        ...
        const previous = state.deployAttempt;
        if (
          loop.repairLoop &&
          previous &&
          ((previous.targetRepo && previous.targetRepo !== target.repo) ||
            (previous.environment &&
              previous.environment !== target.environment) ||
            (previous.branch && previous.branch !== source.branch) ||
            (previous.provider &&
              previous.provider !== (target.provider || "azure")) ||
            (previous.appFile &&
              previous.appFile !== (source.appFile || ".radius/app.bicep")))
        ) {
          return {
            status: 409,
            body: {
              error: `Deploy attempt "${loop.attemptId}" is bound to its original repository, environment, branch, provider, and application file. Nothing was deployed.`
            }
          };
        }
        ...
        if (source.repo !== repo)
          throw new Error(
            "Deployment source and target repositories must match."
          );
        if (!repo || !environment) {
        ...
        const generation = entry.state.deployGeneration;
        const isCurrent = () =>
          entry.state.deployAttempt?.id === lease.attemptId &&
          entry.state.deployGeneration === generation;
        const addLog = (msg: string): void => {
          if (!isCurrent()) return;
          const dl = entry.state.deployLogs || [];
          entry.state.deployLogs = dl;
          dl.push(dependencies.redactDiagnostics(msg));
        ...
        void dependencies.monitor
          ...
          .catch((monErr: unknown) => {
            ...
            try {
              const reason = dependencies.redactDiagnostics(
                monitorFailureText(monErr)
              );
          ...
          .finally(() => {
            ...
            try {
              if (!isCurrent()) return;
              if (entry.state.deployError)
                entry.state.deployError = dependencies.redactDiagnostics(
                  entry.state.deployError
                );
              dependencies.onSettled(entry);
            } finally {
              releaseReservation();
            }
          })
