...
import type { ArtifactFiles, WorkflowArtifact } from "./deploy-artifacts.js";
...
describe("probeDeleteConflict", () => {
  it.each([undefined, { id: 999 }])(
    "refuses force evidence with mismatched run identity %j",
    async (workflow_run) => {
      const { calls, run } = probe({
        listArtifacts: async () => [resultArtifact({ workflow_run })]
      });
      expect(await run()).toMatchObject({
        state: "unknown",
        detail: expect.stringContaining("does not identify")
      });
