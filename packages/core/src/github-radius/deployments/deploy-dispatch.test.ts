...
import type { BicepParam } from "./dispatch-ports.js";
import type { DeploymentState as CanvasState } from "./types.js";
import type {
  GitHubJsonResponse,
  GitHubJsonRunner
} from "../environments/azure-oidc.js";
...
describe("dispatch attempt identity", () => {
  it.each(["root source", "secret presence", "Azure validation"])(
    "does not continue stale %s preparation",
    async (stage) => {
      const { input, state } = request({ deployGeneration: 1 });
      if (stage === "Azure validation") input.provider = "azure";
      let reads = 0;
      let mutations = 0;
      ...
      const service = createDeployDispatchService(
        dependencies({
          ...(stage === "Azure validation" ? azure : {}),
          fetchFileForSelection: async () => {
            reads++;
            if (stage === "root source") {
              if (reads === 2) state.deployGeneration = 2;
              return null;
            }
            return "model";
          },
      ...
      expect(state.deployGeneration).toBe(2);
      expect(mutations).toBe(0);
      expect(state.deployStatus).toBeUndefined();
    }
  );

  it("dispatches only the attempt's selected application file", async () => {
    const { input } = request({
      deployAttempt: { id: "attempt", appFile: "models/production.bicep" }
    });
    const paths: string[] = [];
    ...
    const service = createDeployDispatchService(
      dependencies({
        ...gh,
        fetchFileForSelection: async (_entry, _repo, _branch, path) => {
          paths.push(path);
          return "model";
        },
        buildDeployRadCommand: (path) => `rad deploy ${path}`
      })
    );
    expect(await service.prepareAndDispatch(input)).toMatchObject({
      dispatched: true
    });
    expect(paths).toEqual(["models/production.bicep"]);
    expect(gh.calls[0].args).toContain(
      'rad_commands=["rad deploy models/production.bicep"]'
    );
  });

  it.each(["missing", "unavailable"])(
    "does not dispatch fallback commands when the selected file is %s",
    async (result) => {
      ...
      expect(state.deployError).toContain("models/production.bicep");
  ...
  it.each([
    ...
    "dispatch",
    "retry"
  ])("ignores a stale generation after %s", async (stage) => {
    const { input, state } = request({
      deployGeneration: 1,
    ...
    const replace = () => {
      ...
      state.deployError = "replacement";
    };
    let dispatches = 0;
    const service = createDeployDispatchService(
      dependencies({
        getBranchHeadSha: async () => {
          if (stage === "branch") replace();
          return "sha";
        },
        fetchFileForSelection: async () => {
          if (stage === "source") replace();
          return null;
        },
        ensureDeployWorkflowsOnBranch: async () => {
          calls.push("publish");
          if (stage === "workflow publication") replace();
        },
        ensureWorkflowsCurrent: async () => {
          calls.push("sync");
          if (stage === "workflow sync") replace();
          return {};
        },
        latestWorkflowRunId: async () => {
          if (stage === "baseline") replace();
          return null;
        },
        readProcessEnv: () => ({ GH_TOKEN: "placeholder" }),
        runGh: async () => {
          dispatches++;
          if (stage === "dispatch" || (stage === "retry" && dispatches === 2))
            replace();
          return { code: 1, stdout: "", stderr: "missing workflow scope" };
        }
      })
    );
    expect(await service.prepareAndDispatch(input)).toEqual({
    ...
    if (stage === "workflow publication") expect(calls).toEqual(["publish"]);
  });

  it("does not replace current state with a stale branch failure", async () => {
    const { input, state } = request({ deployGeneration: 1 });
    const service = createDeployDispatchService(
      dependencies({
    ...
    expect(state.deployError).toBe("replacement");
  });
});
...
describe("deploy dispatch workflow publication and dispatch", () => {
  ...
  it("preserves uncertainty when a credential retry times out despite a misleading original error", async () => {
    const { input, state } = request();
    const gh = recordingGh([
      { code: 1, stderr: "missing workflow scope", stdout: "" },
      { code: 1, stderr: "request timed out", stdout: "", timedOut: true }
    ]);
    const service = createDeployDispatchService(
      dependencies({
        ...gh,
    ...
    expect(gh.calls).toHaveLength(2);
    expect(state.deployError).toContain("missing workflow scope");
  });
