...
export interface DeployOutcomeDependencies {
  ...
  cloudAuthDriftKind: DeploymentState["deployErrorKind"];
...
export function createDeployOutcomeService(
  ...
  const readDeployedGraph = async (
    ...
    deployed: DeploymentResource[] | null;
    graphStatus: string | null;
  }> => {
    let deployed: DeploymentResource[] | null = null;
  ...
  const describeFailure = async (
    ...
    const rawLog = await dependencies.fetchRunLog(repo, runId);
    const failLog =
      rawLog === null ? null : dependencies.redactDiagnostics(rawLog);
    ...
    try {
      const rawControlPlaneLog = await statusReader.controlPlaneLog();
      cpLog =
        rawControlPlaneLog === null ? null : (
          dependencies.redactDiagnostics(rawControlPlaneLog)
        );
    } catch {
    ...
    dErr +=
      ...
      runId;
    return {
      message: dependencies.redactDiagnostics(dErr),
      radiusError: detailBlock
    };
  };

  return {
    async settle(request) {
      const attemptId = request.entry.state.deployAttempt?.id;
      const generation = request.entry.state.deployGeneration;
      const isCurrent = () =>
        request.entry.state.deployAttempt?.id === attemptId &&
        request.entry.state.deployGeneration === generation;
      const {
        entry,
        repo,
        provider,
      ...
      try {
        ...
        log(
          "  ⚠ The final resource status could not be read; the workflow conclusion is retained."
        );
      }
      if (!isCurrent()) return;
