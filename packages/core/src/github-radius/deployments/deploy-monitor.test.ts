...
describe("deploy monitor stage sequencing", () => {
  it.each([
    ...
    "detail",
    "progress"
  ] as const)("discards stale attempt evidence after %s", async (boundary) => {
    const fixture = request({
      resources: boundary === "planned" ? [] : [{ name: "app" }]
    });
    const supersede = () => {
      fixture.state.deployAttempt = { id: "new-attempt" };
      fixture.state.deployGeneration = 2;
      fixture.state.deployStatus = "in_progress";
    };
    fixture.state.deployAttempt = { id: "old-attempt" };
    fixture.state.deployGeneration = 1;
    ...
    const deps = dependencies({
      ...
      dispatch: {
        prepareAndDispatch: async () => {
          if (boundary === "dispatch") supersede();
          return {
            dispatched: true,
      ...
      findWorkflowRun: async () => {
        if (boundary === "discovery") supersede();
        return 77;
      },
      createStatusReader: async () => {
        if (boundary === "reader") supersede();
        return progress;
      },
      getRunDetail: async () => {
        if (boundary === "detail") supersede();
    ...
    await createDeployMonitorService(deps).run(fixture.request);
    expect(fixture.state).toMatchObject({
      deployAttempt: { id: "new-attempt" },
      deployGeneration: 2,
      deployStatus: "in_progress"
    });
    expect(fixture.state.deployError).toBeUndefined();
  });
...
describe("deploy monitor progress streaming", () => {
  it("redacts artifact diagnostics before applying them to observable resources", async () => {
    const fixture = request({ resources: [{ name: "app" }] });
    let polls = 0;
    ...
    await createDeployMonitorService(
      dependencies({
        getRunDetail: async () =>
          ...
          : completedRun(),
        createStatusReader: async () => reader(),
        buildDeployMessageMap: () =>
          new Map([["app", "recipe failed opaque-value"]]),
        redactDiagnostics: (text) =>
          text.replaceAll("opaque-value", "[REDACTED]"),
        applyDeployMessages: (_resources, messages) => {
          applied.push(...messages.values());
        },
        outcome: { settle: async () => {} }
      })
    ).run(fixture.request);
    expect(applied).toEqual(["recipe failed [REDACTED]"]);
  });
