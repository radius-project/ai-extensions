...
import type {
  DeploymentResource as CanvasGraphResource,
  DeploymentState as CanvasState
} from "./types.js";
import {
...
describe("deploy outcome construction", () => {
  it("stores an envelope's validated resource array after rejecting incompatible evidence", async () => {
    const { request, state } = outcomeRequest({
      statusReader: statusReader([
        { status: "ok", graph: { resources: [{ name: 42 }] } },
        {
          status: "ok",
          graph: { resources: [{ name: "web" }], application: "app" }
        }
      ])
    });
    await createDeployOutcomeService(dependencies()).settle(request);
    expect(state.deployedGraph).toEqual([{ name: "web" }]);
    expect(state.deployStatus).toBe("complete");
  });
  it.each(["progress", "failure-log", "failure-log-error"] as const)(
    "ignores stale %s completion",
    async (boundary) => {
      const { request, state } = outcomeRequest({ conclusion: "failure" });
      state.deployAttempt = { id: "old" };
      const replace = () => {
        state.deployAttempt = { id: "new" };
        state.deployStatus = "in_progress";
        state.deployError = null;
      };
      if (boundary === "progress")
        request.pollDeployStatus = async () => {
          replace();
        };
  ...
  it("preserves confirmed execution failure when graph and final status reads fail", async () => {
    const { request, state } = outcomeRequest({
      conclusion: "failure",
      statusReader: {
        graph: async () => {
        ...
        controlPlaneLog: async () => "credential=opaque-value"
      },
    ...
    await createDeployOutcomeService(
      dependencies({
        ...
        extractRadDeployError: (text) => text || "",
        redactDiagnostics: (text) =>
          text.replaceAll("opaque-value", "[REDACTED]")
      })
    ).settle(request);
    expect(state.deployStatus).toBe("failed");
    expect(state.deployErrorKind).not.toBe("run-unconfirmed");
    expect(state.deployError).toContain("recipe failed [REDACTED]");
    expect(state.deployError).not.toContain("opaque-value");
  });

  it("ignores evidence from an attempt replaced while the graph was being read", async () => {
    const { request, state } = outcomeRequest();
    state.deployAttempt = { id: "old" };
    request.statusReader.graph = async () => {
      state.deployAttempt = { id: "new" };
      state.deployStatus = "in_progress";
      return { graph: [{ name: "old" }], status: "ok" };
    };
    await createDeployOutcomeService(dependencies()).settle(request);
    expect(state).toMatchObject({
      deployAttempt: { id: "new" },
      deployStatus: "in_progress"
    });
    expect(state.deployedGraph).toBeUndefined();
    expect(state.deployFinishedAt).toBeUndefined();
  });
  it("requires diagnostic redaction at construction", () => {
    const incomplete = dependencies();
    Reflect.deleteProperty(incomplete, "redactDiagnostics");
