...
describe("sanitizeArtifactSegment", () => {
  it("contains an unexpected artifact-port processing failure as an observation error", async () => {
  ...
  it("sorts incomplete artifact metadata deterministically", () => {
    const entries = [
      artifact("radius-deploy-status-dev-a", { id: 0, created_at: undefined }),
      artifact("radius-deploy-status-dev-b", { id: 2, created_at: "invalid" }),
      artifact("radius-deploy-status-dev-c", { id: 1, created_at: "" }),
      artifact("radius-deploy-status-dev-d", { id: 0, created_at: undefined })
    ];
    expect(
      selectDeployStatusArtifacts(entries, "dev").map((entry) => entry.id)
    ).toEqual([2, 1, 0, 0]);
  });

  it("rejects scalar progress and normalizes absent optional resource metadata", () => {
    ...
    expect(
      ...
      state: undefined,
      resources: [{ name: "web", type: "" }]
    });
  });

  it("parses escaped graph strings and all supported resource identity shapes", () => {
    for (const resource of [
      { type: "custom" },
      { connections: {} },
      { outputResources: [] },
      { name: 'quoted "name" with \\ slash' }
    ]) {
      expect(parseDeployGraphArtifact(JSON.stringify([resource]))).toEqual([
        resource
      ]);
    }
    expect(parseDeployGraphArtifact('{] [{"name":"valid"}]')).toEqual([
      { name: "valid" }
    ]);
    expect(parseDeployGraphArtifact("[{}]")).toBeNull();
  });

  it("defaults status to pending and leaves identical observations unchanged", () => {
    const resources = [{ name: "web" }];
    expect(
      applyDeployStatusToResources(resources, new Map([["web", "pending"]]))
    ).toEqual([]);
    expect(
      applyDeployStatusToResources(resources, new Map([["web", "success"]]))
    ).toEqual([{ name: "web", from: "pending", to: "success" }]);
  });

  it("distinguishes primitive listing failures and authenticated download refusals", async () => {
    const reader = createDeployStatusReader({
      repo: "acme/app",
      listArtifacts: async () => {
        throw "offline";
    ...
    expect((await reader.read()).status).toBe("error");
    const authReader = createDeployStatusReader({
      repo: "acme/app",
      listArtifacts: async () => [artifact("radius-deploy-status-dev-app")],
      downloadArtifact: async () => {
        throw { code: "GH_ARTIFACT_AUTH" };
      }
    });
    expect((await authReader.read()).status).toBe("auth");
  });

  it("retains valid progress when the companion graph is malformed", async () => {
    const reader = createDeployStatusReader({
      repo: "acme/app",
      listArtifacts: async () => [artifact("radius-deploy-status-dev-app")],
      downloadArtifact: async () => ({
        [DEPLOY_STATUS_FILES.progress]: progressPayload(),
        [DEPLOY_STATUS_FILES.graph]: "{malformed"
      })
    });
    expect(await reader.read()).toMatchObject({ status: "ok", graph: null });
  });

  it("requires explicit listing and download ports for an independent reader", () => {
    const options = {
      repo: "acme/app",
      listArtifacts: async () => [],
      downloadArtifact: async () => null
    };
    Reflect.deleteProperty(options, "listArtifacts");
    expect(() => createDeployStatusReader(options)).toThrow(
      "requires listing and download ports"
    );
  });
