...
// Upstream verify-template file names in
// radius-project/ai-extensions/.github/extension. The extension fetches these
// from the ai-extensions repo at commit time so user repos get the reviewed
// upstream version; the extension bundles no copies of its own.
 ...
 *
 * `template` is the raw body fetched from `radius-project/ai-extensions`; there
 * is no bundled fallback, so the caller must supply it. Generation fails if any
...
export function generateVerifyWorkflow(
  ...
  if (!template || !template.trim()) {
    throw new Error(
      `Missing verify template for platform "${platform.id}". It must be fetched from radius-project/ai-extensions/.github/extension.`
    );
  }
