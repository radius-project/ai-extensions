...
The delete workflows run with the same cloud credentials but must never gate anything, or a stale pin could leave someone unable to tear down the resources they are paying for. They are therefore repaired alongside the deploy workflows whenever a deployment upgrade is confirmed, so a single confirmation brings every workflow the extension committed up to date rather than leaving the delete path frozen at whatever commit created it.
