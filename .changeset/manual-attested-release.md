...
via `scripts/version.mjs`, attests the built canvas bundle's provenance, and
publishes the `release` branch plus an immutable `v<version>` tag and GitHub
release. CI now also verifies that the manifest versions stay in sync.
