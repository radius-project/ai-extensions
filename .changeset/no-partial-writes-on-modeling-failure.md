...
A run now writes everything into `.radius/.staging-<runId>/` and a new bundled script, `promote-app-model.mjs`, moves it into `.radius/` as the last step. The staging directory sits inside `.radius/` so the publish is a rename within one filesystem, which either happens or does not, rather than a cross-filesystem copy that can fail halfway. The publish refuses unless the run carries the record `--begin` wrote, holds a complete set of files, its origin record describes the application model it produced, and every file it would replace is still the one the run started from. Requiring the record is what makes `--begin` mandatory rather than merely recommended: a staging directory assembled without it cannot be published. Because the origin record is only written after the Bicep checker passes, an application model that never compiled can never be published; and because `git add` now runs only after a successful publish, a failed run leaves nothing staged.

A hand edit made to any file the run would replace — the model, `bicepconfig.json`, a custom-type manifest, a recipe pack — is never overwritten: the publish refuses, the generated model is discarded, and the user is told their files are intact. A run may only publish files it recognizes, so anything else left in the staging directory is discarded with it rather than committed on the strength of being present. A staging directory left behind by an interrupted run is removed at the start of the next run, and `.staging-*/` is ignored so one cannot leave untracked noise in `git status`. On any failure the run reports plainly that nothing was written, and offers a retry only when the failure looks transient — a network or registry error — rather than permanent, such as a repository with no Dockerfile.
